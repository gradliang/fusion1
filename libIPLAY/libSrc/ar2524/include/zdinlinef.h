...
)
{
	...
	card_Setting_t		*pCardSetting = &macp->cardSetting;
    ...
    if (pWlaHdr->DA[0] & BIT_0){        /* Multicast */
        bGroupAddr = 1;
        
        if(memcmp(pWlaHdr->DA, BCAddr,6) == 0) // Use lowest rate for bc and general rate for mc
        {
            if(PURE_A_MODE != pCardSetting->MacMode) 
            {
                Rate = RATE_1M;
                Preamble = 0;
            }
            else if(PURE_A_MODE == pCardSetting->MacMode) 
            {
                Rate = RATE_6M; 
                Preamble =1;
            }
    ...
    if ((FrameType & 0x0c) == DATA && !macp->bContinueTx)
    {
        write_str(DbgStr101, EnCipher);
        if(macp->RF_Mode == UW2453_RF || macp->RF_Mode == AR2124_RF)
        {
            if (pCardSetting->CurrTxRate > 3 && dot11Obj.UW2453CCKSetFlag == 1)
            {
                dot11Obj.UW2453CCKSetFlag = 2;
            }
            else if (pCardSetting->CurrTxRate <= 3 && dot11Obj.UW2453CCKSetFlag == 0)
            {
                dot11Obj.UW2453CCKSetFlag = 3;
            }
        }
    }
#endif
	...
	pCardSetting->CurrTxRate = TxRate;

	/* Length in byte */
	if (EnCipher){
		if (!pCardSetting->SwCipher){
        ...
        { //use software encryption
			if (pCardSetting->DynKeyMode == DYN_KEY_TKIP){
				if ((pWlaHdr->DA[0] & BIT_0) && (pCardSetting->WpaBcKeyLen != 32)) { //multicast
	...
	if (Len > pCardSetting->RTSThreshold){
	...
	if ((EnCipher) && (!pCardSetting->SwCipher)){
		tmp |= BIT_6;
	}
