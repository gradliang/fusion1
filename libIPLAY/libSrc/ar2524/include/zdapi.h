...
//#ifdef ZDCONF_PORTING_BIGENDIAN
    ...
    #define zd_cpu_to_le32(x)   (cpu_to_le32(x))
    #define zd_le32_to_cpu(x)   (le32_to_cpu(x))
    #define zd_cpu_to_le16(x)   (cpu_to_le16(x))
    #define zd_le16_to_cpu(x)   (le16_to_cpu(x))
#endif                       
...
#define INFRASTRUCTURE_BSS		0x1             /* STA mode */
#define PSEUDO_IBSS				0x3
#define	AP_BSS					0x4

//RxFilter
#define AP_RX_FILTER			0x0400ffff
...
typedef struct card_Setting_s
{
    ...
    U8              appie[5][256/*IEEE80211_MAX_WSC_IE*/];
    U32             appieLen[5];
#endif
	U8		WPASupport;
	U8		Rate275;
...
typedef	struct zd_80211Obj_s
{
	...
	void 	(* ExitCS)(unsigned long flags);								// enable interrupt
	unsigned long	(* EnterCS)(void);								// disable interrupt
