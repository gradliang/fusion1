...
} __attribute__ ((__packed__)) zd1205_RFD_t;
...
typedef struct zd1205_SwTcb_s {
	...
	dma_addr_t FirstTbdPhys;

	zd1205_Ctrl_Set_t *pHwCtrlPtr;
	dma_addr_t HwCtrlPhys;

	zd1205_Header_t *pHwHeaderPtr;
	dma_addr_t HwHeaderPhys;
	u32 TcbCount;
	u8 LastFrag;
	u8 MsgID;
	u8 FrameType;	
	u8 Rate;	
	u16 aid;
	u8 bIntraBss;                               /* pkt from another STA or not (wireless forwarding)*/
#if ZDCONF_LP_SUPPORT == 1
	U32					TotalLen;
//    U8	PackInfo[18];
    U8 Padding[3]; //Used as Padding Content for A-MSDU
	U8	HdrInfo[8][16];
	void	*LP_bucket;
#endif
...
struct zsRfPhy_StatSetting
{
    ...
    u32   Acc_SQ_OFDM;
    u32   Acc_SQ;
    u32   TimeElapsedInMT2;
    u32   MinSamplesInMT2;
    u32   CurrentAntenna;
    u8    EnableSwAntennaDiv;
    u8    AntennaScheme;
...
struct zd1205_private
{
    ...
    struct sk_buff *intraBuf;
    struct sk_buff *last_intraBuf;
    int intraBufCnt;
#if ZDCONF_DTIM_BC_FIX == 1
    struct sk_buff *fabBuf; //Frame After Beacon broadcast buff
    struct sk_buff *last_fabBuf; //Frame After Beacon broadcast buff
    int fabBufCnt;
#endif
#ifdef LINUX
    struct sk_buff *psBuf[MAX_AID]; //Frame After Beacon broadcast buff
    struct sk_buff *last_psBuf[MAX_AID]; //Frame After Beacon broadcast buff
    int psBufCnt[MAX_AID]; //Frame After Beacon broadcast buff
#endif
	spinlock_t 		intr_lock;
	spinlock_t		rx_pool_lock;
#ifdef LINUX
	u8          		tx_buff[MAX_WLAN_SIZE];
#endif
...
// debug counter
    ...
    U8 IBSS_DesiredMacMode;
    U8 IBSS_DesiredChannel;
    U32 lastRxComp;
    U8  wds;
    U32 rxurb_submit;
    U8 wps_filter;
...
#endif
#if ZDCONF_ANT_DIVERSITY == 1
    struct zsRfPhy_StatSetting RfStatSetting;
#endif
    BOOLEAN ACS;
#ifdef LINUX
    U8 lastBCN[MAX_BCN_LEN];
#endif
    U32 devBuf_alloc ;
    U32 devBuf_free ;
    U32 devBuf_upper ;
    ...
    U32 UWStrongSingalFlag;
    U32 FrameCountDuring3Sec; 
    U32 MinFrameCountDuring3Sec;
#endif
    U8 ContTxEver;
...
#define ZDAPIOCTLGET    (ZDAPIOCTL+0xB)
