...
static int read_pod(struct zd_chip *chip, u8 *rf_type)
{
	...
	*rf_type = value & 0x0f;
    ...
    pod_value_global = value;
    if ((*rf_type == AL2230_RF) && (value & BIT7))
        *rf_type = AL2230S_RF;
#endif
...
static int patch_cck_gain(struct zd_chip *chip)
{
	...
	ZD_ASSERT(mutex_is_locked(&chip->mutex));
#else
	MP_ASSERT(mutex_is_locked(&chip->mutex));
#endif
//	MP_TRACE_LINE();
