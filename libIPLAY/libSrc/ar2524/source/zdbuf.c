...
void initSignalBuf(void)
{
	...
	for (i=0; i<MAX_SIGNAL_NUM; i++){
		...
		SignalBuf[i] = (Signal_t *)mm_malloc(sizeof(Signal_t));  //can't use for DMA operation

		if (!SignalBuf[i]){
			mpDebugPrint("80211: initSignalBuf failed");
			//pdot11Obj->ExitCS(flags);
			return;
		}
		flags = pdot11Obj->EnterCS();
...
void initFdescBuf(void)
{
	...
	for (i=0; i<MAX_SIGNAL_NUM; i++){
		...
		if (!FdescBuf[i]){
			mpDebugPrint("80211: initFdescBuf failed");
			pdot11Obj->ExitCS(flags);
			return;
		}
...
FrmDesc_t *allocFdesc(void)
{
	...
	if (FreeFdescList != NULL){
        ...
        memset(pfrmDesc->CalMIC,0,sizeof(pfrmDesc->CalMIC));
		freeFdescCount--;
		allocCnt++;

		if (FreeFdescList == NULL){
			mpDebugPrint("FreeFdescList == NULL");
			mpDebugPrint("freeFdescCount", (U32)freeFdescCount);
			mpDebugPrint("Cnt of MgtQ", pMgtQ->cnt);
		}
	}
...
void freeFdesc(FrmDesc_t *pfrmDesc)
{
	...
	if (!pfrmDesc){
		mpDebugPrint("Free NULL pfrmDesc");
		return;
	}
