...
u8 WS11UPh[]
    ...
    #endif
    #if ZDCONF_ENABLE_USELESS_FUNCTION == 1
        u8 WS11UPhm[]
	    #include "WS11UPhm.h"
    #endif
#else
...
#ifdef LINUX
#define init_MUTEX(sem) DEFINE_MUTEX(sem)
#endif
...
#if 1 //LINUX_VERSION_CODE >= KERNEL_VERSION(2,5,0) /* tune me! */
...
#endif
#endif //ZDCONF_ENABLE_USELESS_FUNCTION
...
u32 zd1211_readl(u32 Address, u8 bAddUSBCSRAddress)
{
	...
	int r;
	if (bAddUSBCSRAddress && Address < 0x8000){
		Address += macp_global->USBCSRAddress;
    }
	mpDebugPrint("%s: offset=%x", __func__,Address);
    r = zd_ioread32(macp->chip, (zd_addr_t)Address, &val);
	MP_ASSERT(r == 0);
    return val;
#endif
}
...
int zd1211_USB_PACKAGE_WRITE_REGISTER(u16 *Address, u16 *Value, u16 RegCount, u8 bAddUSBCSRAddress)
{
	...
	if (in_atomic()){
#endif
		printk("********zd1211_USB_PACKAGE_WRITE_REGISTER in_interrupt*********\n");
		return 0;
	}
...
int zd1211_writel(u32 Address, u32 Value, u8 bAddUSBCSRAddress)
{
	...
	mpDebugPrint("%s: offset=%x,v=%x", __func__,Address, Value);
	r = zd_iowrite32(macp->chip, (zd_addr_t) Address, Value);
#endif
}
...
static void zd1211_tx_timeout(struct net_device *dev)
{
    ...
    printk("%s: Tx timed out. , Prepare to Reset\n", dev->name);
...
void zd1211_rx_isr(unsigned long parm)
{
	...
	if (last_pkt_len <= (macp->wMaxPacketSize - 4)){
		...
		for (i=0; i<total_rx_struct; i++){
			...
			if (i > 0){
					...
					mmcp_memcpy(&rfd->RxBuffer[macp->rxOffset],
					pRxBuffer + tmpLen,
					rfd->ActualCount);
			}        
	...
	if(zd1211_submit_rx_urb(macp))
	{
        ...
        set_bit(0, (void *)&macp->rxurb_submit);
	}
...

{
    ...
    static unsigned short xxxError = 0;
    ...
    if (urb->status != 0){
        ...
        else
        {
            ...
            if(xxxError++ > 100)
			{
                printk("Too many xxx error, break\n");
				return;
			}
            else
                zd1211_submit_rx_urb(macp);
...
#if ZDCONF_DEFER_RX != 1
    //zd1211_alloc_rx((unsigned long)macp); //It seems alloc skb in intr. will cause out of skb sometimes
#endif
...
{
#if ZDCONF_AP_SUPPORT == 1 || ZDCONF_STA_PSM == 1
	u32	intr_status; 
	...
	if (intr_status & CFG_NEXT_BCN){
        ...
        {
			if (macp->cardSetting.BssType == AP_BSS && (!UpdateBcn))
				goto done;

			if (macp->cardSetting.BssType == INDEPENDENT_BSS && (!UpdateBcn))
				goto done;

			macp->bcnCnt++;
			down(&macp->bcn_sem);
            //Will cause TKIP mic failure in AP mode
            //if(!test_and_set_bit(ZD1211_TX_BUSY, &macp->flags))
            {
                ...
                UpdateBcn--;
                if(UpdateBcn > 20)
                    printk("Update Bcn could overflow\n");
            ...
            mDtimCount = 0;
            if(!mDtimCount && (mBssType == AP_BSS))
            {
#if ZDCONF_DTIM_BC_FIX == 1
                defer_kevent(macp,KEVENT_DEV_QUEUE_PS);
#else
...
#if fMERGE_RX_FRAME
    if (rx_struct->UnFinishFrmLen){
    	rx_buff = (u8 *)(unsigned long )(rx_struct->dma_addr)+ macp->rxOffset + (rx_struct->UnFinishFrmLen);
    	bufLen -= (rx_struct->UnFinishFrmLen);
    }	
    else    
	    rx_buff = (u8 *)(unsigned long )(rx_struct->dma_addr) + macp->rxOffset;
...
struct zd1205_private *macp = urb->context;
    ...
    {
	    sw_tcb = macp->activeTxQ->first;
	    if (sw_tcb && (macp->tx_buffLen > 1300)) 
        {
            res = SUBMIT_URB(macp->tx_urb, GFP_ATOMIC);
            if(res)
            {
                printk("Submit failed in trDbg TX\n");
            }
            else
            {
...
#ifdef LINUX
#define     B_HIGH ((unsigned long)Tbd->TbdBufferAddrHighPart)
#define     B_LOW  ((unsigned long)Tbd->TbdBufferAddrLowPart)
int CheckMICAgain(u8 *buf,int len, u8 *CalMIC, u8 *micPos)
{
    extern Hash_t *sstByAid[MAX_RECORD];
    MICvar mic ;
    U8 MIC[8];
    int i;
    int bData = (buf[3] & ( BIT_2 | BIT_3)) == 0;
    int uniCast = !(buf[11+4] & 1);
    int bWep = buf[11+1] & WEP_BIT;
    memcpy(&mic,&sstByAid[1]->TxMicKey,sizeof(mic));

    if((!bWep) || (!uniCast) || (micPos == NULL)) return 0;

    MICclear(&mic);
    for(i=0;i<6;i++)
        MICappendByte(buf[11+4+i],&mic); //DA
    for(i=0;i<6;i++)
        MICappendByte(buf[11+4+6+6+i],&mic); //SA
    MICappendByte(0,&mic); //SA
    MICappendByte(0,&mic); //SA
    MICappendByte(0,&mic); //SA
    MICappendByte(0,&mic); //SA
    for(i=0;i<len-32-8-11;i++) //32=HDR+IV+EIV,8=MIC,4=ICV,4=FCS
         MICappendByte(buf[11+24+8+i],&mic);
    MICgetMIC(MIC, &mic);
    if(memcmp(MIC,micPos,8)!=0)
    {
        printk("new MIC=");
        for(i=0;i<8;i++)
            printk("%02x ", MIC[i] & 0xFF);
        printk("\n");
        printk("old MIC=");
        for(i=0;i<8;i++)
            printk("%02x ", micPos[i] & 0xFF);
        printk("\n");
        printk("old MIC=");
        for(i=0;i<8;i++)
            printk("%02x ", CalMIC[i] & 0xFF);
        printk("\n");
        return 1;
    }
...
int zd1211_submit_tx_urb(struct zd1205_private *macp,BOOLEAN LastFrag)
{
	...
	int res;
    U32 TxCbTbdNumber; 
    static unsigned long lastSetBusy = 0;
	//int memflags = GFP_KERNEL;
    ...
    if (test_and_set_bit(ZD1211_TX_BUSY, &macp->flags))
    {
        if(jiffies - lastSetBusy> 5*HZ)
        {
            ...
            printk("jiffies = %ld, lastSetBusy = %ld, HZ=%d\n", jiffies, lastSetBusy, HZ);
        }
        return 0;
    }
    lastSetBusy = jiffies;
...
#define     B2_HIGH ((unsigned long)(Tbd+1)->TbdBufferAddrHighPart)
#define     B2_LOW  ((unsigned long)(Tbd+1)->TbdBufferAddrLowPart)
#if ZDCONF_APC == 1
    if(macp->wds)
        if(memcmp(g_dev->dev_addr, (u8 *)(((B2_HIGH<<31)<<1)+B2_LOW)+4+6,6) != 0)
            if(*(u8 *)(((B2_HIGH<<31)<<1)+B2_LOW) == 0x08)
                WDS=1;
    if(WDS)
    {
        int Len = ((u8 *)pCtrlSet)[1]  + (((u8 *)pCtrlSet)[2] << 8) + 6;
        ((u8 *)pCtrlSet)[1]  = Len & 0xFF;
        ((u8 *)pCtrlSet)[2]  = (Len >> 8) & 0xFF;
    }
#endif
...
#define isWDS(pWlanHdr)                         (((pWlanHdr->FrameCtrl[1] & TO_DS_FROM_DS) == TO_DS_FROM_DS) ? 1 : 0)
    if(WDS)
    {
        U8 *p = (u8 *)(((B_HIGH<<31)<<1)+B_LOW);
        p[1] |= 3; //BIT_0_1
    }
#endif

	//Mac Header
    if(*((u8 *)(((B_HIGH<<31)<<1)+B_LOW)) != PS_POLL)
    {
        *(U8 *)(((B_HIGH<<31)<<1)+B_LOW+2) = 0;
        *(U8 *)(((B_HIGH<<31)<<1)+B_LOW+3) = 0;
    }
	else 
	{
...
*/
    ...
    if(WDS)
    {
        ...
        U8 *p;
        U32 addr0, addr1;
        addr0 = cpu_to_le32(*(u32 *)&macp->macAdr[0]);
        addr1 = cpu_to_le32(*(u32 *)&macp->macAdr[1]);
        //printk("addr0 = %08x\n", addr0);
        //printk("addr1 = %08x\n", addr1);

        p = dst;
        for(i=0;i<4 + 6 ;i++)       //FrameCtl+ AP+ SRC 
            *(dst++) = *((u8 *)(((B_HIGH<<31)<<1)+B_LOW)+i);
#if 0
        *(dst++) =  (U8)(addr0 & 0xFF);
        *(dst++) =  (U8)((addr0 >> 8)& 0xFF);
        *(dst++) =  (U8)((addr0 >> 16)& 0xFF);
        *(dst++) =  (U8)((addr0 >> 24)& 0xFF);
        *(dst++) =  (U8)((addr1 >> 0)& 0xFF);
        *(dst++) =  (U8)((addr1 >> 8)& 0xFF);
#endif
        *(dst++) =  g_dev->dev_addr[0];
        *(dst++) =  g_dev->dev_addr[1];
        *(dst++) =  g_dev->dev_addr[2];
        *(dst++) =  g_dev->dev_addr[3];
        *(dst++) =  g_dev->dev_addr[4];
        *(dst++) =  g_dev->dev_addr[5];

        for(i=0;i<6;i++)
            *(dst++) = *((u8 *)(((B_HIGH<<31)<<1)+B_LOW)+4+6+6+i);
        for(i=0;i<2;i++)
            *(dst++) = *((u8 *)(((B_HIGH<<31)<<1)+B_LOW)+4+6+6+6+i);
        for(i=0;i<6;i++)
            *(dst++) = *((u8 *)(((B_HIGH<<31)<<1)+B_LOW)+4+6+i);

        for(i=0;i<Tbd->TbdCount-24;i++)
            *(dst++) = *((u8 *)(((B_HIGH<<31)<<1)+B_LOW)+24+i);

        //for(i=0;i<Tbd->TbdCount+6;i++)
        //    printk("%02x ", (U8)p[i]);
        //printk("\n");
        TotalLength += Tbd->TbdCount + 6;
        //printk("%p, %p, %d\n", p,dst, Tbd->TbdCount);
    }
    else
...
#endif
    {
        mmcp_memcpy(dst, (u8 *)(((B_HIGH<<31)<<1)+B_LOW), Tbd->TbdCount);
        dst += Tbd->TbdCount;
    ...
    Tbd++;
#if ZDCONF_TX_NOCOPY == 1
    if(txRequest[sw_tcb->MsgID] != NULL)
    {
        skb = txRequest[sw_tcb->MsgID]->buf;
        if(skb != NULL)
        {
            if((skb_headroom(skb)) >= 64 && (skb_tailroom(skb) >= 8))
            {
                //printk("head=%d, tail=%d\n",skb_headroom(sw_tcb->skb), skb_tailroom(sw_tcb->skb));
                nonDataLen = dst-macp->tx_buff;
                //printk("nonDataLen = %d\n", nonDataLen);
                dst =(u8 *)(((B_HIGH<<31)<<1)+B_LOW);
				
                mmcp_memcpy(dst-nonDataLen,macp->tx_buff, nonDataLen);
                noCopyStart = dst-nonDataLen;
                
            }    
            else
                printk("hdr room=%d, tail room=%d\n",skb_headroom(skb),skb_tailroom(skb));

        }
    }
    else
        printk("What!? txRequest is NULL...\n");
#endif
    ...
    while(TxCbTbdNumber)
    {
        ...
        if(LastFrag == TRUE) {
            ...
            mmcp_memcpy(dst-sw_tcb->MIC_Len,sw_tcb->CalMIC+sw_tcb->MIC_Start,sw_tcb->MIC_Len);
        }
    }
...
{
    *(noCopyStart+4) =  (U8)((U16)TotalLength+14) ;
    *(noCopyStart+5) = (U8)(((U16)TotalLength+14) >> 8);
}
#endif
...
{
    *(noCopyStart+4) = (U8)(sw_tcb->LengthDiff);
    *(noCopyStart+5) = (U8)(sw_tcb->LengthDiff >> 8);
}
#endif
#endif
...
//if(CheckMICAgain(macp->tx_buff,TotalLength, sw_tcb->CalMIC, micPos))
//    printk("MIC Failure Packet Detected\n");
if(nonDataLen == 0)
{
#if ZDCONF_TR_DBG == 1
    macp->tx_buffLen = TotalLength;
#endif
...

{
    ...
    mmcp_memcpy(pBuffer, image, uploadLength);
		...
		u16 uCodeOfst, u8 bReboot)
{
	...
	mmcp_memcpy(image, pBuffer, uImgLength);
	...
	while (uImgLength > 0) {
        ...
        ZD1211DEBUG(0, "result = %d\n", result);
        if(result != translen)
        {
            printk(KERN_ERR "##### Warning! ####\n");
            printk(KERN_ERR "result(%d) is shorter than translen(%d)\n", result, translen); 
            printk(KERN_ERR "usb_control_msg doesn't send all data out\n");
            printk(KERN_ERR "You need to decrease the message amount in each send\n");
        }


		if (result < 0) {
			printk(KERN_ERR "zd1211: usb_control_msg 1 fail: %02d\n", result);
			//goto exit;
		}
    ...
    if (bReboot){                                                                               
		...
		if (result < 0) {
			...
			printk(KERN_ERR "zd1211: usb_control_msg 2 fail: %02d\n", result);
        ...
        ZD1211DEBUG(0, "FIRMWARE_CONFIRM = %d\n", ret);
...
{
    extern U8 mcBuffered;
    extern void zd1205_DeviceReset(struct zd1205_private *macp);
    int changed = 0,rc = 0;
	...
	//non tx rx interrupt
#if ZDCONF_DEFER_RX == 1
    if (test_and_clear_bit(KEVENT_DEFER_RX, &macp->kevent_flags))
    {
        zd1205_rx_isr(macp);
    }
#endif
...
#endif
    if (test_and_clear_bit(KEVENT_NON_TX_RX_INT, &macp->kevent_flags)){
        //printk("Get Non Tx Rx\n"); 
        //clear_bit(KEVENT_NON_TX_RX_INT, &macp->kevent_flags);
        zd1211_handle_non_tx_rx(macp);
        //printk("After Clean, macp->bHandleNonRxTxRunning=%d\n", macp->bHandleNonRxTxRunning);
    }
    ...
    if (test_bit(KEVENT_USB_DEVICE_RESET, &macp->kevent_flags)) {
        zd1205_DeviceReset(macp);
        g_dev->trans_start = jiffies;
        clear_bit(KEVENT_USB_DEVICE_RESET, &macp->kevent_flags);
    }
...
*/
	...
	if (test_and_clear_bit(KEVENT_DEV_QUEUE_XMIT, &macp->kevent_flags))
    {
        struct sk_buff *skb;
        unsigned long flag;

		//clear_bit(KEVENT_DEV_QUEUE_XMIT, &macp->kevent_flags);
        flag = dot11Obj.EnterCS();
        while(macp->intraBuf)
        {
            skb = macp->intraBuf;
            macp->intraBuf = macp->intraBuf->next;
            if(macp->intraBuf == NULL)
                macp->last_intraBuf = NULL;
            macp->intraBufCnt--;
            if(macp->intraBufCnt<0)
                printk("!! %s,%d\n",__func__,__LINE__);
            dot11Obj.ExitCS(flag);
            skb->next = NULL;
            skb->dev = g_dev;
            if((rc = dev_queue_xmit(skb)))
                printk("dev_queue_xmit failed at %s,%d,rc=%d\n",__func__,__LINE__,rc);
            else
                macp->devBuf_lower--;

            //if(zd1205_xmit_frame(skb, g_dev) != 0)
...
#if ZDCONF_DTIM_BC_FIX == 1
    if (test_and_clear_bit(KEVENT_DEV_QUEUE_PS, &macp->kevent_flags))
    {
        struct sk_buff *skb;
        unsigned long flag;
        ...
        {
            skb = macp->fabBuf;
            macp->fabBuf = macp->fabBuf->next;
            if(macp->fabBuf == NULL)
                macp->last_fabBuf = NULL;
            macp->fabBufCnt--;
            if(macp->fabBufCnt < 0)
                printk("!! %s,%d\n", __func__,__LINE__);
            dot11Obj.ExitCS(flag);
	...
	//scan timeout
	if (test_and_clear_bit(KEVENT_SCAN_TIMEOUT, &macp->kevent_flags)){
		//FPRINT("scan");       
	...
	if (test_bit(KEVENT_ZD_IOCTL, &macp->kevent_flags)){
		//FPRINT("ioctl"); 
		zd1205_zd_dbg_ioctl(macp, &macp->zdreq,&changed);
        if(changed == 1)
            printk("Should add update cardsetting\n");
