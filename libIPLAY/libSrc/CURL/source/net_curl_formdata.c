...
#include <libgen.h>
#endif
#include "net_curl_urldata.h" /* for struct SessionHandle */
//yuming, add
    ...
    if(size){
        if((type == FORM_DATA) || (type == FORM_CONTENT))
            *size += length;
            ...
            /* Since this is a file to be uploaded here, add the size of the actual file */
...
#endif
#else
                unsigned char converted_store_name[80];
                unsigned char FileHandle;
                unsigned int filesize = 0;

                memset(converted_store_name, 0, 80);
                UtilStringCopy0816(converted_store_name, newform->line);
                if (mpx_DirNodeLocate(converted_store_name) == 0)
                {
                    CURLPRINTF("%s found, Open it\n", newform->line);
                    FileHandle = mpx_FileOpen();
                    if( FileHandle <= 0 )
                        return -1;
                }
                else{
                    CURLPRINTF("%s not found, return\n", newform->line);
                    return 0;
                 }  
                mpx_FileSizeGet(FileHandle, &filesize);
                CURLPRINTF("FileSize = %d\n", *size);
                *size += filesize;
                CURLPRINTF("FileSize = %d\n", *size);
...
//yuming, change driver
    ...
    CURLPRINTF(" form->data->line = %s\n",  form->data->line);
    UtilStringCopy0816(converted_store_name, form->data->line);

    if (mpx_DirNodeLocate(converted_store_name) == 0)
    {
        CURLPRINTF("13.jpg found, Open it\n");
        form->fp = mpx_FileOpen();
    }
    else{
...
#else
  nread = Curl_fread(buffer, 1, size, form->fp);
#endif
...
//yuming, change driver
    ...
    fclose(form->fp);
#else
    CURLPRINTF("mpx_FileClose\n");
    mpx_FileClose(form->fp);
#endif
    form->fp = NULL;
                       ...
                       void *mydata)
{
    ...
    if(!form->data){
        CURLPRINTF("no form->data\n");
        return 0; /* nothing, error, empty */
    }

    CURLPRINTF("form address =  %x\n", form);
    CURLPRINTF("form->data address =  %x\n", form->data);
  if(form->data->type == FORM_FILE) {
    gotsize = readfromfile(form, buffer, wantedsize);
    CURLPRINTF("gotsize = %d\n", gotsize);
  ...
  do {
    ...
    CURLPRINTF(" readfromfile form->data address =  %x\n", form->data);
...
char *Curl_formpostheader(void *formp, size_t *len)
{
    ...
    CURLPRINTF("Curl_formpostheader\n");
...
//yuming, no rand function
  ...
  MP_DEBUG("retstring = %s", retstring);
