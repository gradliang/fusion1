...
#include "net_curl_easyif.h" /* for Curl_convert_... prototypes */
#include "net_curl_formdata.h"
#include "net_curl_progress.h"
#include "net_curl_base64.h"
#include "net_curl_cookie.h"
...
#include "net_curl_parsedate.h" /* for the week day and month names */
  ...
  if((expectsend == -1) || (expectsend > bytessent)) {
    ...
    if((data->state.authproxy.picked == CURLAUTH_NTLM) ||
      ...
      if(((expectsend - bytessent) < 2000) ||
        ...
        if(!conn->bits.authneg){
            CURLPRINTF("ewind data when completely done sending!");
                                           ...
                                           up the proxy tunnel */
{
    ...
    CURLPRINTF("Curl_http_output_auth");
...

{
  ...
  if(size - included_body_bytes > 0) {
    ...
    CURLPRINTF("sendsize %d", sendsize);
    ...
    *bytes_written += amount;
    ...
    CURLPRINTF("amount %d", amount);
    CURLPRINTF("*bytes_written %d", *bytes_written);
...
#define SELECT_TIMEOUT 2
  ...
  do {
    ...
    { /* BEGIN NEGOTIATION PHASE */
      ...
      while((nread<BUFSIZE) && (keepon && !error)) {
        ...
        case -1: /* select() error, stop reading */
          error = SELECT_ERROR;
          CURLPRINTF("Proxy CONNECT aborted due to select/poll error");
        ...
        default:
          ...
          if(res< 0)
            ...
            keepon = FALSE;
            error = SELECT_ERROR;
            CURLPRINTF("Proxy CONNECT aborted");
    ...
    if(200 != k->httpcode) {
        CURLPRINTF("Received HTTP code %d from proxy after CONNECT", k->httpcode);
        failf(data, "Received HTTP code %d from proxy after CONNECT", k->httpcode);
    ...
    CURLPRINTF("Proxy replied OK to CONNECT request\n");
...
CURLcode Curl_http(struct connectdata *conn, bool *done)
{
    ...
    *done = TRUE;
    ...
    if(!data->reqdata.proto.http) {
        ...
        CURLPRINTF("first time to allocate data->reqdata.proto.http");
    ...
    if(data->set.customrequest)
        ...
        if(conn->bits.no_body)
            ...
            switch(httpreq) {
                case HTTPREQ_POST:
                case HTTPREQ_POST_FORM:
                    CURLPRINTF("request: POST httpreq = %d", httpreq);
                    request = (char *)"POST";
                break;
                case HTTPREQ_PUT:
                    request = (char *)"PUT";
...
#if 0
// add by cj 0611 for GCE
    if(checkheaders(data, "x-GCE-Screen-Size:") && conn->allocptr.x_gce_screen_size) {
        free(conn->allocptr.x_gce_screen_size);
        conn->allocptr.x_gce_screen_size=NULL;
    }
    if(checkheaders(data, "x-GCE-Buttons:") && conn->allocptr.x_gce_buttons) {
        free(conn->allocptr.x_gce_buttons);
        conn->allocptr.x_gce_buttons=NULL;
    }
    if(checkheaders(data, "x-GCE-Device-ID:") && conn->allocptr.x_gce_device_id) {
        free(conn->allocptr.x_gce_device_id);
        conn->allocptr.x_gce_device_id=NULL;
    }
    if(checkheaders(data, "x-GCE-Device-Nickname:") && conn->allocptr.x_gce_device_nickname) {
        free(conn->allocptr.x_gce_device_nickname);
        conn->allocptr.x_gce_device_nickname=NULL;
    }	
//end by cj
#endif
  ...
  if(ptr && (!data->state.this_is_a_follow ||
    ...
    if (conn->bits.httpproxy && !conn->bits.tunnel_proxy){
        CURLPRINTF("Using a proxy but does not tunnel through it ");    
    ...
    /* we must build the whole darned post sequence first, so that we have
     ...
     if(CURLE_OK != result) {
       ...
       CURLPRINTF("failed creating formpost data result = %d\n", result);
  ...
  if(!checkheaders(data, "Accept:"))
	...
	if(!checkheaders(data, "Accept:"))
	{
		...
		//if(g_bXpgStatus == XPG_MODE_NET_GCE)
		{
			http->p_accept = "Accept: image/jpeg\r\n"; //cj: request jpeg file only from GCE
		}
		//else
		{
    			http->p_accept = "Accept: */*\r\n";
...
//add by cj for GCE 0610
		  (data->set.x_gce_screen_size && *data->set.x_gce_screen_size && conn->allocptr.x_gce_screen_size)?conn->allocptr.x_gce_screen_size:"",	
		  (data->set.x_gce_buttons && *data->set.x_gce_buttons && conn->allocptr.x_gce_buttons)?conn->allocptr.x_gce_buttons:"",	
		  (data->set.x_gce_device_id && *data->set.x_gce_device_id && conn->allocptr.x_gce_device_id)?conn->allocptr.x_gce_device_id:"",	
		  (data->set.x_gce_device_nickname && *data->set.x_gce_device_nickname && conn->allocptr.x_gce_device_nickname)?conn->allocptr.x_gce_device_nickname:"",	
//end by cj
    ...
    case HTTPREQ_POST_FORM:
      ...
      /* set upload size to the progress meter */
       CURLPRINTF("set upload size to the progress meter = %d \n", http->postsize);
