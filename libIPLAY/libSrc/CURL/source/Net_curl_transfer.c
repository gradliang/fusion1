...
#include "net_curl_content_encoding.h"
...
#include "net_curl_http.h"
#include "net_curl_url.h"
#include "net_curl_getinfo.h"
#include "net_curl_sslgen.h"
#include "net_curl_http_digest.h"
#include "net_curl_http_ntlm.h"
//yuming, add
...
#include "net_curl_easyif.h" /* for Curl_convert_to_network prototype */
...
CURLcode Curl_fillreadbuffer(struct connectdata *conn, int bytes, int *nreadp)
{
    ...
    CURLPRINTF("Curl_fillreadbuffer\n");
    ...
    CURLPRINTF("conn->fread_in %x\n", conn->fread_in);
    
    nread = (int)conn->fread(data->reqdata.upload_fromhere, 1,
                           buffersize, conn->fread_in);

    CURLPRINTF("buffersize = %d   nread = %d\n", buffersize, nread);
                        ...
                        bool *done)
{
    ...
    if(select_res == CSELECT_ERR) {
        CURLPRINTF("select/poll returned error");
    ...
    do{
        ...
        if((k->keepon & KEEP_READ) &&
            ...
            do {
                size_t buffersize = data->set.buffer_size?data->set.buffer_size : BUFSIZE;
                ...
                CURLPRINTF("Read %d bytes from stream (readrc = %d)\n", nread, readrc);
                ...
                if(0 > readrc)
					...
					{
	                    k->start = Curl_tvnow();
						//mpDebugPrint("%ld %ld",k->start.tv_sec,k->start.tv_usec);
					}
#endif				
                ...
                if (0 < nread || is_empty_data){
                    ...
                    CURLPRINTF("nread <= 0, server closed connection, bailing\n");
                ...
                if (k->header) {
                    ...
                    do{
                        ...
                        if (!k->end_ptr){
                            CURLPRINTF("Not a complete header line\n");
                            ...
                            if (k->hbuflen + nread >= data->state.headersize){
                                ...
                                char *newbuff;
                                size_t newsize=CURLMAX((k->hbuflen+nread)*3/2, data->state.headersize*2);
                                hbufp_index = k->hbufp - data->state.headerbuff;
                                newbuff = (char *)realloc(data->state.headerbuff, newsize);
                                if(!newbuff) {
                                    CURLPRINTF("Failed to alloc memory for big header!");
                         ...
                         * it.
                         */
            if (k->hbuflen + full_length >= data->state.headersize) {
              char *newbuff;
              CURLPRINTF("k->hbuflen + full_length >= data->state.headersize!");
            ...
            *k->hbufp = 0;
            ...
            if(!k->headerline) {
              ...
              if((k->hbuflen>5) &&
                ...
                /* this is not the beginning of a HTTP first header line */
                CURLPRINTF("this is not the beginning of a HTTP first header line");
                k->header = FALSE;
                if(nread)
            ...
            if ((0x0a == *k->p) || (0x0d == *k->p)) {
              ...
              if(100 == k->httpcode) {
                ...
                if (k->write_after_100_header) {
                    CURLPRINTF("k->write_after_100_header = TRUE");
              ...
              if(!stop_reading) {
                ...
                if(conn->bits.rewindaftersend) {
                  ...
                  CURLPRINTF("Keep sending data to get tossed away!\n");
                  k->keepon |= KEEP_WRITE;
                }
              }
#endif   /* CURL_DISABLE_HTTP */
...
#define HEADER1 k->p /* no conversion needed, just use k->p */
            ...
            /* check for Content-Type: header lines to get the mime-type */
              ...
              if(end) {
                ...
                data->info.contenttype = (char *)malloc(len + 1);
...
#if 0
// add by cj for GCE 0610			
	     else if(checkprefix("Refresh:", k->p))
	     {
	      		MP_DEBUG1("111 >>>>>>>> GCE Refresh:%s",k->p);
			char *start;
              	char *end;
              	size_t len;

              	/* Find the first non-space letter */
             		 for(start=k->p+8;*start && ISSPACE(*start);start++)
                		;  /* empty loop */
			...
			MP_DEBUG1(".......................len = %d",len);
			/* allocate memory of a cloned copy */
                	Curl_safefree(data->info.Refresh);

                	data->info.Refresh = (char *)malloc(len);
                	if (NULL == data->info.Refresh)
                  		return CURLE_OUT_OF_MEMORY;

                	/* copy the content-type string */
                	memcpy(data->info.Refresh, start, len);
                	data->info.Refresh[len] = 0; /* zero terminate */	
			//gce_header_refresh_time(data->info.Refresh);		
              	}
			
			MP_DEBUG1("222 >>>>>>>> GCE Refresh:%s",data->info.Refresh);	
		
			end = strstr(start, "URL=");
			end = end+4;
			start = end;
			...
			/* allocate memory of a cloned copy */
                	Curl_safefree(data->info.URL);

                	data->info.URL = (char *)malloc(len);
                	if (NULL == data->info.URL)
                  		return CURLE_OUT_OF_MEMORY;

                	/* copy the content-type string */
                	memcpy(data->info.URL, start, len);
                	data->info.URL[len] = 0; /* zero terminate */	
			//gce_header_refresh_URL(data->info.URL);		
              	}		
			MP_DEBUG1("333 >>>>>>>> GCE URL:%s",data->info.URL);	
			
	     }	
	     else if(checkprefix("X-GCE-Button-OK:", k->p))
	     {
	      		MP_DEBUG1("111 >>>>>>>> GCE X-GCE-Button-OK: %s",k->p);
			char *start;
              	char *end;
              	size_t len;

              	/* Find the first non-space letter */
             		 for(start=k->p+16;*start && ISSPACE(*start);start++)
                		;  /* empty loop */
			...
			/* allocate memory of a cloned copy */
                	Curl_safefree(data->info.X_GCE_Button_OK);

                	data->info.X_GCE_Button_OK = (char *)malloc(len + 1);
                	if (NULL == data->info.X_GCE_Button_OK)
                  		return CURLE_OUT_OF_MEMORY;

                	/* copy the content-type string */
                	memcpy(data->info.X_GCE_Button_OK, start, len);
                	data->info.X_GCE_Button_OK[len] = 0; /* zero terminate */	
			//gce_header_center_button(data->info.X_GCE_Button_OK);			
              	}		
			MP_DEBUG1("222 >>>>>>>> GCE X-GCE-Button-OK:%s",data->info.X_GCE_Button_OK);	

	     }
	     else if(checkprefix("X-GCE-Button-Esc:", k->p))
	     {
	      		MP_DEBUG1("111 >>>>>>>> GCE X-GCE-Button-Esc: %s",k->p);
			char *start;
              	char *end;
              	size_t len;

              	/* Find the first non-space letter */
             		 for(start=k->p+17;*start && ISSPACE(*start);start++)
                		;  /* empty loop */
			...
			/* allocate memory of a cloned copy */
                	Curl_safefree(data->info.X_GCE_Button_Esc);

                	data->info.X_GCE_Button_Esc = (char *)malloc(len + 1);
                	if (NULL == data->info.X_GCE_Button_Esc)
                  		return CURLE_OUT_OF_MEMORY;

                	/* copy the content-type string */
                	memcpy(data->info.X_GCE_Button_Esc, start, len);			
                	data->info.X_GCE_Button_Esc[len] = 0; /* zero terminate */	
			start = 	data->info.X_GCE_Button_Esc;	
			//gce_header_esc_button(start); 
              	}		
			MP_DEBUG1("222 >>>>>>>> GCE X-GCE-Button-Esc:%s",data->info.X_GCE_Button_Esc);	

	     }
	     else if(checkprefix("X-GCE-Button-Up:", k->p))
	     {
	      		MP_DEBUG1("111 >>>>>>>> GCE X-GCE-Button-Up: %s",k->p);
			char *start;
              	char *end;
              	size_t len;
			...
			/* allocate memory of a cloned copy */
                	Curl_safefree(data->info.X_GCE_Button_Up);

                	data->info.X_GCE_Button_Up = (char *)malloc(len + 1);
                	if (NULL == data->info.X_GCE_Button_Up)
                  		return CURLE_OUT_OF_MEMORY;

                	/* copy the content-type string */
                	memcpy(data->info.X_GCE_Button_Up, start, len);
                	data->info.X_GCE_Button_Up[len] = 0; /* zero terminate */	
			//gce_header_up_button(data->info.X_GCE_Button_Up);		
              	}		
			MP_DEBUG1("222 >>>>>>>> GCE X-GCE-Button-Up:%s",data->info.X_GCE_Button_Up);	

	     }
	     else if(checkprefix("X-GCE-Button-Down:", k->p))
	     {
	      		MP_DEBUG1("111 >>>>>>>> GCE X-GCE-Button-Down: %s",k->p);
			char *start;
              	char *end;
              	size_t len;

              	/* Find the first non-space letter */
             		 for(start=k->p+18;*start && ISSPACE(*start);start++)
                		;  /* empty loop */
			...
			/* allocate memory of a cloned copy */
                	Curl_safefree(data->info.X_GCE_Button_Down);

                	data->info.X_GCE_Button_Down = (char *)malloc(len + 1);
                	if (NULL == data->info.X_GCE_Button_Down)
                  		return CURLE_OUT_OF_MEMORY;

                	/* copy the content-type string */
                	memcpy(data->info.X_GCE_Button_Down, start, len);
                	data->info.X_GCE_Button_Down[len] = 0; /* zero terminate */	
			//gce_header_down_button(data->info.X_GCE_Button_Down);			
              	}		
			MP_DEBUG1("222 >>>>>>>> GCE X-GCE-Button-Down:%s",data->info.X_GCE_Button_Down);

	     }
	     else if(checkprefix("X-GCE-Button-Left:", k->p))
	     {
	      		MP_DEBUG1("111 >>>>>>>> GCE X-GCE-Button-Left: %s",k->p);
			char *start;
              	char *end;
              	size_t len;
			...
			/* allocate memory of a cloned copy */
                	Curl_safefree(data->info.X_GCE_Button_Left);

                	data->info.X_GCE_Button_Left = (char *)malloc(len + 1);
                	if (NULL == data->info.X_GCE_Button_Left)
                  		return CURLE_OUT_OF_MEMORY;

                	/* copy the content-type string */
                	memcpy(data->info.X_GCE_Button_Left, start, len);
                	data->info.X_GCE_Button_Left[len] = 0; /* zero terminate */	
			//gce_header_left_button(data->info.X_GCE_Button_Left);		
              	}		
			MP_DEBUG1("222 >>>>>>>> GCE X-GCE-Button-Left:%s",data->info.X_GCE_Button_Left);

	     }
	     else if(checkprefix("X-GCE-Button-Right:", k->p))
	     {
	      		MP_DEBUG1("111 >>>>>>>> GCE X-GCE-Button-Right: %s",k->p);
			char *start;
              	char *end;
              	size_t len;

              	/* Find the first non-space letter */
             		 for(start=k->p+19;*start && ISSPACE(*start);start++)
                		;  /* empty loop */
			...
			/* allocate memory of a cloned copy */
                	Curl_safefree(data->info.X_GCE_Button_Right);

                	data->info.X_GCE_Button_Right = (char *)malloc(len + 1);
                	if (NULL == data->info.X_GCE_Button_Right)
                  		return CURLE_OUT_OF_MEMORY;

                	/* copy the content-type string */
                	memcpy(data->info.X_GCE_Button_Right, start, len);
                	data->info.X_GCE_Button_Right[len] = 0; /* zero terminate */	
			//gce_header_right_button(data->info.X_GCE_Button_Right);		
              	}		
			MP_DEBUG1("222 >>>>>>>> GCE X-GCE-Button-Right:%s",data->info.X_GCE_Button_Right);


	     }
#endif	//#if 0 //Mark GCE code
#ifndef CURL_DISABLE_HTTP
              ...
              if(data->set.http_follow_location) {
                ...
                if(ptr != start) {
                  *ptr = '\0';   /* zero terminate */
                  data->reqdata.newurl = (char *)strdup(start); /* clone string */
    ...
    if((k->keepon & KEEP_WRITE) && (select_res & CSELECT_OUT)) {
      ...
      do {
        ...
        CURLPRINTF("write to socket (send away data) \n");
  ...
  //when dongle is 3g, please don't do the NetConnected decision
  if(!NetInterfaceIpGet("ppp0"))	//Dongle isn't 3g.
  {
	if( (NetConnected() == FALSE) )
	{
		mpDebugPrint("NetConnected is FALSE");
  ...
  if (data->set.timeout &&
    ...
    *done = (bool)(0 == (k->keepon&(KEEP_READ|KEEP_WRITE)));
    ...
    CURLPRINTF("Curl_readwrite CURLE_OK k->keepon %d", k->keepon);
    return CURLE_OK;
}
...
CURLcode Curl_readwrite_init(struct connectdata *conn)
{
    ...
    CURLPRINTF("Curl_readwrite_init");
    ...
    if(conn->bits.getheader || !conn->bits.no_body){
        if(conn->sockfd != CURL_SOCKET_BAD) {
            k->keepon |= KEEP_READ;
            CURLPRINTF("1. k->keepon |= KEEP_READ");
        }
        ...
        if(conn->writesockfd != CURL_SOCKET_BAD){
            ...
            */
            if (data->state.expect100header && (data->reqdata.proto.http->sending == HTTPSEND_BODY)){
                ...
                if(data->state.expect100header)
                    ...
                    k->keepon |= KEEP_WRITE;
                    CURLPRINTF("1. k->keepon |= KEEP_WRITE");
            }
        }
    }
...
CURLcode Curl_pretransfer(struct SessionHandle *data)
{
    ...
    CURLPRINTF("Curl_pretransfer");
    
    if(!data->change.url){
        /* we can't do anything wihout URL */
        DPrintf("we can't do anything wihout URL");
                                    ...
                                    opposed to a real redirect following */
{
  ...
  if(!retry) {
    ...
    if(data->set.http_auto_referer) {
        ...
        data->change.referer = (char *)strdup(data->change.url);
  ...
  if(2 != sscanf(newurl, "%15[^?&/:]://%c", prot, &letter)) {
    ...
    */
    ...
    char *url_clone=(char *)strdup(data->change.url);
  ...
  newurl = NULL; /* don't free! */
	...
	//yuming, too long url with printf bug
    ...
    /* If a callback (or something) has altered the URL we should use within
       ...
       to the new URL */
        ...
        if ((CURLE_OK == res) && urlchanged) {
            ...
            if(CURLE_OK == res) {
                char *gotourl = (char *)strdup(data->change.url);
                        ...
                        char **url)
{
    ...
    /* We got no data, we attempted to re-use a connection and yet we want a
       ...
       CURLPRINTF("Connection died, retrying a fresh connect\n");
        infof(conn->data, "Connection died, retrying a fresh connect\n");
        *url = (char *)strdup(conn->data->change.url);
            ...
            if(res == CURLE_OK){
                ...
                if(res == CURLE_OK) {
                    ...
                    if(!retry)
                         ...
                         */
                        newurl = data->reqdata.newurl?(char *)strdup(data->reqdata.newurl):NULL;
                }
                else{
                    ...
                    * possibly know if the connection is in a good shape or not now. */
                    ...
                    if(CURL_SOCKET_BAD != conn->sock[SECONDARYSOCKET]) {
                        ...
                        it was used */
						closesocket(conn->sock[SECONDARYSOCKET]);
