...
#else
  return inet_ntoa(((const struct sockaddr_in*)ip->ai_addr)->sin_addr);
#endif
}
                ...
                int port)
{
    ...
    CURLPRINTF("entry_len = %d", entry_len);
    CURLPRINTF("entry_id = %s", entry_id);
                ...
                struct Curl_dns_entry **entry)
{
  ...
  if(!data->set.no_signal) {
    ...
    if(!dns){
        CURLPRINTF("The entry was not in the cache. Resolve it to IP address'");
