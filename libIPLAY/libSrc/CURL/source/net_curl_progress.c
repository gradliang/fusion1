...
{
    CURLPRINTF("Curl_pgrsResetTimes");
...
void Curl_pgrsTime(struct SessionHandle *data, timerid timer)
{
    ...
    switch(timer) {
        ...
        case TIMER_STARTSINGLE:
            CURLPRINTF("TIMER_STARTSINGLE");
            /* This is set at the start of a single fetch */
            data->progress.t_startsingle = Curl_tvnow();
        break;
        case TIMER_NAMELOOKUP:
            CURLPRINTF("TIMER_NAMELOOKUP");
            data->progress.t_nslookup = Curl_tvdiff_secs(Curl_tvnow(), data->progress.t_startsingle);
			CURLPRINTF("TIMER_NAMELOOKUPed");
        break;
        case TIMER_CONNECT:
            CURLPRINTF("TIMER_CONNECT");
            data->progress.t_connect = Curl_tvdiff_secs(Curl_tvnow(), data->progress.t_startsingle);
        break;
        case TIMER_PRETRANSFER:
            CURLPRINTF("TIMER_PRETRANSFER");
            data->progress.t_pretransfer = Curl_tvdiff_secs(Curl_tvnow(), data->progress.t_startsingle);
        break;
        case TIMER_STARTTRANSFER:
            CURLPRINTF("TIMER_STARTTRANSFER");
            data->progress.t_starttransfer = Curl_tvdiff_secs(Curl_tvnow(), data->progress.t_startsingle);
        break;
        case TIMER_POSTRANSFER:
            CURLPRINTF("TIMER_POSTRANSFER");
            /* this is the normal end-of-transfer thing */
        break;
        case TIMER_REDIRECT:
            CURLPRINTF("TIMER_REDIRECT");
...
{
    CURLPRINTF("Curl_pgrsStartNow : to get start time");
...
{
    CURLPRINTF("Curl_pgrsSetDownloadCounter = %d", size);
    data->progress.downloaded = size;
}

void Curl_pgrsSetUploadCounter(struct SessionHandle *data, curl_off_t size)
{
    CURLPRINTF("Curl_pgrsSetUploadCounter = %d", size);
    data->progress.uploaded = size;
}

void Curl_pgrsSetDownloadSize(struct SessionHandle *data, curl_off_t size)
{
    CURLPRINTF("Curl_pgrsSetDownloadSize = %d", size);
...
{
    CURLPRINTF("Curl_pgrsSetUploadSize = %d", size);
...
int Curl_pgrsUpdate(struct connectdata *conn)
{
    ...
    CURLPRINTF("Curl_pgrsUpdate");
#if 0
  now = Curl_tvnow(); /* what time is it */

  /* The time spent so far (from the start) */
  //no double, so use long and omit usec
  data->progress.timespent = (now.tv_sec - data->progress.start.tv_sec);
...
#else
      result= data->set.fprogress(data->set.progress_client,
                                  (long)data->progress.size_dl,
                                  (long)data->progress.downloaded,
                                  (long)data->progress.size_ul,
                                  (long)data->progress.uploaded);
#endif
      if(result)
