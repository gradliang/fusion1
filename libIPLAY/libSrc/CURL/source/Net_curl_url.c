...
//yuming, add temporary
void
CURLDUMP(char *prompt, unsigned char * buf, int len)
{
    int i = 1;
    DPrintf("%s: ", prompt);
    DPrintf("0x%4x: \\-", i-1);
    for (i = 1; i <= len; i++) {
        DPrintf("%2x \\-", (unsigned char) * buf);
        if((i % 16) == 0){
            DPrintf("");
            if((i+1) <= len)
                DPrintf("0x%4x: \\-", i);
...
CURLcode Curl_open(struct SessionHandle **curl)
{
    ...
    if(!data) {
        ...
        CURLPRINTF("Error: calloc of SessionHandle failed\n");
        return CURLE_OUT_OF_MEMORY;
    }
  ...
  /* make sure that all other returns from this function should destroy the
    ...
    if(!data->state.headerbuff){
        ...
        CURLPRINTF("Error: malloc of headerbuff failed\n");
        res = CURLE_OUT_OF_MEMORY;
    }
    else{
        data->state.headersize = HEADERSIZE;

    //CURLDUMP("data->state.headerbuff = ", data->state.headerbuff, HEADERSIZE);
//yuming, add
#if 0
    ...
    data->set.fwrite = (curl_write_callback)Curl_fwrite;

    /* use fread as default function to read input */
    data->set.fread = (curl_read_callback)Curl_fread;
                     ...
                     va_list param)
{
  ...
  case CURLOPT_FILE:
     ...
     */
//yuming, change driver
#if 0
    data->set.out = va_arg(param, FILE *);
#else
    data->set.out = va_arg(param, size_t);
    CURLPRINTF("data->set.out %d\n", data->set.out);
#endif
    break;
...
CURLcode Curl_disconnect(struct connectdata *conn)
{
  ...
  Curl_hash_apply(data->hostcache,
    ...
    */
    ...
    if((conn->ntlm.state != NTLMSTATE_NONE) ||(conn->proxyntlm.state != NTLMSTATE_NONE)){
                 ...
                 struct connectdata **usethis)
{
    ...
    if(match) {
      if (!IsPipeliningEnabled(data)) {
        ...
        if(dead) {
          check->data = data;
          CURLPRINTF("Connection #%d seems to be dead!\n", i);
                               ...
                               bool *protocol_done)
{
    ...
    CURLPRINTF("Curl_protocol_connect");
                                          ...
                                          struct connectdata *conn)
{
    ...
    CURLPRINTF("ParseURLAndFillConnection");
  ...
  if(path[0] == '?') {
    ...
    CURLPRINTF("conn->protostr %s", conn->protostr);
    CURLPRINTF("conn->host.name %s", conn->host.name);
	//not print so long , 
    //CURLPRINTF("data->reqdata.path %s", data->reqdata.path);
  /*
                                 ...
                                 bool *async)
{
  ...
  void (*keep_sigact)(int);       /* store the old handler here */
    ...
    *in_connect = conn;
    ...
    conn->bits.httpproxy = (bool)(conn->bits.proxy && (conn->proxytype == CURLPROXY_HTTP));
    ...
    */
    ...
    if (result != CURLE_OK) {
        CURLPRINTF("ParseURLAndFillConnection Fail");
        return result;
    }
	CURLPRINTF("ParseURLAndFillConnection Success");
...
//yuming, add
  ...
  if(reuse) {
    ...
    *in_connect = conn;      /* return this instead! */
    ...
    CURLPRINTF("Re-using existing connection! (%d) with host %s\n", conn->connectindex,
            ...
            * there, thus overriding any defaults that might have been set above. */
            ...
            if(rc == CURLRESOLV_PENDING)
                *async = TRUE;
                ...
                CURLPRINTF("Couldn't resolve host '%s'", conn->host.dispname);
            ...
            if(rc == CURLRESOLV_PENDING)
                *async = TRUE;
                ...
                CURLPRINTF("Couldn't resolve proxy '%s'", conn->proxy.dispname);
                      ...
                      bool *protocol_done)
{
    ...
    *asyncp = FALSE; /* assume synchronous resolves by default */
    ...
    if(CURLE_OK == code){
        CURLPRINTF("Create connection no error");
        /* no error */
        if(dns || !*asyncp)
    ...
    if(CURLE_OK != code) {
		CURLPRINTF("Create connection error");
                   ...
                   bool premature)
{
    ...
    if(Curl_removeHandleFromPipeline(data, conn->recv_pipe) && conn->readchannel_inuse)
        conn->readchannel_inuse = FALSE;
    if(Curl_removeHandleFromPipeline(data, conn->send_pipe) && conn->writechannel_inuse)
        conn->writechannel_inuse = FALSE;
...
CURLcode Curl_do(struct connectdata **connp, bool *done)
{
    ...
    if(conn->curl_do){
        ...
        if((CURLE_SEND_ERROR == result) && conn->bits.reuse){
            CURLPRINTF("Re-used connection seems dead, get a new one");
