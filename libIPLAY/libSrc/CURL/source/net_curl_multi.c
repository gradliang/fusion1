                                ...
                                CURL *easy_handle)
{
    ...
    CURLPRINTF("curl_multi_add_handle\n");
    ...
    if((multi->num_easy * 4) > multi->connc->num) {
        ...
        by default. */
        CURLcode res = Curl_ch_connc(easy_handle, multi->connc, multi->connc->num*4);
        if(res != CURLE_OK)
                                   ...
                                   CURL *curl_handle)
{
  ...
  struct Curl_one_easy *easy;
    ...
    CURLPRINTF("curl_multi_remove_handle\n");
                                 ...
                                 struct Curl_one_easy *easy)
{
    ...
    case CURLM_STATE_INIT:
        CURLPRINTF("case CURLM_STATE_INIT");
      /* init this transfer. */
    ...
    case CURLM_STATE_CONNECT:
        CURLPRINTF("case CURLM_STATE_CONNECT");
    ...
    case CURLM_STATE_WAITCONNECT:
        CURLPRINTF("case CURLM_STATE_WAITCONNECT");
    ...
    case CURLM_STATE_PROTOCONNECT:
        CURLPRINTF("case CURLM_STATE_PROTOCONNECT");
    ...
    case CURLM_STATE_WAITDO:
        CURLPRINTF("case CURLM_STATE_WAITDO");
    ...
    case CURLM_STATE_DO:
        CURLPRINTF("case CURLM_STATE_DO");
    ...
    case CURLM_STATE_DOING:
        CURLPRINTF("case CURLM_STATE_DOING");
    ...
    case CURLM_STATE_DO_MORE:
        CURLPRINTF("case CURLM_STATE_DO_MORE");
      /* Ready to do more? */
        easy->result = Curl_is_connected(easy->easy_conn,
                                       SECONDARYSOCKET,
                                       &connected);
    ...
    case CURLM_STATE_DO_DONE:
        CURLPRINTF("case CURLM_STATE_DO_DONE");
      /* Remove ourselves from the send pipeline */
      Curl_removeHandleFromPipeline(easy->easy_handle,
                                    easy->easy_conn->send_pipe);
    ...
    case CURLM_STATE_WAITPERFORM:
        CURLPRINTF("case CURLM_STATE_WAITPERFORM");
#ifdef CURLDEBUG
    ...
    case CURLM_STATE_PERFORM:
        CURLPRINTF("case CURLM_STATE_PERFORM");
        //BREAK_POINT();
      /* check if over speed */
      if ( (  ( easy->easy_handle->set.max_send_speed > 0 ) &&
              ( easy->easy_handle->progress.ulspeed >
      ...
      if (!(k->keepon & KEEP_WRITE)) {
        ...
        CURLPRINTF("easy->result = %d", easy->result);
      if(easy->result)  {
        ...
        if(CURL_SOCKET_BAD != easy->easy_conn->sock[SECONDARYSOCKET]) {
          /* if we failed anywhere, we must clean up the secondary socket if
             it was used */
          closesocket(easy->easy_conn->sock[SECONDARYSOCKET]);
    ...
    case CURLM_STATE_DONE:
        CURLPRINTF("case CURLM_STATE_DONE");
    ...
    case CURLM_STATE_COMPLETED:
        CURLPRINTF("case CURLM_STATE_COMPLETED");
      if (easy->easy_handle->state.cancelled)
  ...
  if ((CURLM_STATE_COMPLETED == easy->state) && !easy->msg) {
    ...
    CURLPRINTF("multi_runsingle result = %d");
    return result;
}
...
CURLMcode curl_multi_perform(CURLM *multi_handle, int *running_handles)
{
    ...
    while(easy != &multi->easy) {
        ...
        if (easy->easy_handle->state.cancelled && easy->state == CURLM_STATE_CANCELLED) {
                               ...
                               long *timeout_ms)
{
    ...
    if(multi->timetree) {
        ...
        *timeout_ms = (multi->timetree->key - now.tv_sec) * 1000 - now.tv_usec/1000;
        if(*timeout_ms < 0)
            /* 0 means immediately */
            *timeout_ms = 0;
    }
