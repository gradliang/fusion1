...
//yuming, add
//#include "strerror.h"
#include "net_curl_easyif.h" /* for the Curl_convert_from_network prototype */
                           ...
                           size_t len)
{
    ...
    if((type & CLIENTWRITE_HEADER) && (data->set.fwrite_header || data->set.writeheader) ){
         ...
         */
         CURLPRINTF("CLIENTWRITE_HEADER");
        curl_write_callback writeit = data->set.fwrite_header?data->set.fwrite_header:data->set.fwrite;
              ...
              ssize_t *n)               /* amount bytes read */
{
    ...
    bool pipelining = (bool)(conn->data->multi && Curl_multi_canPipeline(conn->data->multi));

    CURLPRINTF("Curl_read");
    ...
    if(nread < 0)
      ...
      return CURLE_RECV_ERROR;
        ...
        if(conn->sec_complete)
            nread = Curl_sec_read(conn, sockfd, buffertofill, bytesfromsocket);
        else
...
extern U32 ImageBuffer;
extern U32 ImageBufferCounter;

size_t Curl_fread(char *buffer, size_t size, size_t nitems, size_t instream){
//read to file or read to Buffer
#if 0
    return mpx_FileRead(instream, buffer, nitems);
#else
    return nitems;
#endif
}

size_t Curl_fwrite(char *buffer, size_t size, size_t nitems, size_t outstream){
//write to file or write to Buffer
#if 0
    mpx_FileWrite(outstream, buffer, nitems);
#else
	TaskYield();
    memcpy(ImageBuffer + ImageBufferCounter, buffer, nitems);
    ImageBufferCounter += nitems;
	TaskYield();
#endif
    return nitems;
}
