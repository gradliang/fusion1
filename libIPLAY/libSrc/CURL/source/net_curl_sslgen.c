...
#define SSLGEN_C
#include "net_curl_sslgen.h" /* generic SSL protos etc */
#include "net_curl_ssluse.h" /* OpenSSL versions */
//yuming, add
...
{
    CURLPRINTF("Curl_ssl_initsessions");
#ifdef USE_SSL
    ...
    session = (struct curl_ssl_session *)calloc(sizeof(struct curl_ssl_session), amount);
    if(!session)
        return CURLE_OUT_OF_MEMORY;
    ...
    CURLPRINTF("without SSL, do nothing ");
    (void)data;
    (void)amount;
#endif
