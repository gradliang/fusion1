...
#include "net_curl_http_chunks.h" /* for the structs and enum stuff */
#include "net_curl_hostip.h"
#include "net_curl_hash.h"
#include "net_curl_splay.h"
...
//add by cj for GCE 0610
    char *x_gce_screen_size; /* free later if not NULL */
    char *x_gce_buttons; /* free later if not NULL */
    char *x_gce_device_id; /* free later if not NULL */
    char *x_gce_device_nickname;	/* free later if not NULL */
//end by cj
#endif
  } allocptr;
...
#if 0
//add by cj 0612 for GCE

  char *Refresh;
  char *URL;
  char *X_GCE_Button_OK;
  char *X_GCE_Button_Esc;
  char *X_GCE_Button_Up;
  char *X_GCE_Button_Down;
  char *X_GCE_Button_Left;
  char *X_GCE_Button_Right;
  char *X_GCE_Button_ESC;
//end by cj  
#endif
};
...
struct UserDefined {
  ...
  char *x_gce_screen_size; /* gce_screen_size */
  char *x_gce_buttons; /* gce_buttons */
  char *x_gce_device_id; /* gce_device_id */
  char *x_gce_device_nickname; /* gce_device_nickname */
	
  char *encoding;    /* Accept-Encoding string */
  char *postfields;  /* if POST, set the fields' values here */
  curl_off_t postfieldsize; 
                                /* if POST, this might have a size to use instead
...
struct SessionHandle {
  ...
  struct Curl_multi *multi;    
                                    /* if non-NULL, points to the multi handle
