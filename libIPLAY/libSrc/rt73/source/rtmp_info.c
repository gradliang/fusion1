		   ...
		   __u32 *mode, char *extra)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,"==>rt_ioctl_giwmode :%d \n",*mode);		
	return 0;
}
...
//    if (pAdapter->RTUSBCmdThr_pid < 0)
	...
	do{
		...
		{
            mpDebugPrint("rt_ioctl_siwscan: Reset");
...
{
	mpDebugPrint("rt_ioctl_siwessid ");
	...
	if ((pAdapter->Mlme.CntlMachine.CurrState != CNTL_IDLE) && (pAdapter->MLMEThr_pid > 0))
	{
        mpDebugPrint("rt_ioctl_siwessid: Reset");
		MlmeEnqueue(pAdapter, 
                    MLME_CNTL_STATE_MACHINE,
                    RT_CMD_RESET_MLME,
                    0,
...
{
	RTMP_ADAPTER *pAd = (RTMP_ADAPTER *)dev->priv;
	int keyIdx;
...
//				if (pAdapter->PortCfg.TxPowerDefault < pAdapter->PortCfg.TxPowerPercentage)
		...
		case OID_802_11_ADD_KEY: 
			...
			{		
                mpDebugPrint("AuthMode=%d", pAdapter->PortCfg.AuthMode);
				if (pAdapter->PortCfg.AuthMode >= Ndis802_11AuthModeWPA)
				{
            ...
            else
            {
                ...
                else
                {
			        ...
			        else // PortSecured is NOT SECURED
			        {
			            ...
			            AsicAddSharedKeyEntry(pAdapter, 0, (UCHAR)KeyIdx, CipherAlg, pAdapter->SharedKey[KeyIdx].Key, NULL, NULL); /* TODO */
#endif
...
//#endif	    	    
        ...
        case OID_802_11_DISASSOCIATE:
		    ...
		    if (INFRA_ON(pAdapter))
		    {
			    ...
			    {
                    mpDebugPrint("RTMPSetInformation: Reset");
				    MlmeEnqueue(pAdapter, 
				                MLME_CNTL_STATE_MACHINE,
				                RT_CMD_RESET_MLME,
				                0,
	...
	IN	PUCHAR			arg)
{
	...
	if( strlen(arg) <= MAX_LEN_OF_SSID)
	{
        ...
        mpDebugPrint("Set_SSID_Proc: Reset");
		if (pAdapter->Mlme.CntlMachine.CurrState != CNTL_IDLE)
		{
			MlmeEnqueue(pAdapter, 
                        MLME_CNTL_STATE_MACHINE,
	...
	IN	PUCHAR			arg)
{
	...
	if ((bWmmCapable == 1) && (pAd->NumberOfPipes >= 5))
	...
	IN	PUCHAR			arg)
{
	...
	if ((APSDPsmBit == 0) || (APSDPsmBit == 1))
	{
		...
		DBGPRINT(RT_DEBUG_TRACE, " Set_APSDPsm_Proc::(bAPSDForcePowerSave=%d)\n", 
	...
	IN	PUCHAR			arg)
{
	...
	NdisZeroMemory(&pAd->PortCfg.WscControl.WscSsid, sizeof(NDIS_802_11_SSID));

	if( (strlen(arg) > 0) && (strlen(arg) <= MAX_LEN_OF_SSID))
    {
		NdisMoveMemory(pAd->PortCfg.WscControl.WscSsid.Ssid, arg, strlen(arg));
		pAd->PortCfg.WscControl.WscSsid.SsidLength = strlen(arg);

		mpDebugPrint("Set_WscSsid_Proc:: (Select SsidLen=%d,Ssid=%s)\n", 
				pAd->PortCfg.WscControl.WscSsid.SsidLength, pAd->PortCfg.WscControl.WscSsid.Ssid);
	}
	else
		return FALSE;	//Invalid argument 
	...
	IN	PUCHAR			arg)
{
	...
	WscMode = (ULONG)simple_strtol(arg, 0, 10);
    
    if ((WscMode == WSC_PIN_MODE ) || (WscMode == WSC_PBC_MODE))
    {
    	// save wsc mode
        pAd->PortCfg.WscControl.WscMode = WscMode;
        // generate pin code
        WpsPinCode = pAd->PortCfg.WscControl.WscPinCode = WscGeneratePinCode(pAd);
		printk("Enrollee PinCode(8 digits)= %8u\n", pAd->PortCfg.WscControl.WscPinCode);
		DBGPRINT(RT_DEBUG_TRACE, "Enrollee PinCode(8 digits)= %8u\n", pAd->PortCfg.WscControl.WscPinCode);
    }
	else
	{
...
// add by johnli
	...
	//{
	//	printk("Set_WscGetConf_Proc:: Failed!\nPlease disable NativeWpa in dat file first!\n");
	//	return FALSE;
	//}
#endif // NATIVE_WPA_SUPPLICANT_SUPPORT //
// end johnli

	RTMPCancelTimer(&pAd->PortCfg.WscControl.WscProfileRetryTimer,&Cancelled);  // add by johnli, fix WPS test plan 5.1.1
	
	// Turn off WSC Actions before a new connection
	WscStop(pAd);

	// move here by johnli, bug fix for low power CPU platform.
	//0. Zero the data structure first
	NdisZeroMemory(&pAd->PortCfg.WscControl.RegData, sizeof(WSC_REG_DATA));

	// generate the dh public key before sending a disassoication frame.
	// This will make sure the public key generated successful then start WPS action.
	// DH key generation, it's huge calculation
	GenerateDHPublicKey(pAd->PortCfg.WscControl.pPubKeyMem, pAd->PortCfg.WscControl.RegData.EnrolleeRandom, 192, pAd->PortCfg.WscControl.RegData.Pkr, &DH_Len);
	if(DH_Len)
		memcpy(pAd->PortCfg.WscControl.RegData.Pke, pAd->PortCfg.WscControl.RegData.Pkr, DH_Len);
	// end johnli
	
	// Change to init state before sending a disassociation frame
	pAd->PortCfg.WscControl.WscState = WSC_STATE_INIT;
	...
	if (INFRA_ON(pAd))
	{
		...
		DisassocParmFill(pAd, &DisReq, pAd->PortCfg.Bssid, REASON_DISASSOC_STA_LEAVING);
		MlmeEnqueue(pAd, ASSOC_STATE_MACHINE, MT2_MLME_DISASSOC_REQ, 
		...
		pAd->PortCfg.WscControl.WscTriggered = FALSE;	// check to disable
		RTMPusecDelay(50000);  // leave enough time for this DISASSOC frame
	}
	...
	// Reset Ralink supplicant to not use
	pAd->PortCfg.WpaState = SS_NOTUSE;
	...
	// Action : PIN, PBC
	//
	if (pAd->PortCfg.WscControl.WscMode == WSC_PBC_MODE)
	{
		// WPS - SW PBC
	...
	else	// PIN  - default
	{
		...
		pAd->PortCfg.WscControl.WscMode = WSC_PIN_MODE;
		...
		// 1. Set WSC state to WSC_STATE_INIT
		// TODO: change to init state at the start
		//pAd->PortCfg.WscControl.WscState = WSC_STATE_INIT;
		
		// Init Registrar pair structures
		WscInitRegistrarPair(pAd);
		// Make sure PIN haas been copied
		WscGetRegDataPIN(pAd, pAd->PortCfg.WscControl.WscPinCode);
		
		// Store channel for BssidTable sort
		//TODO: how to get channel ??
		//pAd->MlmeAux.Channel = pWscMode->Channel;
		...
		/* select one SSID */
		if ((pAd->PortCfg.WscControl.WscSsid.SsidLength > 0) && (pAd->PortCfg.WscControl.WscSsid.SsidLength <= MAX_LEN_OF_SSID))
		{
			NdisZeroMemory(&Ssid, sizeof(NDIS_802_11_SSID));
			NdisMoveMemory(Ssid.Ssid, pAd->PortCfg.WscControl.WscSsid.Ssid, pAd->PortCfg.WscControl.WscSsid.SsidLength);
			Ssid.SsidLength = pAd->PortCfg.WscControl.WscSsid.SsidLength;
			...
			DBGPRINT(RT_DEBUG_TRACE, "Set_WscGetConf_Proc:: (SsidLen=%d,Ssid=%s)\n", 
				Ssid.SsidLength, Ssid.Ssid);
		}
		else
		{
			pAd->PortCfg.WscControl.WscState = WSC_STATE_OFF;
			pAd->PortCfg.WscControl.WscStatus = STATUS_WSC_NOTUSED;
			
			DBGPRINT(RT_DEBUG_TRACE, "Set_WscGetConf_Proc:: Failed!!! Please set WscSsid first!!!");
			return FALSE;
		}

		// 3. Set 2 min timout routine
		RTMPSetTimer(&pAd->PortCfg.WscControl.WscConnectTimer, WSC_REG_SESSION_TIMEOUT);
		pAd->PortCfg.WscControl.WscTriggered = TRUE;	//	start work
		
		// 4. Set WSC state to WSC_STATE_START
		pAd->PortCfg.WscControl.WscState = WSC_STATE_START;
		pAd->PortCfg.WscControl.WscStatus = STATUS_WSC_START_ASSOC;
	...
	DBGPRINT(RT_DEBUG_TRACE, "Set_WscGetConf_Proc trigger WSC state machine, WscGetConf=%d\n", wscGetConfMode);
	return TRUE;
}
	...
	IN	PUCHAR			arg)
{
	...
	if (INFRA_ON(pAd) && (pAd->PortCfg.WscControl.WscTriggered == TRUE))
	{
		MLME_DISASSOC_REQ_STRUCT	DisReq;
	...
	DBGPRINT(RT_DEBUG_TRACE, "Set_WscStop_Proc:: Stop WPS process !!!\n");
	return TRUE;
}
#endif // HAVE_WSC //
