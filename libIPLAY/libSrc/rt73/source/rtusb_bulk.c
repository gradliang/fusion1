...
//	mpDebugPrint("--->RTUSBBulkOutMLMEPacketComplete\n");
	pMLMEContext= (PTX_CONTEXT)pUrb->context;
	pAd = pMLMEContext->pAd;
	status = pUrb->status;
//	mpDebugPrint("urb>status %x\n",status);
...
VOID RTUSBBulkRxComplete(purbb_t pUrb, struct pt_regs *pt_regs)
{
	...
	pAd = pRxContext->pAd;
//	mpDebugPrint("RTUSBBulkRxComplete");
	//
	...
	switch (status)
	{
		...
		case -ESHUTDOWN:		// hardware gone = -108
			//pUrb = NULL;
			mpDebugPrint("==> RTUSBBulkRxComplete Error code = %d\n", status);
			break;

		default:
			mpDebugPrint("==> RTUSBBulkRxComplete UnKnown Error code = %d\n", status);
			break;
	}
...
//iverson patch usb 1.1 or 2.0 2007 1109
...
#if 0
		RTUSBRxPacket(pUrb->transfer_buffer);
#else
        ListPushTail(&pUrbInCompleteList,&pUrb->Link);
		ret = EventSet(USB_BULK_COMPLETE_EVENT, 1);
        MP_ASSERT(ret == 0);
#endif
		...
		mpDebugPrint("==> RTUSBBulkRxComplete  (IrpLock) = %d\n", atomic_read(&pRxContext->IrpLock));
#if 0
	...
	IN	usb_complete_t	Func)
{
	...
	//mpDebugPrint("pSrc %p %x",pSrc,pTxContext->BulkOutSize);
	//Initialize a tx bulk urb
	RTusb_fill_bulk_urb(pUrb,
						pAd->pUsb_Dev,
//iverson 2007 1126
#ifdef WMM_SUPPORT
						usb_sndbulkpipe(pAd->pUsb_Dev, pAd->BulkOutPipe[BulkOutPipeId]),
#else
...
//iverson 2007 1126
#ifdef WMM_SUPPORT
						usb_rcvbulkpipe(pAd->pUsb_Dev, pAd->BulkInPipe),
#else
						usb_rcvbulkpipe(pAd->pUsb_Dev, 1),
#endif
//						usb_rcvbulkpipe(pAd->pUsb_Dev, 1),
#if	RX_NO_MEMCPY
						(pRxContext->TransferBuffer+0x10),
#else						
	...
	IN	UCHAR			Index)
{
	...
	int 		ret = 0;
	unsigned long	IrqFlags;
	//K77 mpDebugPrint("RTUSBBulkOutDataPacket");
	NdisAcquireSpinLock(&pAd->BulkOutLock[BulkOutPipeId], IrqFlags);
	...
	IN	UCHAR			Index)
{
	...
	//mpDebugPrint("pMLMEContext %p",pMLMEContext);
	NdisAcquireSpinLock(&pAd->BulkOutLock[0], IrqFlags);
	if (pAd->BulkOutPending[0] == TRUE)
	{
		NdisReleaseSpinLock(&pAd->BulkOutLock[0], IrqFlags);
	...
	if((ret = rtusb_submit_urb(pUrb))!=0)
	{
		mpDebugPrint("Submit MLME URB failed %d\n", ret);
		return;
	}
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	//NdisZeroMemory(pRxContext->TransferBuffer, BUFFER_SIZE); TODO
	RTUSBInitRxDesc(pAd, pRxContext);
	
	pUrb = pRxContext->pUrb;
	if((ret = rtusb_submit_urb(pUrb))!=0)
	{
		mpDebugPrint("Submit Rx URB failed %d\n", ret);
		return;
	}
...
VOID RTUSBBulkCompleteTask( void )
{
	...
	mpDebugPrint("RTUSBBulkCompleteTask");
	while(1)
	{
		EventWait(USB_BULK_COMPLETE_EVENT,0xffffffff,OS_EVENT_OR, &dwEvent);
        if (dwEvent & 1)
        {
            /* ----------  Rx bulk in  ---------- */
            while(pUrb = (struct urb *)ListPopHead(&pUrbInCompleteList))
            {
                RTUSBBulkRxHandle((unsigned long)pUrb);
                TaskYield();
            }
        }
...
*/
		...
		{
			//mpDebugPrint("Mlme frame is next %x",pAd->PrioRingFirstIndex);
		...
		if (RTUSB_TEST_BULK_FLAG(pAd, fRTUSB_BULK_OUT_DATA_NORMAL))
		{
				...
				(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS)))
			{
				...
				mpDebugPrint("No Data or can't send");
		}
/*				
