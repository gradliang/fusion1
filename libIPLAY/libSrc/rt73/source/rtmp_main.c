...
static RTMP_ADAPTER RTpATg;
PRTMP_ADAPTER RTpAT = &RTpATg;
struct net_device *wifi_netdev;
...
extern LM_LIST_CONTAINER pUrbInCompleteList;       
extern int usb_sema;
extern int wifi_event_create;
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	mpDebugPrint("-->usb_rtusb_close_device \n");
...
//		return -1;
	...
	if (INFRA_ON(pAd))
	{
		...
		for (i=0; i<10; i++)
		{
			RTMPSendNullFrame(pAd, RATE_6, FALSE);
			RTMPusecDelay(1000);
		}
...
static int usb_rtusb_init_device(struct net_device *net_dev)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, "%s-->\n", __FUNCTION__);
	mpDebugPrint("usb_rtusb_init_device");
	...
	if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_UNPLUG_MODE))
	{
	    if(usb_sema)
	    {
			usb_sema = (int)SemaphoreCreate(USB_CONTROL_SEMA, OS_ATTR_PRIORITY, 1);
		    MP_ASSERT(usb_sema == 0);
	SemaphoreWait(USB_CONTROL_SEMA);
		...
		ret = TaskCreate(RALINK_HISR_TASK, usb_hisr_poll_handler,WIFI_PRIORITY,0x1000);
    MP_ASSERT(ret == 0);

	//Kevin Add for callback function
	ret = EventCreate(USB_BULK_COMPLETE_EVENT, (OS_ATTR_FIFO|OS_ATTR_WAIT_MULTIPLE|OS_ATTR_EVENT_CLEAR), 0);
    MP_ASSERT(ret == 0);
	ret = TaskCreate(WIFI_USB_BULK_COMPLETE_TASK, RTUSBBulkCompleteTask,WIFI_PRIORITY, 0x1000);
    MP_ASSERT(ret == 0);
	}
#endif
...
#if 1
	if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_UNPLUG_MODE))
    TaskStartup(WIFI_USB_BULK_COMPLETE_TASK);	
#endif
	...
	// Wait for hardware stable
	{
		...
		do
		{
			...
			if (MacCsr0 != 0)
			{
				mpDebugPrint("Init: MAC_CSR0=0x%08x, Status=0x%08x\n", MacCsr0, Status);
				break;
			}
	...
	//COPY_MAC_ADDR(pAd->net_dev->dev_addr, pAd->CurrentAddress); // TODO
...
//iverson 2007 1126
		...
		ifp = &dev->actconfig->interface[interface];
		iface_desc = &ifp->altsetting[0];

		/* get # of enpoints */
		pAd->NumberOfPipes = (USHORT)iface_desc->bNumEndpoints;
		...
		{ 
			...
			{
				pAd->BulkInPipe = endpoint[i].bEndpointAddress;
			...
			{
				pAd->BulkOutPipe[BulkOutIdx++] = endpoint[i].bEndpointAddress;
...
extern struct usb_device   dev_global;
...
{	
...
#else
	struct usb_device   *dev = &dev_global;
#endif
	...
	int mlme_s,rtusb_s;
	BYTE keep_flag = 0;
	BYTE txwaitlock0,txwaitlock1,txwaitlock2,txwaitlock3;
	BYTE dlock0,dlock1,dlock2,dlock3;
	
	DBGPRINT(RT_DEBUG_TRACE, "%s-->\n", __FUNCTION__);
	pUsbhDevDes->psAppClass->dwBulkOnlyState = 0;
...
#endif
	//mpDebugPrint("RTMP_ADAPTER SIZE 0x%x (%d)\n",pAd, sizeof(RTMP_ADAPTER));
	...
	if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_NIC_NOT_EXIST))
	{
	 ...
	 pid1=pAd->MLMEThr_pid;
	 pid2=pAd->RTUSBCmdThr_pid;
	 pid3=pAd->MaCRegWrite_semaphore;
	 mlme_s = pAd->mlme_semaphore;
	 rtusb_s = pAd->RTUSBCmd_semaphore;
		keep_flag = TRUE;
		netdev->flags = 0;
		txwaitlock0 = pAd->SendTxWaitQueueLock[0];
		txwaitlock1 = pAd->SendTxWaitQueueLock[1];
		txwaitlock2 = pAd->SendTxWaitQueueLock[2];
		txwaitlock3 = pAd->SendTxWaitQueueLock[3];
		dlock0 = pAd->DeQueueLock[0];
		dlock1 = pAd->DeQueueLock[1];
		dlock2 = pAd->DeQueueLock[2];
		dlock3 = pAd->DeQueueLock[3];
		
	}
	NdisZeroMemory(pAd, sizeof(RTMP_ADAPTER));
	
	if(keep_flag)
		RTMP_SET_FLAG(pAd, fRTMP_ADAPTER_UNPLUG_MODE);
	...
	if(RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_UNPLUG_MODE))
	{
	  pAd->MLMEThr_pid = pid1;
	  pAd->RTUSBCmdThr_pid = pid2;
	  pAd->MaCRegWrite_semaphore = pid3;
	  pAd->mlme_semaphore = mlme_s;
	  pAd->RTUSBCmd_semaphore = rtusb_s;
		pAd->SendTxWaitQueueLock[0]= txwaitlock0;
		pAd->SendTxWaitQueueLock[1]= txwaitlock1;
		pAd->SendTxWaitQueueLock[2]= txwaitlock2;
		pAd->SendTxWaitQueueLock[3]= txwaitlock3;
		pAd->DeQueueLock[0]= dlock0;
		pAd->DeQueueLock[1]= dlock1;
		pAd->DeQueueLock[2]= dlock2;
		pAd->DeQueueLock[3]= dlock3;
	}
	else
	{
	pAd->MLMEThr_pid= -1;
...
//<-- Benson modify
...
#else
		/* get Max Packet Size from endpoint */
		pAd->BulkOutMaxPacketSize = 64; // TODO
		DBGPRINT(RT_DEBUG_TRACE, "BulkOutMaxPacketSize  %d\n", pAd->BulkOutMaxPacketSize);
#endif			
	//res = register_netdev(pAd->net_dev);
	//if (res)
	...
	ListInitList(&pUrbOutList);
	ListInitList(&pUrbInList);
	
	ListInitList(&pUrbInCompleteList);

    TaskStartup(WIFI_USB_MLME);	
...
//static void usb_rtusb_disconnect(struct usb_interface *intf)
void usb_rtusb_disconnect()
{
	struct net_device   *net_dev = NULL;
	struct usb_device   *dev = NULL;//= interface_to_usbdev(intf);
	PRTMP_ADAPTER       pAd = (PRTMP_ADAPTER)RTpAT;
	...
	RTMP_SET_FLAG(pAd, fRTMP_ADAPTER_NIC_NOT_EXIST);
	//mpDebugPrint("unregister usbnet usb-%s-%s\n",
    ...
    if (wifi_netdev)
    {
        struct net_device *dev = wifi_netdev;
        netif_carrier_off(dev);
#if Make_USB == RALINK_AR2524_WIFI
