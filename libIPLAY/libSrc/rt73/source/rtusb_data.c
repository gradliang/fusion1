	...
	IN	struct net_device	*net_dev)
{
	struct sk_buff	*pSkb;
#if RX_NO_MEMCPY 
	PRX_CONTEXT 		pRxContext = (PRX_CONTEXT)pData;
#endif	
	//mpDebugPrint("REPORT_ETHERNET_FRAME_TO_LLC %x",DataSize);
#ifdef RTMP_EMBEDDED
	if ((pSkb = __dev_alloc_skb(DataSize + LENGTH_802_3 + 2, GFP_DMA|GFP_ATOMIC)) != NULL)
#else
    #if RX_NO_MEMCPY		
	if ((pSkb = dev_alloc_skb(sizeof(struct sk_buff))) != NULL)
	#else	
...
/*
		mpDebugPrint("DataSize = %x",DataSize);
*/
#if RX_NO_MEMCPY 
		memcpy(pRxContext->TransferBuffer+(0x10+sizeof(RXD_STRUC)+4+LENGTH_802_3),p8023hdr,LENGTH_802_3);
		pSkb->data = pRxContext->TransferBuffer+(0x10+sizeof(RXD_STRUC)+4+LENGTH_802_3);
		//mpDebugPrint("pRxContext->TransferBuffer %x",pRxContext->TransferBuffer);
		//mpDebugPrint("pSkb->data %x",pSkb->data);
		pRxContext->TransferBuffer = (PUCHAR) net_buf_mem_malloc(BUFFER_SIZE);
		//mpDebugPrint("pRxContext->TransferBuffer %x",pRxContext->TransferBuffer);
		pSkb->truesize = 1;//reuse for free skb->data
#else
		pSkb->data_len = pSkb->len = DataSize+LENGTH_802_3;
		memcpy(pSkb->data,p8023hdr,LENGTH_802_3);
		memcpy(pSkb->data+LENGTH_802_3,pData ,DataSize);        
#endif
		...
		if ((p8023hdr[0] & 0x1) == 0)
			OneSecRt73RxOkCnt++;
#endif
		netReceive(pAd->net_dev,pSkb);
		pAd->Counters8023.GoodReceives++;
	}
	else
		mpDebugPrint("Alloc Mem Fail");
}
	...
	IN	struct sk_buff	*pSkb)
{
	...
	if (pSrcBufVA == NULL)
	{
        ...
        mpDebugPrint("RTMPSendPacket: Resourece is low");
		return NDIS_STATUS_FAILURE;
	}
	...
	if ((*((PUCHAR) pSrcBufVA) & 0x01) != 0)
	{
		...
		//mpDebugPrint("For multicast & broadcast");
		NumberOfFrag = 1;
	}
	else
	{
	...
	//K77 mpDebugPrint("NumberOfFrag %x",NumberOfFrag);
	...
	if (pTxQueue->Number > pAd->MaxTxQueueSize)
	{
        ...
        mpDebugPrint("RTMPSendPacket: n=%u,m=%u", pTxQueue->Number, pAd->MaxTxQueueSize);
        MP_ASSERT(0);
		return NDIS_STATUS_FAILURE;
	}
	...
	IN	struct net_device	*net_dev)
{
	...
	{
		mpDebugPrint("RTMPSendPackets: pkt dropped: no association");
		RTUSBFreeSkbBuffer(pSkb);
		return 0;
	}
	else
	{
		...
		pAd->RalinkCounters.PendingNdisPacketCount ++;
			...
			if (Status != NDIS_STATUS_SUCCESS)
			{
                ...
                mpDebugPrint("RTMPSendPackets: pkt dropped: RTMPSendPacket returns error");
				return 0;
			}		
	}
	...
	if ((!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS)) && 
		...
		(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_HALT_IN_PROGRESS)))
	{
		//K8 mpDebugPrint("Kevin Dequeue1");
		for (Index = 0; Index < 2; Index++) // TODO 4
		{
			if(pAd->SendTxWaitQueue[Index].Number > 0)
			{
				//K8 mpDebugPrint("Kevin Dequeue2 %x",Index);
                if (rt73_debug)
                    mpDebugPrint("RTMPSendPackets: Call RTMPDeQueuePacket");
...
//iverson 2007 1126
	...
	//K NdisAcquireSpinLock(&pAd->TxRingLock, IrqFlags);//BensonLiu modify
...
//iverson 2007 1126
...
#ifdef PLATFORM_MPIXEL
            memcpy(&Iv32, (&pKey->TxTsc[2]), 4); /* alignment issue */
#else
		...
		//
		//ptmp = (unsigned char * ) (&Header_802_11);
		//K9 mpDebugPrint("%02x %02x %02x %02x %x %p %p",ptmp[0],ptmp[1],ptmp[2],ptmp[3],Header_802_11.FC.Type ,pTxContext->TransferBuffer,pWirelessPacket);
		pDest = pWirelessPacket;
		NdisMoveMemory(pDest, &Header_802_11, sizeof(Header_802_11));
		//ptmp = (unsigned char * ) (pDest);
		//K9 mpDebugPrint("%02x %02x %02x %02x",ptmp[0],ptmp[1],ptmp[2],ptmp[3]);
...
//iverson 2007 1126
		...
		if (Header_802_11.Frag == 0)
		{
			if (pExtraLlcSnapEncap)
			{
				//K8 mpDebugPrint("pExtraLlcSnapEncap");
		...
		if (SrcRemainingBytes > 0) // more fragment is required
		{
			...
			mpDebugPrint("pFrame->Hdr.FC.Type %x",pHeader80211->FC.Type);
#ifdef BIG_ENDIAN
	...
	IN	PMGMT_STRUC		pMgmt)
{
	...
	DBGPRINT_RAW(RT_DEBUG_INFO, "--->RTUSBMlmeHardTransmit\n");
	//mpDebugPrint("--->RTUSBMlmeHardTransmit pMgmt %p\n",pMgmt);
	//pAd->PrioRingTxCnt++;
...
//iverson 2007 1126
	...
	else if (pAd->PortCfg.bAPSDCapable && pAd->PortCfg.APEdcaParm.bAPSDCapable)
	{
		pHeader_802_11->FC.PwrMgmt = pAd->PortCfg.bAPSDForcePowerSave;
	}
#else
...
//	mpDebugPrint("pDest %p pBuffer %p %x",pDest,pBuffer,Length);
//	mpDebugPrint("%d",pMgmt->Length);
...
//	mpDebugPrint("TransferBufferLength %d",TransferBufferLength);
...
//iverson 2007 1126
	...
	//mpDebugPrint("CipherAlg %x",CipherAlg);
	switch (CipherAlg) 
	{
	...
	IN	UCHAR			BulkOutPipeId)
{
	...
	//K77 mpDebugPrint("pAd->TxRingTotalNumber[BulkOutPipeId] %d %d",pAd->TxRingTotalNumber[BulkOutPipeId],BulkOutPipeId);
/*
...
*/		
	...
	//K77 mpDebugPrint("pQueue->Head %p",pQueue->Head);
	...
	{		
			 ...
			 RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_HALT_IN_PROGRESS))
		{
			...
			mpDebugPrint("--RTMPDeQueuePacket %d reset-in-progress !!--\n", BulkOutPipeId);
			break;
		}
...
unsigned int RTUSBBulkInCount(unsigned long data);
...
//iverson 2007 1126
#ifdef WMM_SUPPORT
	UCHAR               Offset = 0; //  offset in QoS control bytes
#endif
	struct net_device			*net_dev;

	//mpDebugPrint("--->RTUSBRxPacket\n");
	...
	do
	{
		...
		{
#if	RX_NO_MEMCPY
		pData = pRxContext->TransferBuffer+0x10;
#else		
...
#endif
#if	0
		mpDebugPrint("pRxD CipherAlg %x",pRxD->CipherAlg);
		mpDebugPrint("pRxD Rsv %x",pRxD->Rsv);
		mpDebugPrint("pRxD DataByteCnt%x",pRxD->DataByteCnt);
		mpDebugPrint("pRxD KeyIndex %x",pRxD->KeyIndex);
		mpDebugPrint("pRxD CipherErr %x",pRxD->CipherErr);
		mpDebugPrint("pRxD Ofdm %x",pRxD->Ofdm);
		mpDebugPrint("pRxD Crc %x",pRxD->Crc);
		mpDebugPrint("pRxD MyBss %x",pRxD->MyBss);
		mpDebugPrint("pRxD Bcast %x",pRxD->Bcast);
		mpDebugPrint("pRxD Mcast %x",pRxD->Mcast);
		mpDebugPrint("pRxD U2M %x",pRxD->U2M);
		mpDebugPrint("pRxD Drop %x",pRxD->Drop);
		mpDebugPrint("pRxD Owner %x",pRxD->Owner);
		mpDebugPrint("pRxD Rsv1 %x",pRxD->Rsv1);
		mpDebugPrint("pRxD FrameOffset %x",pRxD->FrameOffset);
		mpDebugPrint("pRxD Rsv0 %x",pRxD->Rsv0);
		mpDebugPrint("pRxD PlcpRssi %x",pRxD->PlcpRssi);
		mpDebugPrint("pRxD PlcpSignal %x",pRxD->PlcpSignal);
		mpDebugPrint("pRxD Iv %x",pRxD->Iv);
		mpDebugPrint("pRxD Eiv %x",pRxD->Eiv);
#endif

		// Cast to 802.11 header for flags checking
		pHeader	= (PHEADER_802_11) (pData + sizeof(RXD_STRUC) );
//        NetPacketDump((unsigned long)pHeader, 32);
		...
		else
		{
			...
			//K7 mpDebugPrint("--->RTMPCheckRxDescriptor %x\n",Status);
		}
		if (Status == NDIS_STATUS_SUCCESS)
		{
		...
		if (Status == NDIS_STATUS_SUCCESS)
		{
			do				
			{
				...
				//K7 mpDebugPrint("pHeader->FC.Type %x",pHeader->FC.Type);
				...
				{
					//K7 mpDebugPrint("pHeader->FC.Type == BTYPE_DATA");
...
*/
					...
					if (pHeader->FC.Wep) // frame received in encrypted format
					{
						...
						{
							mpDebugPrint("Rx- drop unsupported cipher suite");
							break; // give up this frame
						}
						else if (pAd->SharedKey[pRxD->KeyIndex].KeyLen == 0)
						{
							mpDebugPrint("Rx- drop keylen is invalid");
...
#if RX_NO_MEMCPY
				REPORT_ETHERNET_FRAME_TO_LLC(pAd,Header802_3, pRxContext, DataSize, net_dev);
#else
...
//#ifdef RALINK_WPA_SUPPLICANT_SUPPORT
...
#if RX_NO_MEMCPY
                            		REPORT_ETHERNET_FRAME_TO_LLC(pAd, pAd->FragFrame.Header802_3, pRxContext, pAd->FragFrame.RxSize, net_dev);
#else
					...
					{
						//mpDebugPrint("ZZZZZZZZZZZZZZZZ Special DATA");
//#else	
...
*/
								mpDebugPrint("!!! report EAPOL/AIRONET DATA to MLME (len=%d) !!!\n");
...
#if RX_NO_MEMCPY
									REPORT_ETHERNET_FRAME_TO_LLC(pAd, Header802_3, pRxContext, Payload1Size, net_dev);
									pData2 = pRxContext->TransferBuffer + Payload1Size + LENGTH_802_3;
...
*/
								...
								else
								{
									...
									REPORT_ETHERNET_FRAME_TO_LLC(pAd, pAd->FragFrame.Header802_3, pData, pAd->FragFrame.RxSize, net_dev);
									DBGPRINT(RT_DEBUG_TRACE, "!!! report DATA (fragmented) to LLC (len=%d) !!!\n", pAd->FragFrame.RxSize);
									mpDebugPrint("!!! report DATA (fragmented) to LLC (len=%d) !!!\n", pAd->FragFrame.RxSize);
								}
								// end johnli
							}
					}
...
//					REPORT_MGMT_FRAME_TO_MLME(pAd, pHeader, pRxD->DataByteCnt, pRxD->PlcpRssi, pRxD->PlcpSignal);
					...
					REPORT_MGMT_FRAME_TO_MLME(pAd, pHeader, pData, DataSize, pRxD->PlcpRssi, pRxD->PlcpSignal);
#else
//					mpDebugPrint("Rx SubType=%d", pHeader->FC.SubType);
					if ( pHeader->FC.SubType == SUBTYPE_BEACON )
					{
						if( keep_scan  )
						{
							//K mpDebugPrint("pHeader->FC.Type == BTYPE_MGMT\n");
							REPORT_MGMT_FRAME_TO_MLME(pAd, pHeader, pData, DataSize, pRxD->PlcpRssi, pRxD->PlcpSignal);
						}
					}
					else if ( pHeader->FC.SubType != SUBTYPE_ASSOC_REQ)
					{
						...
						mpDebugPrint("pHeader->FC.Type == BTYPE_MGMT\n");
#endif
					// end johnli
					break;	// end of processing this frame
				}
...
//        NdisReleaseSpinLock(&pAd->DeMGMTQueueLock, IrqFlags2);
	...
	while ((pAd->PopMgmtIndex != pAd->PushMgmtIndex) || (atomic_read(&pAd->MgmtQueueSize) > 0))
	{
		...
		if (RTUSBFreeDescriptorRequest(pAd, PRIO_RING, 0, 1) == NDIS_STATUS_SUCCESS)
		{
			...
			//mpDebugPrint("while AAA-2");
			RTUSBMlmeHardTransmit(pAd, pMgmt);
			//mpDebugPrint("while AAA-3");
			MlmeFreeMemory(pAd, pMgmt->pBuffer);
			//mpDebugPrint("while AAA-5");
			pMgmt->pBuffer = NULL;
			pMgmt->Valid = FALSE;
		...
		{
			mpDebugPrint("not enough space in PrioRing[pAdapter->MgmtQueueSize=%d]", atomic_read(&pAd->MgmtQueueSize));
	...
	//NdisReleaseSpinLock(&pAd->DeMGMTQueueLock, IrqFlags2);
	//mpDebugPrint("<====================RTUSBDequeueMLMEPacket");
}	 
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	mpDebugPrint("SCANNING, suspend MSDU transmission ...\n");
	...
	
{
	...
	//mpDebugPrint("---> MiniportMMRequest\n");
	...
	if (pBuffer)
	{
		...
		//K NdisAcquireSpinLock(&pAd->MLMEWaitQueueLock, IrqFlags);
		pMgmt = (PMGMT_STRUC)&pAd->MgmtRing[pAd->PushMgmtIndex];
		// This management cell has been occupied
		if (pMgmt->Valid == TRUE)
		{
			mpDebugPrint("XXXXXXXXXXXXXXpMgmt->Valid == TRUE\n");			
			//NdisReleaseSpinLock(&pAd->MLMEWaitQueueLock, IrqFlags);
		...
		else
		{
			...
			//K NdisReleaseSpinLock(&pAd->MLMEWaitQueueLock, IrqFlags);
		}
	}
	else
	...
	{
//		mpDebugPrint("pAd->PrioRingTxCnt > 0\n");	
		if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_DOZE))
			AsicForceWakeup(pAd);
		RTUSBKickBulkOut(pAd);
	}
...
{	
	...
	{	
		mpDebugPrint("pRxD->Crc error %p %x",pRxD,pRxD->DataByteCnt);
...
*/
	...
	if (pRxD->CipherErr)
	{
		mpDebugPrint("pRxD->CipherErr=%d", pRxD->CipherErr);
	...
	IN	PCIPHER_KEY		pWpaKey)
{
	...
	{	//2007/12/18:KH modified to correct time-unit
...
//iverson 2007 1126
	...
	{
		pHdr80211->FC.PwrMgmt = (pAd->PortCfg.Psm == PWR_SAVE) ? 1: 0;
	}
#else
