...
void md5_mac(u8 *key, size_t key_len, u8 *data, size_t data_len, u8 *mac)
{
	...
	MD5Inita(&context);
	MD5Updatea(&context,	key, key_len);
	MD5Updatea(&context,	data, data_len);
	MD5Updatea(&context,	key, key_len);
	MD5Finala(mac, &context);
}
 ...
 */
void hmac_md5a(u8 *key, size_t key_len, u8 *data, size_t data_len, u8 *mac)
{
	MD5_CTX	context;
	...
	if (key_len	> 64) {
		...
		MD5Inita(&ttcontext);
		MD5Updatea(&ttcontext, key, key_len);
		MD5Finala(tk, &ttcontext);
	...
	/* perform inner MD5 */
	MD5Inita(&context);					 /*	init context for 1st pass */
	MD5Updatea(&context,	k_ipad,	64);	 /*	start with inner pad */
	MD5Updatea(&context,	data, data_len); /*	then text of datagram */
	MD5Finala(mac, &context);			 /*	finish up 1st pass */

	/* perform outer MD5 */
	MD5Inita(&context);					 /*	init context for 2nd pass */
	MD5Updatea(&context,	k_opad,	64);	 /*	start with outer pad */
	MD5Updatea(&context,	mac, 16);		 /*	then results of	1st	hash */
	MD5Finala(mac, &context);			 /*	finish up 2nd pass */
}
 ...
 */
	VOID MD5Updatea(MD5_CTX *pCtx, unsigned char *pData, unsigned int LenInBytes)
{
    ...
    if (temp) 
    {
        ...
        MD5Transforma(pCtx->Buf, (ULONG *)pCtx->Input);
 ...
 */
VOID MD5Finala(UCHAR Digest[16], MD5_CTX *pCtx)
{
    UCHAR Remainder;
    UCHAR PadLenInBytes;
    UCHAR *pAppend=0;
 ...
 */
	VOID MD5Transforma(unsigned int Buf[4], unsigned int Mes[16])
{  
    ULONG Reg[4], Temp; 
	unsigned int i;
...
void aes_encrypta(aes_context *ctx, uint8 input[16],	uint8 output[16] )
{
...
void aes_decrypta( aes_context *ctx,	uint8 input[16], uint8 output[16] )
{
	uint32 *RK,	X0,	X1,	X2,	X3,	Y0,	Y1,	Y2,	Y3;
