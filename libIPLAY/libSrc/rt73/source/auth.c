    ...
    IN MLME_QUEUE_ELEM *Elem) 
{
    ...
    else if(MlmeAuthReqSanity(pAd, Elem->Msg, Elem->MsgLen, Addr, &Timeout, &Alg))
    {
        ...
        {
            mpDebugPrint("AUTH - MlmeAuthReqAction() allocate memory failed\n");
            pAd->Mlme.AuthMachine.CurrState = AUTH_REQ_IDLE;
    ...
    {
        mpDebugPrint("AUTH - MlmeAuthReqAction() sanity check failed. BUG!!!!!\n");
        pAd->Mlme.AuthMachine.CurrState = AUTH_REQ_IDLE;
    ...
    IN MLME_QUEUE_ELEM *Elem) 
{
    ...
    mpDebugPrint("PeerAuthRspAtSeq2Action");
    if (PeerAuthSanity(pAd, Elem->Msg, Elem->MsgLen, Addr2, &Alg, &Seq, &Status, ChlgText))
    {
        if (MAC_ADDR_EQUAL(&pAd->MlmeAux.Bssid, Addr2) && Seq == 2) 
        {
            ...
            if (Status == MLME_SUCCESS) 
            {
                ...
                {
				    mpDebugPrint("PeerAuthRspAtSeq2Action AUTH_REQ_IDLE");
                    pAd->Mlme.AuthMachine.CurrState = AUTH_REQ_IDLE;
                    MlmeEnqueue(pAd, MLME_CNTL_STATE_MACHINE, MT2_AUTH_CONF, 2, &Status);
                } 
                else
            ...
            {
			    mpDebugPrint("PeerAuthRspAtSeq2Action-5");
    ...
    IN MLME_QUEUE_ELEM *Elem) 
{
	...
	mpDebugPrint("MlmeDeauthReqAction");
    ...
    IN MLME_QUEUE_ELEM *Elem) 
{
	...
	mpDebugPrint("AUTH - AuthTimeoutAction\n");
    pAd->Mlme.AuthMachine.CurrState = AUTH_REQ_IDLE;
...
{
    USHORT  Status;
	mpDebugPrint("InvalidStateWhenAuth");
