...
//    mpDebugPrint("mac wr:o=0x%x,v=0x%x", Offset,Value);
	RTUSBMacRegDown(pAd);
...
retry:
	...
	if ((i == RETRY_LIMIT) || (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_NIC_NOT_EXIST)))
	{
		...
		*pValue = pAd->BbpWriteLatch[Id];
		...
		DBGPRINT_RAW(RT_DEBUG_ERROR, "Retry count exhausted or device removed 1!!!\n");
				mpDebugPrint("reg=%x", PhyCsr3.word);
                k++;
                if (k < 2)
                    goto retry;
	...
	if ((i == RETRY_LIMIT) || (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_NIC_NOT_EXIST)))
	{
		...
		*pValue = pAd->BbpWriteLatch[Id];
		...
		DBGPRINT_RAW(RT_DEBUG_ERROR, "Retry count exhausted or device removed 2!!!(%d)\n", i);
		return STATUS_UNSUCCESSFUL;
	}
	...
	IN	UCHAR			Value)
{
	...
	if ((i == RETRY_LIMIT) || (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_NIC_NOT_EXIST)))
	{
		DBGPRINT_RAW(RT_DEBUG_ERROR, "Retry count exhausted or device removed 3!!!\n");
        mpDebugPrint("i=%d", i);
		return STATUS_UNSUCCESSFUL;
	}
	...
	IN	ULONG			Value)
{
	...
	if ((i == RETRY_LIMIT) || (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_NIC_NOT_EXIST)))
	{
		DBGPRINT_RAW(RT_DEBUG_ERROR, "Retry count exhausted or device removed 4!!!\n");
		mpDebugPrint("i=%d", i);
		return STATUS_UNSUCCESSFUL;
	}
...
//    mpDebugPrint("%s (req=%d,typ=%x)",__FUNCTION__, Request,RequestType);
	...
	else
	{
		...
		do
		{
			if( RequestType == DEVICE_VENDOR_REQUEST_OUT)
				ret=usb_control_msg(dev,pipe,Request, RequestType, Value,Index, TransferBuffer, TransferBufferLength, CONTROL_TIMEOUT_JIFFIES);
