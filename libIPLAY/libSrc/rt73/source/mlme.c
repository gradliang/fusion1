...
extern ULONG			OneSecRt73RxOkCnt;
static ULONG			Rt73RxOkCnt;
#endif
	...
	IN PRTMP_ADAPTER pAd) 
{
	...
	do 
	{
		...
		if(!MlmeHandlerSemid)
		{
		MlmeHandlerSemid = mpx_SemaphoreCreate(OS_ATTR_PRIORITY, 1);			
		pAd->MaCRegWrite_semaphore = mpx_SemaphoreCreate(OS_ATTR_PRIORITY, 1);			
		}
		
        MP_ASSERT(pAd->MaCRegWrite_semaphore > 0);
		// initialize table
	...
	IN PRTMP_ADAPTER pAd) 
{
	...
	{	 
		...
		//mpDebugPrint("Kevin::if(pAd->Mlme.bRunning) ");
		return;
	} 
	else 
	{
	...
	while (!MlmeQueueEmpty(&pAd->Mlme.Queue)) 
	{
			...
			RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_NIC_NOT_EXIST))
		{
			...
			mpDebugPrint("Device Halted or Removed or MlmeRest, exit MlmeHandler! (queue num = %d)\n", pAd->Mlme.Queue.Num);
			break;
		}
		...
		if (MlmeDequeue(&pAd->Mlme.Queue, &Elem)) 
		{
			if (Elem->MsgType == RT_CMD_RESET_MLME)
			{
				...
				mpDebugPrint("!!! reset MLME state machine !!!\n");
			...
			//mpDebugPrint("MLME(%d) %d\n",Elem->Machine, Elem->MsgType);
			...
			switch (Elem->Machine) 
			{
				...
				default:
					...
					mpDebugPrint("ERROR: Illegal machine in MlmeHandler()\n");
					break;
			} // end of switch
...
//2007/12/20:Added by WY
      if (!((pAd->net_dev) && (pAd->net_dev->flags & IFF_UP)) )
      {
		mpDebugPrint("MlmePeriodicExec: device down");
		return;
      }
				...
				(MlmeValidateSSID(pAd->MlmeAux.AutoReconnectSsid, pAd->MlmeAux.AutoReconnectSsidLen) == TRUE))
			{
				if ((pAd->ScanTab.BssNr==0) && (pAd->Mlme.CntlMachine.CurrState == CNTL_IDLE))
				{
					...
					if ((pAd->PortCfg.LastScanTime + 10 * HZ) < pAd->Mlme.Now32)
					{
						...
						mpDebugPrint("CNTL - No matching BSS, start a new ACTIVE scan SSID[%s]\n", pAd->MlmeAux.AutoReconnectSsid);
...
//2007/11/09:KH replace the original MlmeAutoReconnectLastSSID
	...
	IN PRTMP_ADAPTER pAd)
{
    ...
    if ((pAd->Mlme.CntlMachine.CurrState == CNTL_IDLE) && ((pAd->PortCfg.LastScanTime + (10*HZ)) >= Now))		// BUG (10 sec)
    {
	if (pAd->Mlme.CntlMachine.CurrState == CNTL_IDLE &&
		(MlmeValidateSSID(pAd->MlmeAux.AutoReconnectSsid, pAd->MlmeAux.AutoReconnectSsidLen) == TRUE))
	{
      ...
      else if ((pAd->PortCfg.LastScanTime + (10*HZ)) < Now)								
      {
		// check CntlMachine.CurrState to avoid collision with NDIS SetOID request
		DBGPRINT(RT_DEBUG_TRACE, "MMCHK - FastRoaming, No eligable entry, try new scan!\n");
	...
	IN	ULONG			Now)
{
	...
	//
        ...
        //
        //KH modified time-unit from jiffies to HZ
	...
	for (i = 0; i < pAd->ScanTab.BssNr; i++)
	{
		...
		/*2007/11/05:KH replace the line"if ((pBss->Rssi <= 45) && (pBss->Channel == pAd->PortCfg.Channel))" to enable the roaming between different channels.
		*/
			if(pBss->Rssi<=45)
			continue;    // RSSI too weak. forget it.
		if (MAC_ADDR_EQUAL(pBss->Bssid, pAd->PortCfg.Bssid))
			continue;    // skip current AP
                ...
                //2007/12/20:correct hidden bugs 
		...
		NdisMoveMemory(&pRoamTab->BssEntry[pRoamTab->BssNr], pBss, sizeof(BSS_ENTRY));
                 ...
                 //2007/12/06:KH add to fix roaming with hidden SSID
	...
	IN ULONG Now32)
{
		...
		(pAd->PortCfg.LastBeaconRxTime + BEACON_LOST_TIME < Now32))
	{
        ...
        mpDebugPrint("BEACON lost > %d msec with TxOkCnt=%d -> CQI=0\n", BEACON_LOST_TIME, TxOkCnt);
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	// Disable Rx
	RTUSBWriteMACRegister(pAd, TXRX_CSR0, 0x0257b032);			
...
{	
	...
	RTUSBWriteMACRegister(pAd, TXRX_CSR0, 0x025eb032);	// Staion not drop control frame will fail WiFi Certification.
	...
	// To 
	//
	mpDebugPrint("MlmeRadioOn: Reset");
	MlmeEnqueue(pAd, 
				MLME_CNTL_STATE_MACHINE,
				RT_CMD_RESET_MLME,
				0,
	...
	IN PNDIS_802_11_VARIABLE_IEs pVIE)	
{
	...
	MP_ASSERT(SupRateLen <= MAX_LEN_OF_SUPPORTED_RATES);
	NdisMoveMemory(pBss->SupRate, SupRate, SupRateLen);
	pBss->SupRateLen = SupRateLen;
	MP_ASSERT(ExtRateLen <= MAX_LEN_OF_SUPPORTED_RATES);
	...
	if (LengthVIE != 0)
	{
		MP_ASSERT(LengthVIE <= MAX_VIE_LEN);
	...
	IN PNDIS_802_11_VARIABLE_IEs pVIE) 
{
	...
	{
		mpDebugPrint("###############3Idx == BSS_NOT_FOUND################# %d", Tab->BssNr);
		...
		Idx = Tab->BssNr;
		mpDebugPrint("Ssid=%s %x", wpa_ssid_string(NULL, Ssid, SsidLen),Tab->BssNr);
		BssEntrySet(pAd, &Tab->BssEntry[Idx], pBssid, Ssid, SsidLen, BssType, BeaconPeriod,
					CfParm, AtimWin, CapabilityInfo, SupRate, SupRateLen, ExtRate, ExtRateLen,
					ChannelNo, Rssi, TimeStamp, CkipFlag, pEdcaParm, pQosCapability, pQbssLoad, LengthVIE, pVIE);
...
#if	0
	mpDebugPrint("pAd->ScanTab.BssNr %x %x",pAd->ScanTab.BssNr,pAd->PortCfg.AuthMode);
	mpDebugPrint("SsidLen %x",SsidLen);
	mpDebugPrint("Ssid %s",wpa_ssid_string(NULL, Ssid, SsidLen));
#endif
	for (i = 0; i < pAd->ScanTab.BssNr; i++) 
	{
		BSS_ENTRY *pInBss = &pAd->ScanTab.BssEntry[i];
	#if	0
		mpDebugPrint("pAd->ScanTab.BssNr %c%c",pInBss->Ssid[0],pInBss->Ssid[1]);
		
		mpDebugPrint("pInBss->BssType %x pAd->PortCfg.BssType %x",pInBss->BssType,pAd->PortCfg.BssType);
		
		mpDebugPrint("pInBss->SsidLen %x",pInBss->SsidLen);
		
		mpDebugPrint("pInBss->WepStatus %x",pInBss->WepStatus);
	#endif	
...
#else
				if (SWAP16(pRsnHeader->Version) != 1)
#endif
					break;
	...
	IN MLME_QUEUE *Queue) 
{
	...
	if(!Queue_Lock)
	Queue_Lock = mpx_SemaphoreCreate(OS_ATTR_PRIORITY, 1);			
...
//	PFRAME_802_11	pFrame = (PFRAME_802_11)Msg;
	...
	if(pAd->ate.Mode != ATE_STASTART)
		return FALSE;
#endif
	//mpDebugPrint("MlmeEnqueueForRecv==============1");
	// Do nothing if the driver is starting halt state.
	// This might happen when timer already been fired before cancel timer with mlmehalt
	if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_HALT_IN_PROGRESS))
		return FALSE;
...
*/
	...
	if (MlmeQueueFull(Queue)) 
	{
		mpDebugPrint("MlmeQueueFull(Queue)");
...
*/
	...
	//mpDebugPrint("MlmeEnqueueForRecv==============4");	
	//mpDebugPrint("Machine %x %x",Machine,SYNC_STATE_MACHINE);
	//mpDebugPrint("MsgType %x %x",MsgType,MT2_PEER_PROBE_RSP);
	//mpDebugPrint("pAd->PortCfg.bGetAPConfig %d",pAd->PortCfg.bGetAPConfig);
	...
	//mpDebugPrint("MlmeEnqueueForRecv, num=%d\n",Queue->Num);	
	Queue->Entry[Tail].Occupied = TRUE;
	Queue->Entry[Tail].Machine = Machine;
	Queue->Entry[Tail].MsgType = MsgType;
	// edit by johnli, fix WPS & WPAPSK/WPA2PSK bugs for receiving EAPoL fragmentation packets
...
//	NdisMoveMemory(Queue->Entry[Tail].Msg, Msg, MsgLen);
	...
	SemaphoreRelease(Queue_Lock);	
	RTUSBMlmeUp(pAd);
	//mpDebugPrint("MlmeEnqueueForRecv==============5");	
	return TRUE;
}
	...
	OUT MLME_QUEUE_ELEM **Elem) 
{
	...
	*Elem = &(Queue->Entry[Queue->Head]);
	Queue->Num--;
	//mpDebugPrint("AQueue->Head %x\n",Queue->Head);
		Queue->Head++;
	//mpDebugPrint("BQueue->Head %x\n",Queue->Head);
	...
	//mpDebugPrint("MlmeDequeue, num=%d\n",Queue->Num);
...
//iverson 2007 1126
#ifdef WMM_SUPPORT
		if ((pFrame->Hdr.FC.SubType & 0x08) && OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_WMM_INUSED))
			Offset += 2;
#endif	
#ifdef WMM_SUPPORT
...
#endif
*/
		EAPType = *(pFrame+LENGTH_802_1_H+1);
// end johnli
	...
	IN STATE_MACHINE_FUNC Func) 
{
	...
	MsgIdx = Msg - S->Base;
	mpDebugPrint("%x %x %x",MsgIdx,Msg,S->Base);

	if (St < S->NrState && MsgIdx < S->NrMsg) 
	{
		// boundary checking before setting the action
		S->TransFunc[St * S->NrMsg + MsgIdx] = Func;
		mpDebugPrint("%p ",S->TransFunc[St * S->NrMsg + MsgIdx]);
	} 
	else
		mpDebugPrint("Kevin Watch %x %x %x ",St,S->NrMsg,MsgIdx);
}
...
{
	MP_ASSERT(*(S->TransFunc[S->CurrState * S->NrMsg + Elem->MsgType - S->Base]));

	//mpDebugPrint("KKKKJJJJJ %p",*(S->TransFunc[S->CurrState * S->NrMsg + Elem->MsgType - S->Base]));
...
VOID Drop(
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
//	mpDebugPrint("Msg Dropped %d", Elem->MsgType);
    // 9 = MT2_PEER_PROBE_REQ
}
	...
	IN UCHAR Channel) 
{
	...
	switch (pAd->RfIcType)
	{
	    ...
	    case RFIC_3020:
			...
			{
				if (Channel == RF3020RegTable[index].Channel)
				{
					R3 = R3 | (RF3020RegTable[index].R3 & 0xffffc1ff); // set TX power
					R4 = (RF3020RegTable[index].R4 & (~0x0003f000)) | (pAd->RfFreqOffset << 12);

					// Update variables
					pAd->LatchRfRegs.Channel = Channel;
					pAd->LatchRfRegs.R1 = RF3020RegTable[index].R1;
					pAd->LatchRfRegs.R2 = RF3020RegTable[index].R2;
					pAd->LatchRfRegs.R3 = R3;
					pAd->LatchRfRegs.R4 = R4;
...
//	mpDebugPrint("K2 %x",csr.word);
	RTUSBWriteMACRegister(pAd, TXRX_CSR9, csr.word);
//	mpDebugPrint("K3 %x",csr.word);
}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	for (i=0; i<TXINFO_SIZE; )  // 24-byte TXINFO field
	{
		longptr =  *ptr + (*(ptr+1)<<8) + (*(ptr+2)<<16) + (*(ptr+3)<<24);
		RTUSBWriteMACRegister(pAd, HW_BEACON_BASE0 + i, longptr);
		ptr += 4;
		i+=4;
	}
	...
	ptr = pAd->BeaconBuf;
	for (i=0; i< (pAd->BeaconTxD.DataByteCnt);)
	{
		longptr =  *ptr + (*(ptr+1)<<8) + (*(ptr+2)<<16) + (*(ptr+3)<<24);
		RTUSBWriteMACRegister(pAd, HW_BEACON_BASE0 + TXINFO_SIZE + i, longptr);
		ptr += 4;
		i+=4;
	}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	{
		DBGPRINT(RT_DEBUG_INFO, "RSSI=%d, CCA=%d, keep R17 at 0x%x\n", dbm, pAd->RalinkCounters.OneSecFalseCCACnt, R17);
	}
...
#else
	DBGPRINT_DUMP(RT_DEBUG_TRACE, pKey, 16);
#endif
	if (pRxMic)
	{
		...
		DBGPRINT_RAW(RT_DEBUG_TRACE, "\n");
#else
		DBGPRINT_DUMP(RT_DEBUG_TRACE, pRxMic, 8);
#endif
	}
	if (pTxMic)
	{
		...
		DBGPRINT_RAW(RT_DEBUG_TRACE, "\n");
#else
		DBGPRINT_DUMP(RT_DEBUG_TRACE, pTxMic, 8);
#endif
	}
	...
	IN PUCHAR		 pRxMic)
{
	...
	for (i=0; i<MAX_LEN_OF_PEER_KEY; i = i+4) 
	{
		key = (ULONG)(pKey[i])		 | 
				(ULONG)(pKey[i+1] << 8)  | 
				(ULONG)(pKey[i+2] << 16) |
		...
		RTUSBWriteMACRegister(pAd, (USHORT)(offset + i), key);
	}
	offset += MAX_LEN_OF_PEER_KEY;
	...
	addr = (ULONG)(pAddr[0])		| 
			...
			(ULONG)(pAddr[2] << 16) |
			(ULONG)(pAddr[3] << 24);
	RTUSBWriteMACRegister(pAd, (USHORT)(offset), addr);
		
	addr = (ULONG)(pAddr[4]) | (ULONG)(pAddr[5] << 8) | (ULONG)(CipherAlg << 16);
	RTUSBWriteMACRegister(pAd, (USHORT)(offset + 4), addr);
	...
	{		
		...
		DBGPRINT(RT_DEBUG_TRACE,"PAIRWISE_KEY_TABLE_BASE==> \n");
		tmp_offset = PAIRWISE_KEY_TABLE_BASE + (KeyIdx * HW_KEY_ENTRY_SIZE);
		for (i=0; i<HW_KEY_ENTRY_SIZE; i=i+4)
		{
			RTUSBReadMACRegister(pAd, tmp_offset + i, &Value1);
			DBGPRINT(RT_DEBUG_TRACE, "MAC:Offset[0x%04x]=[0x%04x]\n", tmp_offset + i, Value1);
		}

		DBGPRINT(RT_DEBUG_TRACE,"PAIRWISE_TA_TABLE_BASE==> \n");																
		tmp_offset = PAIRWISE_TA_TABLE_BASE + (KeyIdx * HW_PAIRWISE_TA_ENTRY_SIZE);
		for (i=0; i<HW_PAIRWISE_TA_ENTRY_SIZE; i = i+4)
		{
			RTUSBReadMACRegister(pAd, tmp_offset + i, &Value1);
			DBGPRINT(RT_DEBUG_TRACE, "MAC:Offset[0x%04x]=[0x%04x]\n", tmp_offset + i, Value1);
		}				
	...
	IN UCHAR			Pair2)
{
	...
	if ((pAd->RfIcType == RFIC_2528) || (pAd->RfIcType == RFIC_3020))
	{
		UCHAR		R77;
	...
	IN PRTMP_ADAPTER pAd) 
{
	...
	if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED))
		RTMPSetTimer(&pAd->RxAnt.RxAntDiversityTimer, 100);
	else
		RTMPSetTimer(&pAd->RxAnt.RxAntDiversityTimer, 300);
	...
	OUT PVOID		 *AllocVa)
{
	...
	*AllocVa = pMlmeMemoryStruct->AllocVa;			//Saved porint to Pointer the base virtual address of the nonpaged memory for caller use.
	...
	DBGPRINT(RT_DEBUG_INFO, "MlmeAllocateMemory [pMlmeMemoryStruct=0x%x][VA=0x%x]\n", (UINT)pMlmeMemoryStruct, (UINT)pMlmeMemoryStruct->AllocVa);
	...
	IN UINT  Size)
{
	...
	for (i = 0; i < Number; i++)
	{
		...
		if (!Current) 
		{
			mpDebugPrint("Not enough memory\n");
		...
		Current->AllocVa= (VOID *) net_buf_mem_malloc(Size);
		if (!Current->AllocVa) 
		{
			mpDebugPrint("Not enough memory\n");
			kfree((VOID *)Current);
		...
		NdisZeroMemory(Current->AllocVa, Size);
	...
	{
		Status = NDIS_STATUS_RESOURCES;
		mpDebugPrint("MlmeInitMemory Initial failed [Require=%d, available=%d]\n", Number, pAd->Mlme.MemHandler.MemoryCount);
	}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	while (pAd->Mlme.MemHandler.pInUseHead != NULL)
	{
		...
		net_buf_mem_free(pMlmeMemoryStruct->AllocVa);
