...
//UCHAR	 RT_BIT8[] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80};
...
//	pAd->MLMEThr_pid = kernel_thread(MlmeThread, pAd, CLONE_VM);
	ret = TaskCreate(WIFI_USB_MLME, MlmeThread,WIFI_PRIORITY, 0x8000);
    if (ret == 0)
        pAd->MLMEThr_pid = WIFI_USB_MLME;
	...
	ret = TaskCreate(WIFI_USB_CMD,RTUSBCmdThread,WIFI_PRIORITY, 0x1000);
    if (ret == 0)
        pAd->RTUSBCmdThr_pid = WIFI_USB_CMD;
	...
	{		
		...
		for (acidx = 0; acidx < 1; acidx++)//4
		{
			for ( i= 0; i < TX_RING_SIZE; i++ )
			{
				...
				//mpDebugPrint("TransferBuffer = %x",sizeof(TX_BUFFER));
				//pTxContext->TransferBuffer= (PTX_BUFFER) kmalloc(sizeof(TX_BUFFER), GFP_KERNEL);
//tx_realloc:
				pTxContext->TransferBuffer= (PTX_BUFFER) net_buf_mem_malloc(sizeof(TX_BUFFER));
//				if( ( (unsigned long)(pTxContext->TransferBuffer) & 0xfff) + 0x800 >  0x1000 )
//				{
					//mpDebugPrint("KGGGGGGGGGGGGG %p",pTxContext->TransferBuffer->u.WirelessPacket);
//					net_buf_mem_free(pTxContext->TransferBuffer);
//					goto tx_realloc;
//				}
...
//retry1:
			...
			pMLMEContext->TransferBuffer= (PTX_BUFFER) net_buf_mem_malloc(sizeof(TX_BUFFER));
	...
	//		if( ( (unsigned long)(pMLMEContext->TransferBuffer) & 0xfff) + 0x800 >  0x1000 )
	//		{
	//			mpDebugPrint("KGGGGGGGGGGGGG %p",pMLMEContext->TransferBuffer);
	//			net_buf_mem_free(pMLMEContext->TransferBuffer);
	//			goto retry1;
	//		}
...
//retry2:
		...
		pNullContext->TransferBuffer= (PTX_BUFFER) net_buf_mem_malloc(sizeof(TX_BUFFER));
		if(!pNullContext->TransferBuffer){
			DBGPRINT(RT_DEBUG_ERROR,"Not enough memory\n");
			Status = NDIS_STATUS_RESOURCES;
			goto out4;
		}
	//	if( ( (unsigned long)(pNullContext->TransferBuffer) & 0xfff) + 0x800 >  0x1000 )
	//	{
	//		mpDebugPrint("KFFFFFFFFFGGGG %p",pNullContext->TransferBuffer);
	//		net_buf_mem_free(pNullContext->TransferBuffer);
	//		goto retry2;
	//	}
...
//retry3:
		...
		pRTSContext->TransferBuffer= (PTX_BUFFER) net_buf_mem_malloc(sizeof(TX_BUFFER));
		if(!pRTSContext->TransferBuffer){
			DBGPRINT(RT_DEBUG_ERROR,"Not enough memory\n");
			Status = NDIS_STATUS_RESOURCES;
			goto out5;
		}
	//	if( ( (unsigned long)(pRTSContext->TransferBuffer) & 0xfff) + 0x800 >  0x1000 )
	//	{
	//		mpDebugPrint("KFFFFFFFFFGGGG %p",pRTSContext->TransferBuffer);
	//		net_buf_mem_free(pRTSContext->TransferBuffer);
	//		goto retry3;
	//	}
...
//retry4:
		...
		pPsPollContext->TransferBuffer= (PTX_BUFFER) net_buf_mem_malloc(sizeof(TX_BUFFER));
		if(!pPsPollContext->TransferBuffer){
			DBGPRINT(RT_DEBUG_ERROR,"Not enough memory\n");
			Status = NDIS_STATUS_RESOURCES;
			goto out6;
		}
//		if( ( (unsigned long)(pPsPollContext->TransferBuffer) & 0xfff) + 0x800 >  0x1000 )
//		{
//			mpDebugPrint("KFFFFFFFFFGGGG %p",pPsPollContext->TransferBuffer);
//			net_buf_mem_free(pPsPollContext->TransferBuffer);
//			goto retry4;
//		}
...
#else
			net_buf_mem_free(pBeaconContext->TransferBuffer);
#endif
...
//rx_realloc:						
		...
		//if( ( (unsigned long)(pRxContext->TransferBuffer) & 0xfff) + 0xc00 >  0x1000 )
		//{
			//mpDebugPrint("RXGGGGGGGGGGGG %p",pRxContext->TransferBuffer);
		//	net_buf_mem_free(pRxContext->TransferBuffer);
		//	goto rx_realloc;
		//}
		//mpDebugPrint("RX %p",pRxContext->TransferBuffer);
	...
	IN  BOOLEAN         IsOnlyTx) 
{
	...
	for (acidx = 0; acidx < 1; acidx++)         /* TODO 4->1 */
	{
		for ( i= 0; i < TX_RING_SIZE; i++ )
		{
			PTX_CONTEXT pTxContext = &(pAd->TxContext[acidx][i]);
...
#else
		if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_UNPLUG_MODE))
		{
		pAd->mlme_semaphore = mpx_SemaphoreCreate(OS_ATTR_PRIORITY, 1);			
        MP_ASSERT(pAd->mlme_semaphore > 0);
        SemaphoreWait(pAd->mlme_semaphore);	
		pAd->RTUSBCmd_semaphore = mpx_SemaphoreCreate(OS_ATTR_PRIORITY, 1);			
        MP_ASSERT(pAd->RTUSBCmd_semaphore > 0);
        SemaphoreWait(pAd->RTUSBCmd_semaphore);	
		}
#endif
		//2008/01/07:KH add to solve the racing condition of Mac Registers
	...
	IN	PRTMP_ADAPTER		pAd)
{
	...
	mpDebugPrint("Local MAC = %02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	mpDebugPrint("--> NICReadEEPROMParameters\n");
	...
	mpDebugPrint("E2PROM: Version = %d, FAE release #%d\n", Version.field.Version, Version.field.FaeReleaseNumber);
	...
	// 0. 11b/g
	{
		...
		{
			mpDebugPrint("pAd->bAutoTxAgcG = FALSE");
	...
	// Get LED Setting.
	//
	RTUSBReadEEPROM(pAd, EEPROM_LED_OFFSET, &LedSetting, 2);
	/*
	...
	pAd->LedCntl.word = LedSetting<<4;
	
	mpDebugPrint("pAd->LedCntl.word = %x\n",pAd->LedCntl.word);
	/*
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	if (Antenna.word == 0xFFFF)
	{
		...
		mpDebugPrint("E2PROM error, hard code as 0x%04x\n", Antenna.word);
	}

	pAd->RfIcType = (UCHAR) Antenna.field.RfIcType;
	mpDebugPrint("pAd->RfIcType = %d\n", pAd->RfIcType);
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	mpDebugPrint("--> NICInitializeAsic\n");
...
//	mm_segment_t			orgfs;
	...
	{
		mpDebugPrint("NICLoadFirmware-Memory allocate fail\n");
	...
	IN	PRTMP_ADAPTER pAd)
{
	...
	mpDebugPrint("RTMPReadParametersFromFile");
	pAd->PortCfg.CountryRegion = 0;
	pAd->PortCfg.CountryRegionForABand = 7;
	pAd->PortCfg.BssType = BSS_INFRA;
	pAd->RfIcType = RFIC_2528;
	//WirelessMode
...
//	pAd->PortCfg.wx_eapol_flags = 0;
	...
	// If the device controlled by wpa_supplicant wext dirver, we should not enable AutoReconnect.
	pAd->PortCfg.bAutoReconnect = FALSE;
#endif // RALINK_WPA_SUPPLICANT_SUPPORT //
#endif
...
#else
	_init_timer(&pTimer->TimerObj, pAd, pTimerFunc, pTimer);
#endif
