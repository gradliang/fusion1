...
static void Local_HW_InputWindow2_Set(void)
{
	...
	MP_ALERT("#### set win2 H RegF6=%d",ipu->Ipu_reg_F6);
	MP_ALERT("#### set win2 W regF7=%d",ipu->Ipu_reg_F7);
...
BYTE Global_Motion_Detection_Calc(void)
{
		...
		MD_Value=ov2643_Sensor_GetAvgLuminance();
...
int Global_Sensor_Initial_OV2643(void)
{
	extern BYTE Sensor_Mode;
	IPU *ipu = (IPU *) IPU_BASE;
	INTERRUPT *isr = (INTERRUPT *) INT_BASE;
