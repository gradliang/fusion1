...
#if (defined(MAGIC_SHOP_ENABLE) && (MAGIC_SHOP_DEBUG == 1))
extern DWORD start_addr_PC_tool_mem_ColorIPU[];
...
static void Local_HW_Path_Set(void)
{
	...
	***************************/
...
#else
	/*Add color manager into path*/
	#if (MAGIC_SHOP_DEBUG) //for Magic Shop debug only
		start_addr_PC_tool_Ptr = (BYTE *)start_addr_PC_tool_mem_ColorIPU;
	#else
		start_addr_PC_tool_Ptr = start_addr_PC_tool ;
	#endif
	
	DWptr  = ((U32)start_addr_PC_tool_Ptr + MAGIC_SHOP_COLOR_MANAGER);
	
	tmp = ((*DWptr) & 0x80000000);
	if(tmp == 0x80000000)
	{
		MP_ALERT("%s:Using Magic Shop Color Manager", __FUNCTION__);
		 /*--Add	color manager into sensor image input path--*/
		if(sensor_mode != MODE_CAPTURE)
		{
		...
		else
		{
			...
			{	
				/*capture mode not color manager*/	
	...
	DWptr  = ((U32)start_addr_PC_tool_Ptr + MAGIC_SHOP_COLOR_MANAGER);
	tool = (TOOL_Color *) DWptr;
	
	DWptr  = ((U32)start_addr_PC_tool_Ptr + MAGIC_SHOP_COLOR_MANAGER + 4);
	pc_tool_ipu = (IPU *) DWptr;
	...
	if(tmp == 0x80000000){
		...
		if(tmp)	
		{
			ipu->Ipu_reg_82 = pc_tool_ipu->Ipu_reg_82;
		}
...
#define Motion_Detection_Enable 1
...
BYTE Global_Motion_Detection_Calc(void)
{
		...
		MD_Value = SensorTool_Sensor_GetAvgLuminance();
...
int Global_Sensor_Initial_SensorTool(void)
{
	extern BYTE Sensor_Mode;
	IPU *ipu = (IPU *) IPU_BASE;
	INTERRUPT *isr = (INTERRUPT *) INT_BASE;
	...
	WORD Capture_preview_W,Capture_preview_H;
	BYTE  *start_addr_PC_tool_Ptr;
	TOOL  *toolAddrTemp;
	...
	extern DWORD start_addr_PC_tool_mem[];
	start_addr_PC_tool_Ptr = start_addr_PC_tool_mem;
#else

	start_addr_PC_tool_Ptr = start_addr_PC_tool;
#endif

	toolAddrTemp  = (TOOL *)((U32)start_addr_PC_tool_Ptr + PREVIEW_RESOLUTION);
	temp		  = toolAddrTemp->b_code1;

	Capture_preview_H = (WORD)((temp & 0xffff0000)>>16);
	Capture_preview_W = (WORD)(temp & 0x0000ffff);

    MP_ALERT("Capture_preview_H = %d ,Capture_preview_W =%d",Capture_preview_H, Capture_preview_W);
	...
	if(Setflag==0)
	{
			...
			}else if(sensor_mode==MODE_CAPTURE)
			{
                //VGA preview
   #if defined(SENSOR_TYPE_MAGIC_SENSOR)
				SensorInputWidth  = Capture_preview_W;
				SensorInputHeight = Capture_preview_H;
   #else
   				 //VGA preview
            	SensorInputWidth  = 640;
				SensorInputHeight = 480;
