...
*               : Read/Write/TotalSector/Lun etc map to DriverID(FileSystem) 
*               : Present/ReadOnly etc map to DeviceID(MCard)
******************************************************************************** 
*/
/*
...
void USBDeviceInfSetup( WHICH_OTG eWhichOtg )
{    
	PUSB_DEVICE_MSDC psUsbDevMsdc = (PUSB_DEVICE_MSDC)UsbOtgDevMsdcGet(eWhichOtg);
...
static WORD ReadDataFromDrive(USB_READ10 *pRead, BYTE lun, DWORD sent_data_length, WHICH_OTG eWhichOtg)
{
    ...
    bDeviceId = Mcard_CurLunGetCardID(lun);
    if ((bDeviceId != 0) && Mcard_GetFlagPresent(bDeviceId))
    {
        DWORD sector_read = 0;
...
#define mUsbEPinMxPtSz(EPn)		(*(&psUsbOtg->psUsbReg->DeviceInEndpoint1MaxPacketSize + (EPn - 1)) & 0x7ff)
///////////////////////////////////////////////////////////////////////////////
//      eUsbMsDataOut()
...
TRANSACTION_STATE eUsbMsDataOut(WORD u16FIFOByteCount, WHICH_OTG eWhichOtg)
{
	...
	if (u16FIFOByteCount == 0 || psUsbDevDesc->pbBulkTxRxData == 0 || psUsbDevMsdc->psCsw->u32DataResidue == 0) {
		mpDebugPrint("eOTGDataOut error1");
		while(1);
		return STATE_IDLE;
	}
    psUsbDevDesc->pbBulkTxRxData = (BYTE*)((DWORD)psUsbDevDesc->pbBulkTxRxData | 0xa0000000);

    bOTGCxFxWrRd(FOTG200_DMA2FIFO1,DIRECTION_OUT,(BYTE*)((DWORD)psUsbDevDesc->pbBulkTxRxData),u16FIFOByteCount,eWhichOtg);
	psUsbDevDesc->pbBulkTxRxData += u16FIFOByteCount;
	...
	if (psUsbDevMsdc->psCsw->u32DataResidue == 0)
		...
		// set stall
		mpDebugPrint("eOTGDataOut error2");
		while(1);
		return STATE_IDLE;
	}
	else
...
TRANSACTION_STATE eUsbMsDataIn(WHICH_OTG eWhichOtg)
{
	...
	psUsbDevDesc->pbBulkTxRxData = (BYTE*)((DWORD)psUsbDevDesc->pbBulkTxRxData | 0xa0000000);
	// Send u16count Bytes data to F0
	if (psUsbDevMsdc->psCsw->u32DataResidue > mUsbEPinMxPtSz(EP1)) // L
	{
		bOTGCxFxWrRd(FOTG200_DMA2FIFO0,DIRECTION_IN,(BYTE*)((DWORD)psUsbDevDesc->pbBulkTxRxData),mUsbEPinMxPtSz(EP1),eWhichOtg);
		psUsbDevDesc->pbBulkTxRxData += mUsbEPinMxPtSz(EP1);
		psUsbDevMsdc->psCsw->u32DataResidue -= mUsbEPinMxPtSz(EP1);
		return STATE_CB_DATA_IN;
	}
	else if (psUsbDevMsdc->psCsw->u32DataResidue != 0)
	{
		bOTGCxFxWrRd(FOTG200_DMA2FIFO0,DIRECTION_IN,(BYTE*)((DWORD)psUsbDevDesc->pbBulkTxRxData),psUsbDevMsdc->psCsw->u32DataResidue,eWhichOtg);
...
#define TOTAL_SECTORS (12*1024)
volatile unsigned char disk[TOTAL_SECTORS][BLOCK_LEN];
...
WORD Read10(BYTE** hData, USB_READ10 *pRead, BYTE lun, DWORD* pData_residue, DWORD expect_tx_length, WHICH_OTG eWhichOtg)
{
    ...
    *pData_residue = len;
	...
	//mpDebugPrint("Read10: lba %d cnt %x bl %x", lba, cnt, BLOCK_LEN);
...
WORD Write10(BYTE** hData, USB_WRITE10 *pWrite, BYTE lun, DWORD* pData_residue, DWORD expect_tx_length, WHICH_OTG eWhichOtg)
{
    ...
    *hData = &disk[lba][0];
	MP_DEBUG("*hData %x", *hData);
    len = cnt * BLOCK_LEN;
    *pData_residue = len;

	//mpDebugPrint("Write10: lba %d cnt %x bl %x", lba, cnt, BLOCK_LEN);
...
BYTE ReadCapacity( BYTE** hData, BYTE lun, DWORD* pData_residue, WHICH_OTG eWhichOtg)
{
	...
	psUsbDevMsdc->sMediaInfo.LastLBA= (TOTAL_SECTORS - 1);
...
WORD ReadFormatCapacities ( BYTE** hData, BYTE lun, DWORD* pData_residue, WHICH_OTG eWhichOtg)
{
	...
	psUsbDevMsdc->sCapacityDescriptor.NumOfBlock = TOTAL_SECTORS;     //0x4000 sectors;
	psUsbDevMsdc->sCapacityDescriptor.BlockLength = BLOCK_LEN;
	psUsbDevMsdc->sCapacityDescriptor.BlockLength |= DESCRIPTOR_CODE_DEFINITION << 24;//DESCRIPTOR_CODE_DEFINITION;
...
WORD ModeSense6 (BYTE** hData, BYTE lun, DWORD* pData_residue, WHICH_OTG eWhichOtg)
{
	...
	memset((BYTE*)&psUsbDevMsdc->sModeParaHdr6, 0, sizeof(psUsbDevMsdc->sModeParaHdr6));
    psUsbDevMsdc->sModeParaHdr6.ModeParaLen = sizeof(psUsbDevMsdc->sModeParaHdr6) - 1;
    ...
    *pData_residue = sizeof(psUsbDevMsdc->sModeParaHdr6);		
    return  STS_GOOD;
}
...
WORD ModeSense10 (BYTE** hData, USB_MODE_SENSE10 *pModeSense, BYTE lun, DWORD* pData_residue, WHICH_OTG eWhichOtg)
{
    ...
    WORD    modeParaLen;
    WORD    blkDscrptLen;
    MODE_PARA_HDR10 *pModeParaHdr10 = &psUsbDevMsdc->sModeParaHdr10;
    BYTE nFifoFlag ;
...
void USBCardStsByUI(BYTE bCardID, BYTE bCardIn, WHICH_OTG eWhichOtg)
{
	PUSB_DEVICE_MSDC psUsbDevMsdc = (PUSB_DEVICE_MSDC)UsbOtgDevMsdcGet(eWhichOtg);
    BYTE bLun = Mcard_CurCardIDGetLun(bCardID);
...
// Test Code for only run USB device using polling and use DDRRAM to be a storage for Main.c
// Call this function after calling SystemInit function in the Main function at Main.c
void UsbDeviceOnlyLoop(WHICH_OTG eWhichOtg)
{
    PST_USB_OTG_DES psUsbOtg = (PST_USB_OTG_DES)UsbOtgDesGet(eWhichOtg);
    volatile DWORD dwReg = 0;
    
    MP_ALERT("-USBOTG%d- %s Test Start.........", eWhichOtg, __FUNCTION__);
    
    mUsbOtgGlobIntDis(); 

    SystemMaxLunSet(1);  // One Virtual Slot 
    ...
    while(1)    
    {
        dwReg = mUsbOtgIntGroupRegRd() &~ mUsbOtgIntGroupMaskRd();  
        vOtgHandler(dwReg, eWhichOtg);       //  interrupt request handler 
    }
}
