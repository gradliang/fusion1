...
#define NETLINK_RTMGRP_PID 1111     /* RTMGRP from user space */
...
#else
	netlink->sock = lwip_socket(PF_NETLINK, SOCK_RAW, NETLINK_ROUTE);
#endif
	if (netlink->sock < 0) {
...
#else
	local.nl_pid = NETLINK_RTMGRP_PID;
	if (lwip_bind(netlink->sock, (struct sockaddr *) &local, sizeof(local)) < 0)
#endif
	{
