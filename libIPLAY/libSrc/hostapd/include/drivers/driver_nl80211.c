...
//    DBG("set valid_handler=%p", valid_handler);
	if (valid_handler)
	...
	while (err > 0)
    {
//        DBG("calls nl_recvmsgs(%d)", err, cnt++);
			      ...
			      void *valid_data)
{
	...
	return send_and_recv(drv->global, drv->global->nl, msg,
			     valid_handler, valid_data);
#else
	int r = send_and_recv(drv->global, drv->global->nl, msg,
			     valid_handler, valid_data);
    if (r < 0)
    {
        DBG("send_and_recv returns %d", r);
//        if (r != -EINVAL &&
//            r != -ENOENT && r != -EBUSY)
//            MP_ASSERT(0);
    }
...
#define NETLINK_REGULATORY_PID  20
	if (os_strcmp(group, "scan") == 0) {
        return NETLINK_SCAN_PID;
    }
    else if (os_strcmp(group, "mlme") == 0) {
        return NETLINK_MLME_PID;
    } else if (os_strcmp(group, "regulatory") == 0) {
        return NETLINK_REGULATORY_PID;
    } else {
        MP_ASSERT(0);
    }
#endif
					     ...
					     void *handle)
{
    ...
    DBG("sock=%d,taskid=%d", sock, TaskGetId());
...
static void nl80211_get_phy_name(struct wpa_driver_nl80211_data *drv)
{
	...
	if (pos)
		*pos = '\0';
#else
	strcpy(&drv->phyname[0], "ath9k_phy");
#endif
				      ...
				      void *global_priv)
{
    ...
    DBG("before wpa_driver_nl80211_init_nl");
    ...
    DBG("before nl80211_get_phy_name");
    ...
    DBG("before wpa_driver_nl80211_finish_drv_init");
	if (wpa_driver_nl80211_finish_drv_init(drv))
    {
        MP_ASSERT(0);
		goto failed;
    }
    DBG("after wpa_driver_nl80211_finish_drv_init");

#ifdef LINUX
	drv->eapol_tx_sock = socket(PF_PACKET, SOCK_DGRAM, 0);
#else
	drv->eapol_tx_sock = lwip_socket(PF_PACKET, SOCK_DGRAM, 0);
    DBG("drv->eapol_tx_sock=%x", drv->eapol_tx_sock);
#endif
	...
	if (drv->data_tx_status) {
		...
		if (lwip_setsockopt(drv->eapol_tx_sock, SOL_SOCKET, SO_WIFI_STATUS,
...
wpa_driver_nl80211_finish_drv_init(struct wpa_driver_nl80211_data *drv)
{
    ...
    DBG("before wpa_driver_nl80211_capa");
				     ...
				     const u8 *addr, int wds)
{
    ...
    DBG("ifindex=%d,ifname=%s", drv->ifindex, ifname);
...
#else
	len = recv(sock | (TaskGetId() << 8), buf, sizeof(buf), 0);
#endif
	if (len < 0) {
		perror("recv");
		return;
...
nl80211_create_monitor_interface(struct wpa_driver_nl80211_data *drv)
{
    ...
    DBG("calls nl80211_create_iface");
	drv->monitor_ifidx =
		nl80211_create_iface(drv, buf, NL80211_IFTYPE_MONITOR, NULL,
				     0);
    DBG("drv->monitor_ifidx=%d", drv->monitor_ifidx);
...
#else
	drv->monitor_sock = lwip_socket(PF_PACKET, SOCK_RAW, htons(ETH_P_ALL));
#endif
	if (drv->monitor_sock < 0) {
		perror("socket[PF_PACKET,SOCK_RAW]");
		goto error;
	}
    DBG("drv->monitor_sock=%x", drv->monitor_sock);
...
#else
	len = recvfrom(sock | (TaskGetId() << 8), buf, sizeof(buf), 0,
		       (struct sockaddr *)&lladdr, &fromlen);
#endif
	if (len < 0) {
		perror("recv");
        ...
        struct wpa_init_params *params)
{
	...
	drv->if_indices = drv->default_if_indices;
    DBG("drv->num_if_indices=%d", drv->num_if_indices);
#ifdef LINUX
...
#else
	drv->eapol_sock = lwip_socket(PF_PACKET, SOCK_DGRAM, htons(ETH_P_PAE));
    DBG("drv->eapol_sock=%x", drv->eapol_sock);
#endif
	if (drv->eapol_sock < 0) {
...
struct nl80211_global *g_global;
static void * nl80211_global_init(void)
{
    ...
    DBG("calls wpa_driver_nl80211_init_nl_global");
...
#else
	global->ioctl_sock = lwip_socket(PF_INET, SOCK_DGRAM, 0);
    DBG("global->ioctl_sock=%x", global->ioctl_sock);
#endif
...
static int wpa_driver_nl80211_get_survey(void *priv, unsigned int freq)
{
	...
	genlmsg_put(msg, 0, 0, genl_family_get_id(drv->global->nl80211_id), 0, NLM_F_DUMP,
