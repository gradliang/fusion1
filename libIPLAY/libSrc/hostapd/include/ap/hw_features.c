...
#define ieee802_11_parse_elems(a,b,c,d) hapd_ieee802_11_parse_elems(a,b,c,d)
#endif
...
static int hostapd_check_chans(struct hostapd_iface *iface)
{
	...
	if (iface->chans_surveyed) {
		mpDebugPrint("ACS: no usable channels found");
		return 0;
	}

	if (!(iface->drv_flags & WPA_DRIVER_FLAGS_OFFCHANNEL_TX)) {
		mpDebugPrint("ACS: offchannel TX support required");
		return 0;
	}

	mpDebugPrint("ACS: automatic channel selection started...");
...
int hostapd_hw_get_freq(struct hostapd_data *hapd, int chan)
{
    ...
    DBG("hapd->iface->current_mode=%d", hapd->iface->current_mode);
