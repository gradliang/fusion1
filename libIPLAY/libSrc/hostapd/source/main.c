...
#include "log.h"
#define printf(fmt, ...)  mpDebugPrint(fmt, ##__VA_ARGS__)
#endif
...
#endif
{
    ...
    DBG("before hostapd_global_ctrl_iface_init");
 ...
 out:
    DBG("before hostapd_global_ctrl_iface_deinit");
    ...
    DBG("before hostapd_global_deinit");
...
#ifdef PLATFORM_MPIXEL
void m_nl_debug_init(int _debug);
int m_task_socket_data_setup(int task_id);
int hostapd_queue_global_lock;
void hostapd_main(void)
{
    char *argv[2];

    m_hostapd_init();

    /* init some userspace environment */
    m_task_socket_data_setup(TaskGetId());
    init_msg_size();
    m_nl_debug_init(0);
...
wait_for_driver:
    ...
    argv[0] = "hostapd";
#if 0
    if (dev_get_by_index(0, 1))
#else
    if (1)
#endif
    {
        argv[1] = "hostapd.conf";               /* config file name */
        main(2, argv);
    }
    else
        main(1, argv);

    goto wait_for_driver;
}
#endif
