...
struct hostapd_config *g_conf;
extern int my_channel;
extern char *my_ssid;
#endif
...
struct hostapd_config * hostapd_config_read(const char *fname)
{
    ...
    os_strlcpy(conf->bss[0].iface, "wlan0", sizeof(conf->bss[0].iface));

    conf->channel = 1;
    if (my_channel > 0)
        conf->channel = my_channel;

	bss = conf->last_bss = conf->bss;

    sprintf(bss->ssid.ssid, "magic-" COMPACT_MACSTR, MAC2STR(myethaddr));
    if (strlen(my_ssid) > 0)
        strcpy(bss->ssid.ssid, my_ssid);

    bss->ssid.ssid_len = os_strlen(bss->ssid.ssid);
    bss->ssid.ssid_set = 1;

    bss->isolate = 0;
    bss->wmm_enabled = 0; /* disable wmm */
//    bss->ap_max_inactivity = atoi(pos);  
    os_memcpy(conf->country, "US", 2);
    conf->country[2] = ' ';
    bss->max_num_sta = 16;
    conf->hw_mode = HOSTAPD_MODE_IEEE80211G;
    conf->ap_table_max_size = 0;
    conf->ieee80211n = 1;       /* enable 11n */
#ifdef LINUX
    if (hostapd_config_ht_capab(conf, "[HT40-][SHORT-GI-40][DSSS_CCK-40]") < 0) 
#else
    if (hostapd_config_ht_capab(conf, "[SHORT-GI-40][DSSS_CCK-40]") < 0) 
#endif
    {
        wpa_printf(MSG_ERROR, "Line %d: invalid "
                "ht_capab", line);
...
#ifdef HAVE_ACS
int hostapd_set_channel(unsigned int channel)
{
    g_conf->channel = my_channel = channel;
	return 0;
}
#endif
