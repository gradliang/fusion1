...
#        ifdef DEBUG
#            define dprintf(fmt,...) MP_DPF(fmt, __VA_ARGS__)
#        else
#            define dprintf(fmt,...)
#        endif
...
typedef void (*WriteDataFunc)(void *, BYTE *, int);
...
typedef struct PutBitContext {
    ...
    uint32_t bit_buf;
    int bit_left;
    BYTE *buf, *buf_ptr, *buf_end;
#endif
} PutBitContext;

//void init_put_bits(PutBitContext *s, BYTE *buffer, int buffer_size);
...
static inline void put_bits(PutBitContext *s, int n, unsigned int value)
{
    ...
    //    MP_DPF("n=%d value=%x cnt=%d buf=%x\n", n, value, bit_cnt, bit_buf);
    /* XXX: optimize */
    if (n < bit_left) {
...
#        else
    int index= s->index;
    uint32_t *ptr= (uint32_t*)(((BYTE *)s->buf)+(index>>3));
...
static inline BYTE* pbBufPtr(PutBitContext *s)
{
#ifdef ALT_BITSTREAM_WRITER
...
#   define UPDATE_CACHE(name, gb)\
        name##_cache= unaligned32_be( ((BYTE *)(gb)->buffer)+(name##_index>>3) ) << (name##_index&0x07);\
...
#   define OPEN_READER(name, gb)\
        ...
        BYTE * name##_buffer_ptr=(gb)->buffer_ptr;\
...
static inline int get_bits_count(GetBitContext *s){
    return ((BYTE*)s->buffer_ptr - s->buffer)*8 - 32 + s->bit_count;
}
...
//#if (CHIP_VER == CHIP_VER_615)
    MPV * mpv=(MPV *)MPV_BASE;
    mpv->VldSysMReg = n - 1;
    return mpv->VldRdFlushFlcReg;
#else
    register int tmp;
    OPEN_READER(re, s)
    UPDATE_CACHE(re, s)
...
//#if (CHIP_VER == CHIP_VER_615)
    ...
    BYTE result;
    int index= s->index;
    // MP_DPF("get_bits1 index = %d\n",index);
    result= s->buffer[ index>>3 ];
    // MP_DPF("get_bits1 result = %d\n",(unsigned int)result);
    result<<= (index&0x07);
    // MP_DPF("get_bits1 result = %d\n",(unsigned int)result);
    result>>= 8 - 1;
    // MP_DPF("get_bits1 result = %d\n",(unsigned int)result);
...
void init_get_bits(GetBitContext *s,
                   const BYTE *buffer, int buffer_size);
...
static inline int get_bits_trace(GetBitContext *s, int n, char *file, char *func, int line){
    ...
    print_bin(r, n);
    MP_DPF("%5d %2d %3d bit @%5d in %s %s:%d\n", r, n, r, get_bits_count(s)-n, file, func, line);
    return r;
}
static inline int get_vlc_trace(GetBitContext *s, VLC_TYPE (*table)[2], int bits, int max_depth, char *file, char *func, int line){
    ...
    MP_DPF("%5d %2d %3d vlc @%5d in %s %s:%d\n", bits2, len, r, pos, file, func, line);
    return r;
}
static inline int get_xbits_trace(GetBitContext *s, int n, char *file, char *func, int line){
    ...
    print_bin(show, n);
    MP_DPF("%5d %2d %3d xbt @%5d in %s %s:%d\n", show, n, r, get_bits_count(s)-n, file, func, line);
    return r;
}
...
/* misc math functions */
/*
extern const BYTE ff_log2_tab[256];
...
/* math */
//extern const BYTE ff_sqrt_tab[128];
...
if(256*256*256*64%(tcount+tskip_count)==0){\
    MP_FDPF(stderr, "%Ld dezicycles in %s, %d runs, %d skips\n", tsum*10/tcount, id, tcount, tskip_count);\
}
#endif
