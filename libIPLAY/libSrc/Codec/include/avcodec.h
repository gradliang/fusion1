...
#define MB_WIDTH_E 		0
#define MB_HEIGHT_E 	8
#define MPEG4_FILE_B  	16
...
static const int Motion_Est_QTab[] =
...
typedef struct AVCodec {
    ...
    int (*encode)(AVCodecContext *, BYTE *buf, int buf_size, void *data);
...
int avpicture_fill(AVPicture *picture, BYTE *ptr,
...
int avcodec_decode_video(AVCodecContext *avctx, AVFrame *picture, 
                         int *got_picture_ptr,
                         BYTE *buf, int buf_size);
int avcodec_parse_frame(AVCodecContext *avctx, BYTE **pdata, 
                        int *data_size_ptr,
                        BYTE *buf, int buf_size);
//int avcodec_encode_audio(AVCodecContext *avctx, BYTE *buf, int buf_size, 
//                         const short *samples);
//int avcodec_encode_video(AVCodecContext *avctx, BYTE *buf, int buf_size, 
//                         const AVFrame *pict);
...
typedef enum {
    ...
    AVC_FLUSH,
    // pin - struct { BYTE* src, uint_t src_size }
    // pout - struct { AVPicture* img, consumed_bytes,
    AVC_DECODE,
    // pin - struct { AVPicture* img, BYTE* dest, uint_t dest_size }
...
typedef struct AVCodecParser {
    ...
    int (*parser_parse)(AVCodecParserContext *s, 
                        ...
                        BYTE **poutbuf, int *poutbuf_size, 
                        const BYTE *buf, int buf_size);
...
//align malloc 1024 bytes.
void *align_malloc(unsigned int size);
...
extern void av_log(AVCodecContext*, int level, const char *fmt, ...);
...
#define MP_DPF DO NOT USE
#define MP_FDPF DO NOT USE
#undef stderr
