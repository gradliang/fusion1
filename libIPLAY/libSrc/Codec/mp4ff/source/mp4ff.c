...
{
    mp4ff_t *ff = mp4ff_malloc(sizeof(mp4ff_t));
    ...
    for (i = 0; i < ff->total_tracks; i++)
    {
        if (ff->track[i])
        {
            if (ff->track[i]->stsz_table)
                mp4ff_free(ff->track[i]->stsz_table);
            if (ff->track[i]->stts_sample_count)
                mp4ff_free(ff->track[i]->stts_sample_count);
            if (ff->track[i]->stts_sample_delta)
                mp4ff_free(ff->track[i]->stts_sample_delta);
            if (ff->track[i]->stsc_first_chunk)
                mp4ff_free(ff->track[i]->stsc_first_chunk);
            if (ff->track[i]->stsc_samples_per_chunk)
                mp4ff_free(ff->track[i]->stsc_samples_per_chunk);
            if (ff->track[i]->stsc_sample_desc_index)
                mp4ff_free(ff->track[i]->stsc_sample_desc_index);
            if (ff->track[i]->stco_chunk_offset)
                mp4ff_free(ff->track[i]->stco_chunk_offset);
            if (ff->track[i]->decoderConfig)
                mp4ff_free(ff->track[i]->decoderConfig);
			if (ff->track[i]->ctts_sample_count)
				mp4ff_free(ff->track[i]->ctts_sample_count);
			if (ff->track[i]->ctts_sample_offset)
				mp4ff_free(ff->track[i]->ctts_sample_offset);
#ifdef ITUNES_DRM
            if (ff->track[i]->p_drms)
                drms_free(ff->track[i]->p_drms);
#endif
            mp4ff_free(ff->track[i]);
        }
    }
...
static void mp4ff_track_add(mp4ff_t *f)
{
    ...
    f->track[f->total_tracks - 1] = mp4ff_malloc(sizeof(mp4ff_track_t));
                                 ...
                                 uint8_t** ppBuf, uint32_t* pBufSize)
{
    ...
    if (f->track[track]->decoderConfig == NULL || f->track[track]->decoderConfigLen == 0)
    {
        ...
        *ppBuf = mp4ff_malloc(f->track[track]->decoderConfigLen);
        if (*ppBuf == NULL)
        {
            *pBufSize = 0;
            return 1;
                          ...
                          uint8_t **audio_buffer,  uint32_t *bytes)
{
    ...
    *audio_buffer = (uint8_t*)mp4ff_malloc(*bytes);
