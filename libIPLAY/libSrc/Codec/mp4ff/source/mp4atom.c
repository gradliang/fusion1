...
static int32_t mp4ff_read_stsz(mp4ff_t *f)
{
    ...
    if (f->track[f->total_tracks - 1]->stsz_sample_size == 0)
    {
        ...
        f->track[f->total_tracks - 1]->stsz_table =
            (int32_t*)mp4ff_malloc(f->track[f->total_tracks - 1]->stsz_sample_count*sizeof(int32_t));
...
static int32_t mp4ff_read_esds(mp4ff_t *f)
{
    ...
    if (f->track[f->total_tracks - 1]->decoderConfig)
        mp4ff_free(f->track[f->total_tracks - 1]->decoderConfig);
    f->track[f->total_tracks - 1]->decoderConfig = mp4ff_malloc(f->track[f->total_tracks - 1]->decoderConfigLen);
    if (f->track[f->total_tracks - 1]->decoderConfig)
    {
...
#   define SHGFP_TYPE_CURRENT 0
    ...
    if( ( shfolder_dll = LoadLibrary( _T("SHFolder.dll") ) ) != NULL )
    {
        ...
        {
            p_homedir = (char *)mp4ff_malloc( MAX_PATH );
            if( !p_homedir )
            {
                return NULL;
            }
...
{
    uint8_t *data = mp4ff_malloc(size);
...
static int32_t mp4ff_read_stsc(mp4ff_t *f)
{
    ...
    f->track[f->total_tracks - 1]->stsc_first_chunk =
        (int32_t*)mp4ff_malloc(f->track[f->total_tracks - 1]->stsc_entry_count*sizeof(int32_t));
...
static int32_t mp4ff_read_stco(mp4ff_t *f)
{
    ...
    f->track[f->total_tracks - 1]->stco_chunk_offset =
        (int32_t*)mp4ff_malloc(f->track[f->total_tracks - 1]->stco_entry_count*sizeof(int32_t));
...
static int32_t mp4ff_read_ctts(mp4ff_t *f)
{
    ...
    p_track->ctts_sample_count = (int32_t*)mp4ff_malloc(p_track->ctts_entry_count * sizeof(int32_t));
    p_track->ctts_sample_offset = (int32_t*)mp4ff_malloc(p_track->ctts_entry_count * sizeof(int32_t));

    if (p_track->ctts_sample_count == 0 || p_track->ctts_sample_offset == 0)
    {
        if (p_track->ctts_sample_count) {mp4ff_free(p_track->ctts_sample_count);p_track->ctts_sample_count=0;}
        if (p_track->ctts_sample_offset) {mp4ff_free(p_track->ctts_sample_offset);p_track->ctts_sample_offset=0;}
...
static int32_t mp4ff_read_stts(mp4ff_t *f)
{
    ...
    p_track->stts_sample_count = (int32_t*)mp4ff_malloc(p_track->stts_entry_count * sizeof(int32_t));
    p_track->stts_sample_delta = (int32_t*)mp4ff_malloc(p_track->stts_entry_count * sizeof(int32_t));

    if (p_track->stts_sample_count == 0 || p_track->stts_sample_delta == 0)
    {
        if (p_track->stts_sample_count) {mp4ff_free(p_track->stts_sample_count);p_track->stts_sample_count=0;}
        if (p_track->stts_sample_delta) {mp4ff_free(p_track->stts_sample_delta);p_track->stts_sample_delta=0;}
