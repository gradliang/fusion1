...
//Temporarily used by C.W 080912
#define codec_mem_malloc(a)			mem_malloc(a)
#define codec_mem_free(a)			mem_free(a)
#define codec_mem_reallocm(a, b)	mem_reallocm((a), (b))

#if 0	//Mark by C.W 081014
void *mp4ff_malloc(uint32_t size)
{
    return (void *)codec_mem_malloc(size);
}
...
{
    return (void *)codec_mem_reallocm(b, newsize);
}
#endif
...
{
	char * str = (char*)mp4ff_malloc(length + 1);
	if (str!=0)
	{
