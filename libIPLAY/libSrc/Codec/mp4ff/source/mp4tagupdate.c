...
unsigned membuffer_write(membuffer * buf,const void * ptr,unsigned bytes)
{
	...
	if (dest_size > buf->allocated)
	{
		...
		{
			void * newptr = mp4ff_realloc(buf->data,buf->allocated);
			if (newptr==0)
			{
				mp4ff_free(buf->data);
				buf->data = 0;
...
membuffer * membuffer_create()
{
	...
	membuffer * buf = (membuffer *) mp4ff_malloc(sizeof(membuffer));
	buf->data = mp4ff_malloc(initial_size);
	buf->written = 0;
...
{
	if (buf->data) mp4ff_free(buf->data);
	mp4ff_free(buf);
}
...
void * membuffer_detach(membuffer * buf)
{
	...
	ret = mp4ff_realloc(buf->data,buf->written);
	
	if (ret == 0) mp4ff_free(buf->data);
...
static uint32_t create_ilst(const mp4ff_metadata_t * data,void ** out_buffer,uint32_t * out_size)
{
	...
	unsigned metaptr;
	char * mask = (char*)mp4ff_malloc(data->count);
...
static uint32_t modify_moov(mp4ff_t * f,const mp4ff_metadata_t * data,void ** out_buffer,uint32_t * out_size)
{
	...
	else
	{
		...
		*out_buffer = mp4ff_malloc(*out_size);
		if (*out_buffer == 0)
		{
			mp4ff_free(new_ilst_buffer);
			return 0;
		}
