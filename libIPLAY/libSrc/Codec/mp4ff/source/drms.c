...
void *drms_alloc( char *psz_homedir )
{
    ...
    p_drms = mp4ff_malloc( sizeof(struct drms_s) );
...
#define ISCINFO "iSCInfo"
    ...
    if( file != NULL )
    {
        ...
        {
            *pp_sci = mp4ff_malloc( st.st_size );
            if( *pp_sci != NULL )
            {
