...
static int32_t mp4ff_tag_add_field(mp4ff_metadata_t *tags, const char *item, const char *value)
{
    ...
    tags->tags = (mp4ff_tag_t*)mp4ff_realloc(tags->tags, (tags->count+1) * sizeof(mp4ff_tag_t));
    if (!tags->tags)
    {
        if (backup) mp4ff_free(backup);
        return 0;
    } else {
...
int32_t mp4ff_tag_delete(mp4ff_metadata_t *tags)
{
    ...
    for (i = 0; i < tags->count; i++)
    {
        if (tags->tags[i].item) mp4ff_free(tags->tags[i].item);
        if (tags->tags[i].value) mp4ff_free(tags->tags[i].value);
    }

    if (tags->tags) mp4ff_free(tags->tags);
    ...
    while (sumsize < size)
    {
		...
		if (!done)
		{
			if (atom_type == ATOM_DATA)
			{
				...
				{
					if (data) {mp4ff_free(data);data = NULL;}
