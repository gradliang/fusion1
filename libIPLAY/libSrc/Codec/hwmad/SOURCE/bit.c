 ...
 */
void mad_bit_write(struct mad_bitptr *bitptr, unsigned int len, unsigned long value)
{
	unsigned char *ptr;
 ...
 * DESCRIPTION:	read an arbitrary number of bits and return their UIMSBF value
 */
unsigned long mad_bit_read(register struct mad_bitptr *bitptr, register unsigned int len)
{
	register unsigned long value;
	register unsigned short bitLeft = bitptr->left;
	register unsigned short bitCache;

	//if (bitLeft == CHAR_BIT)
	bitCache = *bitptr->byte;

	if (len == bitLeft) {
		value = (bitCache & ((1 << bitLeft) - 1));
		bitptr->byte++;
		bitptr->left = CHAR_BIT;
		return value;
	} else if (len < bitLeft) {
	...
	if (len > 0) {
		...
		value = (value << len) | (bitCache >> (CHAR_BIT - len));
		bitLeft -= len;
	}
	bitptr->left = bitLeft;
	return value;
 ...
 */
unsigned short mad_bit_crc2(struct mad_bitptr bitptr, unsigned int len, unsigned short init)
{
