...
static decode_func_t *const decoder_table[3] = {
	mad_layer_I,
	mad_layer_II,
	mad_layer_III
};
...
static int decode_header(struct mad_header *header, struct mad_stream *stream)
{
	...
	if (mad_bit_read(&stream->ptr, 1) == 0)
	{
		...
		header->crc_check = mad_bit_crc2(stream->ptr, 16, 0xffff);
	}

	/* bitrate_index */
	index = mad_bit_read(&stream->ptr, 4);
//  header->sample_freq = index;
...
static int free_bitrate(struct mad_stream *stream, struct mad_header const *header)
{
	...
	while (mad_stream_sync(stream) == 0)
	{
		...
		if (decode_header(&peek_header, &peek_stream) == 0 &&
			peek_header.layer == header->layer && peek_header.samplerate == header->samplerate)
		{
			unsigned int N;
 ...
 */
int mad_frame_decode(struct mad_frame *frame, struct mad_stream *stream, unsigned char *buf)
{
	decode_func_t *decode_func = NULL;
	...
	if (1 <= frame->header.layer && frame->header.layer <= 3)
		decode_func = decoder_table[frame->header.layer - 1];
//		MP_DEBUG("decode_func =mad_layer_%d",frame->header.layer);
	if (decode_func && decode_func(stream, frame, buf) == -1)
	{
