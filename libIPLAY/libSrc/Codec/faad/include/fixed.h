...
#elif defined(__mips_mp600__)

/* taken from MAD */
#define mips_mul(x, y, SCALEBITS)  \
    ({ register int32_t __hi;  \
       register uint32_t __lo;  \
       __asm__ ("mult	%2,%3"  \
        : "=l" (__lo), "=h" (__hi)  \
        : "%r" (x), "r" (y));  \
       ((real_t)  \
...
#define mips_mul_opt_speed(x, y, SCALEBITS)  \
    ({ register int32_t __result;  \
       __asm__ ("smulwhh	%1,%2\n\t"  \
                "mhld	%0,0x2000"  \
        : "=&r" (__result)  \
        : "%r" (x), "r" (y));  \
       ((real_t) ((__result) << (31 - SCALEBITS)));  \
    })

static INLINE real_t MUL_R(real_t A, real_t B)
{
    return mips_mul(A, B, REAL_BITS);
}

static INLINE real_t MUL_C(real_t A, real_t B)
{
//    return mips_mul_opt_speed(A, B, COEF_BITS);
    return mips_mul(A, B, COEF_BITS);	//C.W temporarily using
}

static INLINE real_t MUL_Q2(real_t A, real_t B)
{
    return mips_mul(A, B, Q2_BITS);
}

static INLINE real_t MUL_SHIFT6(real_t A, real_t B)
{
    return mips_mul(A, B, 6);
}

static INLINE real_t MUL_SHIFT23(real_t A, real_t B)
{
    return mips_mul(A, B, 23);
}

static INLINE real_t MUL_F(real_t A, real_t B)
{
//    return mips_mul_opt_speed(A, B, FRAC_BITS);
    return mips_mul(A, B, FRAC_BITS);	//C.W temporarily using
}
...
static INLINE void ComplexMult(real_t *y1, real_t *y2,
    real_t x1, real_t x2, real_t c1, real_t c2)
{
//Not support in mp615 case
/*
    register int32_t yt1, yt2;
    __asm__ ("smulwhh	%3,%5\n\t"
             "smlawhh	%4,%6\n\t"
             "mhld	%0,0x2000\n\t"
             "subu	%2,$0,%3\n\t"
