                    ...
                    bitfile *ld, program_config *pce, drc_info *drc)
{
    ...
    if (hDecoder->object_type < ER_OBJECT_START)
    {
        ...
        {
			//mpDebugPrint("ID Sync ele: %x, Error code: %d", id_syn_ele, hInfo->error);
...
//    ALIGN int16_t spec_data[1024] = {0};
    ...
    if (!hDecoder->skip_frame)
    {
        faad_decode_sram_init();
...
static uint16_t extension_payload(bitfile *ld, drc_info *drc, uint16_t count)
{
    ...
    case EXT_DYNAMIC_RANGE:
        ...
        return n;
    case EXT_FILL_DATA:
        /* fill_nibble = */ faad_flushbits(ld, 4
            DEBUGVAR(1,136,"extension_payload(): fill_nibble")); /* must be ?000?*/
        for (i = 0; i < count-1; i++)
        {
            /* fill_byte[i] = */ faad_flushbits(ld, 8
    ...
    case EXT_DATA_ELEMENT:
        ...
        case ANC_DATA:
            ...
            {
                /* data_element_byte[i] = */ faad_flushbits(ld, 8
    ...
    case EXT_FIL:
    default:
        faad_flushbits(ld, align
            DEBUGVAR(1,88,"extension_payload(): fill_nibble"));
        for (i = 0; i < count-1; i++)
        {
            /* other_bits[i] = */ faad_flushbits(ld, 8
...
static uint8_t dynamic_range_info(bitfile *ld, drc_info *drc)
{
        ...
        DEBUGVAR(1,90,"dynamic_range_info(): has instance_tag")) & 1)
    {
        ...
        /* drc->drc_tag_reserved_bits = */ faad_flushbits(ld, 4
        ...
        DEBUGVAR(1,94,"dynamic_range_info(): has bands data")) & 1)
    {
        ...
        /* drc->drc_bands_reserved_bits = */ faad_flushbits(ld, 4
...
void get_adif_header(adif_header *adif, bitfile *ld)
{
    ...
    /* adif_id[0] = */ faad_flushbits(ld, 8
        DEBUGVAR(1,106,"get_adif_header(): adif_id[0]"));
    /* adif_id[1] = */ faad_flushbits(ld, 8
        DEBUGVAR(1,107,"get_adif_header(): adif_id[1]"));
    /* adif_id[2] = */ faad_flushbits(ld, 8
        DEBUGVAR(1,108,"get_adif_header(): adif_id[2]"));
    /* adif_id[3] = */ faad_flushbits(ld, 8
...
static uint8_t adts_fixed_header(adts_header *adts, bitfile *ld)
{
    ...
    if (i != 0)
        MagicPixel_AAC_FAAD_songinfo_callback(ld->buffer, i, 0);
