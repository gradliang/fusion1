                              ...
                              void **sample_buffer2, uint32_t sample_buffer_size)
{
    ...
    {
		mpDebugPrint("aac decode error Input NULL");
        return NULL;
    }
    ...
    if (hDecoder->adts_header_present)
    {
        ...
        if (hDecoder->skip_frame)
        {
            hInfo->bytesconsumed = adts.aac_frame_length;
        ...
        (hDecoder->alloced_channels != output_channels))
    {
        ...
        if (sample_buffer_size == 0)
        {
            ...
            hDecoder->sample_buffer = NULL;
            hDecoder->sample_buffer = (void *)faad_malloc(frame_len*output_channels*stride);
...
error:
    ...
    bitsconsumed = faad_get_processed_bits(&ld);
    hInfo->bytesconsumed = bit2byte(bitsconsumed);
    MagicPixel_AAC_FAAD_songinfo_callback(ld.buffer, hInfo->bytesconsumed, 1);
    faad_endbits(&ld);

mpDebugPrint("aac decode error: %d, byte consume: %d!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!", hInfo->error, bitsconsumed);
