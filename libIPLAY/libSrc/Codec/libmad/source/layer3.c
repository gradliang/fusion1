                                  ...
                                  struct channel *gr1ch, int mode_extension)
{
    ...
    register unsigned char *ch_scalefac = channel->scalefac;
    start = *ptr;
    ...
    if (!((mode_extension & I_STEREO) && gr1ch))
    {
        if (scalefac_compress < 400)
        {
            ...
            slen[2] = (scalefac_compress & 0xf) >> 2;   // % 16
        ...
        for (part = 0; part < 4; ++part)
        {
            for (i = 0; i < nsfb[part]; ++i)
                ch_scalefac[n++] = mad_bit_read(ptr, slen[part]);
        }
    ...
    {  /* (mode_extension & I_STEREO) && gr1ch (i.e. ch == 1) */
        ...
        else if (scalefac_compress < 244)
        {
            ...
            slen[0] = (scalefac_compress & 0x3f) >> 4;
            slen[1] = (scalefac_compress & 0xf) >> 2;
                              ...
                              struct channel const *gr0ch, unsigned int scfsi)
{
    ...
    register unsigned char *fac = channel->scalefac;
    ...
    if (channel->block_type == 2)
    {
        ...
        while (nsfb--)
            fac[sfbi++] = mad_bit_read(ptr, slen1);

        nsfb = 6 * 3;
        while (nsfb--)
            fac[sfbi++] = mad_bit_read(ptr, slen2);
                   ...
                   unsigned char const *sfbwidth, signed int exponents[39])
{
    ...
    if (channel->block_type == 2)
    {
        ...
        if (channel->flags & mixed_block_flag)
        {
            ...
            {
                exponents[sfbi] = gain -
                                  (signed int) ((ch_scalefac[sfbi] + (pretab[sfbi] & premask)) <<
                                                scalefac_multiplier);
        ...
        while (l < 576)
        {
            exponents[sfbi + 0] = gain0 -
                                  (signed int) (ch_scalefac[sfbi + 0] << scalefac_multiplier);
            exponents[sfbi + 1] = gain1 -
                                  (signed int) (ch_scalefac[sfbi + 1] << scalefac_multiplier);
            exponents[sfbi + 2] = gain2 -
                                  (signed int) (ch_scalefac[sfbi + 2] << scalefac_multiplier);
    ...
    {  /* channel->block_type != 2 */
        if (channel->flags & preflag)
        {
            ...
            {
                exponents[sfbi] = gain -
                                  (signed int) ((ch_scalefac[sfbi] + pretab[sfbi]) <<
                                                scalefac_multiplier);
            }
        }
        else
        {
            ...
            {
                exponents[sfbi] = gain -
                                  (signed int) (ch_scalefac[sfbi] << scalefac_multiplier);
            }
        }
    }
}
...
mad_fixed_t III_requantize(unsigned int value, signed int exp)
{
    ...
    frac = exp & 3;  /* assumes sign(frac) == sign(exp) */
    exp >>= 2;
            ...
            for (sfbi = l = 0; l < 576; ++sfbi, l += n)
            {
                ...
                for (i = 0; i < n; ++i)
                {
                    ...
                    else
                    {
                        ...
                        opposite = mad_f_mul(left, lsf_scale[(is_pos - 1) >> 1]);
...
#  if 1
static
void fastsdct(register mad_fixed_t const x[9], mad_fixed_t y[18])
{
...
void dctIV(mad_fixed_t const y[18], mad_fixed_t X[18])
{
    ...
    for (i = 1; i < 17; i += 4)
    {
        X[i + 0] = (X[i + 0] >> 1) - X[(i + 0) - 1];
        X[i + 1] = (X[i + 1] >> 1) - X[(i + 1) - 1];
        X[i + 2] = (X[i + 2] >> 1) - X[(i + 2) - 1];
 ...
 */
static mad_fixed_t overlap[2 * 32 * 18];
int mad_layer_III(struct mad_stream *stream, struct mad_frame *frame)
{
    ...
    {
        stream->main_data = libmad_malloc(MAD_BUFFER_MDLEN);
        if (stream->main_data == 0)
        {
    ...
    if (frame->overlap == 0)
    {
        frame->overlap = &overlap[0];
        //frame->overlap = libmad_malloc(2 * 32 * 18 * sizeof(mad_fixed_t));
        if (frame->overlap == 0)
        {
            stream->error = MAD_ERROR_NOMEM;
            return -1;
