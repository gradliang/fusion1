...
#define NET_SEM_ID  28
#define pthread_mutex_lock(mutex)   SemaphoreWait(NET_SEM_ID)
#define pthread_mutex_unlock(mutex)   SemaphoreRelease(NET_SEM_ID)
#endif
...
#else
	sk->s_seq_expect = sk->s_seq_next = mpx_time(0);
#endif
