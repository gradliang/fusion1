...
#else
extern void  mpDebugPrint(const char *fmt, ...);
#define NL_DBG(LVL,FMT,ARG...) \
	do {	\
		if (LVL <= nl_debug) \
			mpDebugPrint("DBG<" #LVL ">: " FMT, ##ARG); \
	} while (0)
#endif
...
#ifdef PLATFORM_MPIXEL
#define pthread_mutex_lock(mutex)   SemaphoreWait(mutex)
#define pthread_mutex_unlock(mutex)   SemaphoreRelease(mutex)
#endif
#endif
