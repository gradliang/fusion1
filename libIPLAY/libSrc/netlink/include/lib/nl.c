...
int nl_connect(struct nl_sock *sk, int protocol)
{
...
#else
	sk->s_fd = lwip_socket(AF_NETLINK, SOCK_RAW | flags, protocol);
#endif
	if (sk->s_fd < 0) {
	...
	err = bind(sk->s_fd, (struct sockaddr*) &sk->s_local,
		   sizeof(sk->s_local));
#else
	err = lwip_bind(sk->s_fd, (struct sockaddr*) &sk->s_local,
		   sizeof(sk->s_local));
#endif
	if (err < 0) {
		err = -nl_syserr2nlerr(errno);
	...
	err = getsockname(sk->s_fd, (struct sockaddr *) &sk->s_local,
			  &addrlen);
#else
	err = lwip_getsockname(sk->s_fd, (struct sockaddr *) &sk->s_local,
			  &addrlen);
#endif
	if (err < 0) {
        MP_ASSERT(0);
...
continue_reading:
	...
	while (nlmsg_ok(hdr, n)) {
		...
		if (hdr->nlmsg_flags & NLM_F_ACK) {
            DBG("NLM_F_ACK message %p", cb->cb_set[NL_CB_VALID]);
		...
		if (hdr->nlmsg_type == NLMSG_DONE) {
            DBG("NLMSG_DONE message %p", cb->cb_set[NL_CB_VALID]);
			multipart = 0;
		 ...
		 * NL_PROCEED. */
            ...
            DBG("NLMSG_NOOP message %p", cb->cb_set[NL_CB_VALID]);
		 ...
		 * NL_SKIP or NL_PROCEED (dangerous) */
            ...
            DBG("NLMSG_OVERRUN message %p", cb->cb_set[NL_CB_VALID]);
		...
		/* Message carries a nlmsgerr */
            ...
            DBG("NLMSG_ERROR message %p", cb->cb_set[NL_CB_VALID]);
...
#ifdef PLATFORM_MPIXEL
extern void m_global_nl_set_kernel(int fd, __u32 pid);
void m_global_nl_set(struct nl_sock *nl)
{
    m_global_nl_set_kernel(nl->s_fd, nl->s_local.nl_pid);
}
