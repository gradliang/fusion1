...
#define EAP_MAX_AUTH_ROUNDS 500 //50 to 500 for EAP timeout issue
...
{
    mpDebugPrint("eap_get_config sm->eapol_ctx %x",sm->eapol_ctx);
...
{
    //mpDebugPrint("eap_is_wps_pin_enrollee");
	if (conf->identity_len != WSC_ID_ENROLLEE_LEN ||
	    os_memcmp(conf->identity, WSC_ID_ENROLLEE, WSC_ID_ENROLLEE_LEN))
		return 0; /* Not a WPS Enrollee */
