...
static void wpa_supplicant_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	mpDebugPrint("wpa_supplicant_timeout");
...
//	mpDebugPrint("Cancelling authentication timeout");
	eloop_cancel_timeout(wpa_supplicant_timeout, wpa_s, NULL);
...
int wpa_supplicant_reload_configuration(struct wpa_supplicant *wpa_s)
{
	...
	int reconf_ctrl;
	mpDebugPrint("wpa_supplicant_reload_configuration");
			      ...
			      struct wpa_scan_res *bss, struct wpa_ssid *ssid)
{
	...
	mpDebugPrint("wpa_supplicant_associate");
			     ...
			     const u8 *buf, size_t len)
{
	...
	wpa_printf(MSG_DEBUG, "wpa_s->key_mgmt %x",wpa_s->key_mgmt);
...
int wpa_supplicant_driver_init(struct wpa_supplicant *wpa_s)
{
	...
	static int interface_wait = 2;//sec
	
	mpDebugPrint("wpa_supplicant_driver_init");
	if (wpa_s->driver->send_eapol) {
		...
		wpa_s->l2 = l2_packet_init(wpa_s->ifname,
					   &myethaddr,//wpa_drv_get_mac_addr(wpa_s),
					   ETH_P_EAPOL,
	...
	wpa_supplicant_req_scan(wpa_s, interface_wait, 100000);
	interface_count++;
