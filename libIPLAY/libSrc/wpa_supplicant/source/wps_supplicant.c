...
extern BYTE WpsStatus;
struct os_time WPS_time;

int wpas_wps_eapol_cb(struct wpa_supplicant *wpa_s)
{
    mpDebugPrint("wpas_wps_eapol_cb");
				   ...
				   const struct wps_credential *cred)
{
	...
	wpa_msg(wpa_s, MSG_DEBUG, WPS_EVENT_CRED_RECEIVED);
	...
	mpDebugPrint("wpa_s->conf->update_config %x",wpa_s->conf->update_config);
	wpa_s->conf->update_config = 1;

#ifndef CONFIG_NO_CONFIG_WRITE
	if (wpa_s->conf->update_config &&
	    wpa_config_write(wpa_s->confname,cred)) {
					  ...
					  struct wps_event_fail *fail)
{
	...
	mpDebugPrint("wpa_supplicant_wps_event_fail");
...
int wpas_wps_start_pbc(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
	...
	mpDebugPrint("wpas_wps_start_pbc");
		
	wpas_clear_wps(wpa_s);
	
	ssid = wpas_wps_add_network(wpa_s, 0, bssid);
	
	mpDebugPrint("ssid->ssid %s",ssid->ssid);
		
	if (ssid == NULL)
		return -1;
	wpa_config_set(ssid, "phase1", "\"pbc=1\"", 0);
	os_get_time(&WPS_time);
	eloop_register_timeout_wps(WPS_PBC_WALK_TIME, 0, wpas_wps_timeout,
			       wpa_s, NULL);
		       ...
		       const char *pin)
{
	...
	mpDebugPrint("!!!!!!!wpas_wps_start_pin");
	...
	if (ssid == NULL)
	{
		mpDebugPrint("!!!!!!!ssid == NULL");
		return -1;
	}
	
	if (pin)
	{
		mpDebugPrint("!!!!!!!pin == %s",pin);
			      ...
			      struct wpa_scan_res *bss)
{
	...
	mpDebugPrint("wpas_wps_ssid_wildcard_ok");
	if (eap_is_wps_pbc_enrollee(&ssid->eap)) {
		wps_ie = wpa_scan_get_vendor_ie_multi(bss, WPS_IE_VENDOR_TYPE);
		if (wps_ie && wps_is_selected_pbc_registrar(wps_ie)) {
			/* allow wildcard SSID for WPS PBC */
			mpDebugPrint("allow wildcard SSID for WPS PBC");
			ret = 1;
		}
		...
		if (wps_ie && wps_is_selected_pin_registrar(wps_ie)) {
			...
			mpDebugPrint(" allow wildcard SSID for WPS PIN");
			ret = 1;
		}
	}
	...
	if (!ret && ssid->bssid_set &&
		...
		mpDebugPrint("allow wildcard SSID due to hardcoded BSSID match");
		ret = 1;
	}
