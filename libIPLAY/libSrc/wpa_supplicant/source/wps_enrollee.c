				     ...
				     enum wsc_op_code *op_code)
{
	...
	mpDebugPrint("wps_enrollee_get_msg");
	...
	case SEND_M1:
		...
		*op_code = WSC_MSG;
		...
		mpDebugPrint("SEND_M1");
		break;
	case SEND_M3:
		msg = wps_build_m3(wps);
		*op_code = WSC_MSG;
		
		mpDebugPrint("SEND_M3");
		break;
	case SEND_M5:
		msg = wps_build_m5(wps);
		*op_code = WSC_MSG;
		
		mpDebugPrint("SEND_M5");
		break;
	case SEND_M7:
		msg = wps_build_m7(wps);
		*op_code = WSC_MSG;
		
		mpDebugPrint("SEND_M7");
		break;
	case RECEIVED_M2D:
		if (wps->wps->ap) {
		...
		*op_code = WSC_ACK;
		...
		mpDebugPrint("RECEIVED_M2D");
		break;
	case SEND_WSC_NACK:
		msg = wps_build_wsc_nack(wps);
		*op_code = WSC_NACK;
		
		mpDebugPrint("SEND_WSC_NACK");
		break;
	case WPS_MSG_DONE:
		msg = wps_build_wsc_done(wps);
		*op_code = WSC_Done;
		
		mpDebugPrint("WPS_MSG_DONE");
		break;
	default:
