...
extern struct wps_credential wps_cred;
						...
						void *sock_ctx)
{
	...
	mpDebugPrint("wpa_supplicant_stop_countermeasures");
					...
					struct wpa_ssid *ssid)
{
	...
	mpDebugPrint("wpa_supplicant_match_privacy %s",ssid->ssid);
	if (ssid->mixed_cell)
	{
	
		mpDebugPrint("return 1");
		return 1;
	}
	...
	for (i = 0; i < NUM_WEP_KEYS; i++) {
		if (ssid->wep_key_len[i]) {
			...
			mpDebugPrint("ssid->wep_key_len[i] %x",ssid->wep_key_len[i]);
			privacy = 1;
			break;
		}
	}
...
#endif /* IEEE8021X_EAPOL */
#if Make_WPS
   	if((WpsRun)||(wps_cred.key_len))
		privacy = 1;
#endif	

	if (bss->caps & IEEE80211_CAP_PRIVACY)
	{
		mpDebugPrint("return privacy");
		return privacy;
	}
	return !privacy;
}
			      ...
			      struct wpa_ssid **selected_ssid)
{
	...
	mpDebugPrint("Try to find WPA-enabled AP");
	for (i = 0; i < wpa_s->scan_res->num; i++) {
		const u8 *ssid_;
		u8 wpa_ie_len, rsn_ie_len, ssid_len;
		bss = wpa_s->scan_res->res[i];
		
		//NetPacketDump(bss+1,bss->ie_len);
				  ...
				  struct wpa_ssid **selected_ssid)
{
	...
	mpDebugPrint("Try to find non-WPA AP  num = %d", wpa_s->scan_res->num);
	for (i = 0; i < wpa_s->scan_res->num; i++) {
		const u8 ssid_[32];
		u8 wpa_ie_len, rsn_ie_len, ssid_len;
		bss = wpa_s->scan_res->res[i];
		...
		for (ssid = group; ssid; ssid = ssid->pnext) {
			...
			if (check_ssid &&((ssid_len != ssid->ssid_len) ||(os_memcmp(ssid_, ssid->ssid, ssid_len) != 0))) {
				wpa_printf(MSG_DEBUG, "   skip - "
					   "SSID mismatch");
				continue;
			}
...
static void wpa_supplicant_event_scan_results(struct wpa_supplicant *wpa_s)
{
	...
	mpDebugPrint("wpa_supplicant_event_scan_results");
				       ...
				       union wpa_event_data *data)
{
	...
	MP_DEBUG("wpa_supplicant_event_assoc");
...
static void wpa_supplicant_event_disassoc(struct wpa_supplicant *wpa_s)
{
	...
	mpDebugPrint("wpa_supplicant_event_disassoc");
