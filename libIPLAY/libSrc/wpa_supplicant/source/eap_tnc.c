...
struct eap_tnc_data {
	enum { EAP_TNC_WAIT_START, EAP_TNC_PROC_MSG, EAP_TNC_WAIT_FRAG_ACK, EAP_TNC_DONE, EAP_TNC_FAIL } state;
					 ...
					 struct eap_method_ret *ret, u8 id)
{
	...
	if (data->out_used == wpabuf_len(data->out_buf)) {
		...
		wpa_printf(MSG_DEBUG, "EAP-TNC: Sending out %lu bytes "
			   ...
			   data->out_used);
		data->state = EAP_TNC_WAIT_FRAG_ACK;
	}
				       ...
				       const struct wpabuf *reqData)
{
	...
	if (data->state == EAP_TNC_WAIT_FRAG_ACK) {
		if (len != 0) {
	...
	if (data->state == EAP_TNC_WAIT_START) {
