				 ...
				 size_t *respDataLen, int err)
{
	...
	return eap_sim_msg_finish(msg, respDataLen, NULL, 0);
}
				       ...
				       size_t *respDataLen)
{
	...
	return eap_sim_msg_finish(msg, respDataLen, data->k_aut,0);
}
					  ...
					  u16 notification)
{
	...
	return eap_sim_msg_finish(msg, respDataLen, k_aut, 0);
}
				      ...
				      struct eap_sim_attrs *attr)
{
	...
	eap_sim_derive_keys(data->mk, data->k_encr, data->k_aut, data->msk,
			    data->emsk);
	if (eap_sim_verify_mac(data->k_aut, (const u8 *) req,attr->mac, (u8 *) "", 0)) {
		wpa_printf(MSG_WARNING, "EAP-AKA: Challenge message "
			   "used invalid AT_MAC");
		return eap_aka_client_error(data, req, respDataLen,
					    EAP_AKA_UNABLE_TO_PROCESS_PACKET);
