	...
	TNC_MessageType messageType)
{
	...
	if (imc->imc_send == NULL) {
		base64_mfree(b64); /* To be consistent with the malloc function in base64_en/decode */
	...
	*end = '\0';
	...
	while (*start) {
		...
		*end = '\0';
		...
		base64_mfree(decoded); /* To be consistent with the malloc function in base64_en/decode */
