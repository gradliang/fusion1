...
static int wpa_driver_ralink_get_bssid(void *priv, u8 *bssid)
{
	...
	os_memset(&iwr, 0, sizeof(iwr));
	os_strlcpy(iwr.ifr_name, drv->ifname, IFNAMSIZ);
	//mpDebugPrint("drv->ioctl_sock %x",drv->ioctl_sock);

	if (ioctl(drv->ioctl_sock, SIOCGIWAP, &iwr) < 0) {
		mpDebugPrint("ioctl[SIOCGIWAP]");
		ret = -1;
	}
	os_memcpy(bssid, iwr.u.ap_addr.sa_data, ETH_ALEN);
	
	//mpDebugPrint("wpa_driver_ralink_get_bssid %x %x %x %x %x %x",
				 ...
				 void *ctx, char *data, int len)
{
	...
	while (pos + IW_EV_LCP_LEN <= end) {
		...
		mpDebugPrint("IW_EV_LCP_LEN %x",IW_EV_LCP_LEN);
		...
		case IWEVCUSTOM:
			...
			mpDebugPrint("iwe->u.data.flags %x",iwe->u.data.flags);
					      ...
					      void *sock_ctx)
{
    ...
    MP_DEBUG("event sock rx len=%d", ret);
    MP_ASSERT(sizeof(buf) >= ret);

    if (ret > 0)
    {
        wpa_driver_ralink_event_wireless(ctx, sock_ctx,  (char *)buf, ret);
    }
}	
#endif
...
static void * wpa_driver_ralink_init(void *ctx, const char *ifname)
{
	...
	drv->event_sock = s;
		...
		if (s < 0) {
			mpDebugPrint("socket(PF_UNIX, SOCK_DGRAM, 0)");
			os_free(drv);
			return NULL;
		}
...
static int wpa_driver_ralink_scan(void *priv, const u8 *ssid, size_t ssid_len)
{
	...
	eloop_cancel_timeout(wpa_driver_ralink_scan_timeout, drv, drv->ctx);
	eloop_register_timeout(6, 0, wpa_driver_ralink_scan_timeout, drv,
			       drv->ctx);
		...
		while (pos + 1 < end && pos + 2 + pos[1] <= end) {
			...
			if (pos[0] == WLAN_EID_VENDOR_SPECIFIC &&
				...
				results[ap_num].rsn_ie_len = ielen;
					...
					if(results[ap_num].wpa_ie_len == 0)
					{
						os_memcpy(results[ap_num].wpa_ie, pos, ielen);
						results[ap_num].wpa_ie_len = ielen;
					}
					...
					{
					 os_memcpy(results[ap_num].wpa_ie+results[ap_num].wpa_ie_len, pos, ielen);
					 results[ap_num].wpa_ie_len += ielen;
			}
				     ...
				     const u8 *key, size_t key_len)
{
	...
	mpDebugPrint("key_len %d",key_len);
	mpDebugPrint("key_idx %d",key_idx);
	...
	mpDebugPrint("NDIS_802_11_KEY size %d",sizeof(NDIS_802_11_KEY));
	mpDebugPrint("NDIS_802_11_MAC_ADDRESS size %d",sizeof(NDIS_802_11_MAC_ADDRESS));
	mpDebugPrint("nkey NDIS_802_11_KEY_RSC size %d",sizeof(NDIS_802_11_KEY_RSC));
