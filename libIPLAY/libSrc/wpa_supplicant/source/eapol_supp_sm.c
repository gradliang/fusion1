...
{
    mpDebugPrint("eapol_sm_notify_portEnabled sm %x",sm);
	if (sm == NULL)
		return;
...
{
    mpDebugPrint("eapol_sm_notify_portValid sm %x",sm);
	if (sm == NULL)
		return;
	wpa_printf(MSG_DEBUG, "EAPOL: External notification - "
		   "portValid=%d", valid);
...
void eapol_sm_notify_eap_success(struct eapol_sm *sm, Boolean success)
{
	...
	mpDebugPrint("EAPOL: External notification - "
		   "EAP success=%d", success);
...
{
    mpDebugPrint("MATT @@@@ eapol_sm_notify_config");
	if (sm == NULL)
		return;
