					  ...
					  u16 ver)
{
	...
	//mpDebugPrint("src_addr %x %x %x %x %x %x ",src_addr[0],src_addr[1],src_addr[2],src_addr[3],src_addr[4],src_addr[5]);
					    ...
					    const u8 *addr, int secure)
{
	...
	mpDebugPrint("WPA: Key negotiation completed with %02x:%02x:%02x:%02x:%02x:%02x [PTK=%s GTK=%s]", MAC2STR(addr),
		wpa_cipher_txt(sm->pairwise_cipher),
		wpa_cipher_txt(sm->group_cipher));
				      ...
				      const struct wpa_eapol_key *key)
{
	...
	mpDebugPrint("WPA: Installing PTK to the driver.");
			       ...
			       struct wpa_ptk *ptk)
{
	...
	mpDebugPrint("WPA: Sending EAPOL-Key 4/4");
#ifdef PLATFORM_MPIXEL
    is_packet_4_of_4 = TRUE;
#endif
	wpa_eapol_key_send(sm, ptk->kck, ver, dst, ETH_P_EAPOL,
			   rbuf, rlen, reply->key_mic);
#ifdef PLATFORM_MPIXEL
    is_packet_4_of_4 = FALSE;
#endif
...
#define SET_KEY_BEFORE_SEND_4_OF_4 0
#else
#define SET_KEY_BEFORE_SEND_4_OF_4 1
#endif
#define SET_KEY_AFTER_SEND_2_OF_2 0
...
#if SET_KEY_BEFORE_SEND_4_OF_4 == 1
    {
        if (key_info & WPA_KEY_INFO_INSTALL) {
#if Make_USB == AR2524_WIFI||Make_USB == RALINK_AR2524_WIFI
			if((sm->ap_wpa_ie_len!=0)&&(sm->pairwise_cipher==WPA_CIPHER_CCMP))
				wpa_4way=0;
			else	
            wpa_4way++;
#endif			
...
#if SET_KEY_BEFORE_SEND_4_OF_4 == 0
	if (key_info & WPA_KEY_INFO_INSTALL) {
		wpa_supplicant_install_ptk(sm, key);
	}
#endif
				      ...
				      int ver, u16 key_info)
{
    ...
    DPrintf("--> wpa_supplicant_send_2_of_2\n");
	rbuf = wpa_sm_alloc_eapol(sm, IEEE802_1X_TYPE_EAPOL_KEY, NULL,
				  sizeof(*reply), &rlen, (void *) &reply);
	if (rbuf == NULL)
		return -1;
...
#if SET_KEY_AFTER_SEND_2_OF_2  == 1
				 ...
				 void *ctx, int replace)
{
	...
	mpDebugPrint("wpa_sm_pmksa_free_cb");
