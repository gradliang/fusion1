					  ...
					  int linkmode, int operstate)
{
	...
	if (linkmode != -1) {
        /* IFLA_LINKMODE */
        set_linkmode(NULL, linkmode);
	}
	if (operstate != -1) {
        /* IFLA_OPERSTATE; */
        set_operstate(NULL, operstate);
	}
#endif
...
int wpa_driver_wext_set_ssid(void *priv, const u8 *ssid, size_t ssid_len)
{
    ...
    MP_DEBUG("wpa_driver_wext_set_ssid\n");
					  ...
					  void *sock_ctx)
{
    ...
    struct iw_event *iwe;
        ...
        mpDebugPrint("wpa_driver_wext_event_receive");
	struct sockaddr_un from;
	socklen_t fromlen = sizeof(from);
    
    iwe = (struct iw_event *)os_malloc(sizeof(struct iw_event) + IW_CUSTOM_MAX);
    if (iwe == NULL)
        return;

	ret = recvfrom(sock, iwe, sizeof(struct iw_event) + IW_CUSTOM_MAX, 0,
		       (struct sockaddr *) &from, &fromlen);
	if (ret < 0) {
		perror("recvfrom(event_iface)");
        os_free(iwe);
        MP_ASSERT(0);
		return;
	}

    MP_ASSERT(iwe->len <= (sizeof(struct iw_event) + IW_CUSTOM_MAX));

//		sendto(sock, reply, reply_len, 0, (struct sockaddr *) &from, TODO
//		       fromlen);
#if DEMO_PID
    TaskSleep(1);
#else
    ...
    if (ret > 0)
    {
        ...
        struct ifinfomsg *ifi = (struct ifinfomsg *)os_malloc(sizeof(struct ifinfomsg));
        extern unsigned mpx_dev_get_flags(void);

        if (ifi)
        {
            ifi->ifi_flags = mpx_dev_get_flags();

            wpa_printf(MSG_DEBUG, "wpa_driver_wext_event_receive: operstate=%d ifi_flags=0x%x "
                    "(%s%s%s%s)",
                    drv->operstate, ifi->ifi_flags,
                    (ifi->ifi_flags & IFF_UP) ? "[UP]" : "",
                    (ifi->ifi_flags & IFF_RUNNING) ? "[RUNNING]" : "",
        ...
        wpa_driver_wext_event_wireless(
                eloop_ctx, sock_ctx,  (char *)iwe, iwe->len);
    }
    os_free(iwe);
...
{
	struct ifreq ifr;
	mpDebugPrint("wpa_driver_wext_get_ifflags_ifname");
					      ...
					      const char *ifname, int flags)
{
	...
	mpDebugPrint("wpa_driver_wext_set_ifflags_ifname");
	os_memset(&ifr, 0, sizeof(ifr));
	os_strlcpy(ifr.ifr_name, ifname, IFNAMSIZ);
...
{
	int s;
	mpDebugPrint("wpa_driver_wext_init");
#ifdef LINUX
	...
	drv->mlme_sock = -1;
		...
		os_snprintf(local.sun_path, UNIX_PATH_MAX, "/wpa_event");
...
static void wpa_driver_wext_finish_drv_init(struct wpa_driver_wext_data *drv)
{
	...
	mpDebugPrint("wpa_driver_wext_finish_drv_init");

	if (wpa_driver_wext_get_ifflags(drv, &flags) != 0)
		mpDebugPrint("Could not get interface '%s' flags\n", drv->ifname);
	else if (!(flags & IFF_UP)) {
		if (wpa_driver_wext_set_ifflags(drv, flags | IFF_UP) != 0) {
			mpDebugPrint("Could not set interface '%s' UP\n",
			       drv->ifname);
		} else {
			/*
	...
	if (wpa_driver_wext_set_mode(drv, 0) < 0) {
		mpDebugPrint("Could not configure driver to use managed mode\n");
	}
...
void wpa_driver_wext_scan_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	mpDebugPrint("Scan timeout - try to get results");
				    ...
				    size_t *len)
{
	...
	mpDebugPrint("wpa_driver_wext_giwscan");

#ifdef LINUX
	res_buf_len = IW_SCAN_MAX_DATA;
#else
	res_buf_len = IW_SCAN_MAX_DATA * 2;
#endif
	for (;;) {
		...
		{
		    mpDebugPrint("ioctl SIOCGIWSCAN break;");
			break;
		}
		...
		if (errno == E2BIG && res_buf_len < 100000) {
			...
			mpDebugPrint("ioctl[SIOCGIWSCAN]");
			os_free(res_buf);
			return NULL;
		}
	}
	...
	*len = iwr.u.data.length;
	...
	mpDebugPrint("res_buf %x len %d",res_buf,*len);
	return res_buf;
}
			       ...
			       char *end)
{
	...
	//mpDebugPrint("res->ssid %s",res->ssid);
}
				 ...
				 char *end)
{
	size_t clen= 0;
	u8 *tmp;
	//mpDebugPrint("wext_get_scan_custom");
	//mpDebugPrint("iwe->u.data.length %x",iwe->u.data.length);
	//mpDebugPrint("clen %x",clen);
	//mpDebugPrint("custom %x",custom);
	//mpDebugPrint("end %x",end);
	//NetAsciiDump(custom,iwe->u.data.length);
...
struct wpa_scan_results * wpa_driver_wext_get_scan_results(void *priv)
{
	...
	mpDebugPrint("wpa_driver_wext_get_scan_results");
	...
	while (pos + IW_EV_LCP_LEN <= end) {
		...
		//mpDebugPrint("iwe->cmd %x",iwe->cmd);
			    ...
			    const u8 *key, size_t key_len)
{
	...
	if (ret == -2 &&
		...
		wpa_printf(MSG_DEBUG, "Driver did not support "
			   "SIOCSIWENCODEEXT");
		mpDebugPrint("%s: errno=%d", __func__, errno);
		return ret;
	}
