...
int wpa_run_scan = 0;
#if (AUTO_CONNECTION || (SHOW_FRAME_TEST && Make_ADHOC && (Make_USB == 3)))
...
static void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx)
{
	...
	mpDebugPrint("wpa_supplicant_scan");
	...
	if (!enabled && !wpa_s->scan_req) {
		mpDebugPrint("No enabled networks - do not scan");
		wpa_supplicant_set_state(wpa_s, WPA_INACTIVE);
		//for pwd check
		wpa_supplicant_set_run_status(TRUE);
#if (AUTO_CONNECTION || (SHOW_FRAME_TEST && Make_ADHOC && (Make_USB == 3)))
		auto_scan++;
		mpDebugPrint("[%s] auto_scan= %d\n", __func__, auto_scan);
		if(auto_scan == 2)
	...
	if (wpa_s->conf->ap_scan != 0 &&
		...
		mpDebugPrint("Using wired authentication - "
	...
	if (scan_req != 2 && wpa_s->conf->ap_scan == 2) {
		 ...
		 */
		if (ssid == NULL) {
			mpDebugPrint("wpa_supplicant_scan: Reached "
	...
	mpDebugPrint("Starting AP scan (%s SSID)",
	...
	if (wpa_s->scan_res_tried == 0 && wpa_s->conf->ap_scan == 1 &&
		...
		mpDebugPrint("Trying to get current scan results "
	...
	if (ret) {
		mpDebugPrint("Failed to initiate AP scan.");
...
void wpa_supplicant_req_scan(struct wpa_supplicant *wpa_s, int sec, int usec)
{
	 ...
	 */
	 mpDebugPrint("wpa_supplicant_req_scan");
...
void wpa_supplicant_set_run_status(BYTE flag)
{
    mpDebugPrint("wpa_supplicant_set_run_status %d",flag);
	wpa_run_scan = flag;
}
int wpa_supplicant_get_run_status()
{
    mpDebugPrint("wpa_supplicant_get_run_status %d",wpa_run_scan);
	return wpa_run_scan;
}