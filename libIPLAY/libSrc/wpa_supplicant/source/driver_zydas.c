...
extern const struct wpa_driver_ops wpa_driver_wext_ops;
static int
				     ...
				     const u8 *key, size_t key_len)
{
    ...
    wpa_driver_wext_set_key(drv->wext, alg,
				     addr, key_idx, set_tx,
...
#if 0
#ifdef PLATFORM_MPIXEL
    wpa_driver_wext_set_key(priv, alg,
				     addr, key_idx, set_tx,
				     seq, seq_len, key, key_len);
#endif
#endif
...
#else
	int r;
	r = zydas_set_param(drv, ZD_PARAM_COUNTERMEASURES, enabled, 1);
	wpa_driver_wext_ops.set_countermeasures(drv->wext, enabled);
	return r;
#endif
}
...
#else
	int r;
	r = zydas_set_param(drv, ZD_PARAM_DROPUNENCRYPTED, enabled, 1);
	wpa_driver_wext_ops.set_drop_unencrypted(drv->wext, enabled);
	return r;
#endif
}
...
wpa_driver_zydas_deauthenticate(void *priv, const u8 *addr, int reason_code)
{
	...
	memset(param, 0, blen);
	param->cmd = ZD_CMD_SET_MLME;               /* TODO */
	...
	ret = zydas_ioctl(drv, param, blen, 1);
        ...
        free(param);
#ifdef PLATFORM_MPIXEL
	wpa_driver_wext_ops.deauthenticate(drv->wext, addr, reason_code);
	usleep(100000);
#endif
        return ret;
}
...
wpa_driver_zydas_disassociate(void *priv, const u8 *addr, int reason_code)
{
	...
	ret = zydas_ioctl(drv, param, blen, 1);
        free(param);
#ifdef PLATFORM_MPIXEL
	wpa_driver_wext_ops.disassociate(drv->wext, addr, reason_code);
#endif
        return ret;
}
...
{
	struct wpa_driver_zydas_data *drv = priv;
	struct wpa_supplicant *wpa = drv->ctx;
	int _ap_scan = 1;
	...
	mpDebugPrint("%s: params->wpa_ie_len=%d", __func__,params->wpa_ie_len);
	...
	{
//	memcpy(&mBssId, params->ssid, 6);
	wpa_driver_wext_ops.associate(drv->wext, params);
	}
...
#else
	int r;
	r = zydas_set_param(drv, ZD_PARAM_AUTH_ALGS, algs, 1);
	wpa_driver_wext_ops.set_auth_alg(drv->wext, auth_alg);
	return r;
#endif
}
...
#else
static struct wpa_scan_results * wpa_driver_zydas_get_scan_results(void *priv,
					    struct wpa_scan_result *results,
					    size_t max_size)
{
	struct wpa_driver_zydas_data *drv = priv;
...
#endif
#ifdef PLATFORM_MPIXEL
static int wpa_driver_zydas_set_operstate(void *priv, int state)
{
	struct wpa_driver_zydas_data *drv = priv;
	return wpa_driver_wext_set_operstate(drv->wext, state);
}
static int wpa_driver_zydas_set_mode(void *priv, int mode)
{
	struct wpa_driver_zydas_data *drv = priv;
    extern struct net_device *g_dev;

    if (g_dev && g_dev->do_ioctl)
    {
        struct iwreq wreq;
        memset(&wreq, 0, sizeof(wreq));
        wreq.u.mode = mode;
        MP_DEBUG("%s: calls g_dev->do_ioctl(SIOCSIWMODE)", __func__);
        g_dev->do_ioctl(g_dev, (struct ifreq *)&wreq, SIOCSIWMODE);
    }
...
//	.set_wpa		= wpa_driver_zydas_set_wpa,
	...
	.get_scan_results2 = wpa_driver_zydas_get_scan_results,
...
#ifdef PLATFORM_MPIXEL
	.set_operstate = wpa_driver_zydas_set_operstate,
	.set_mode = wpa_driver_zydas_set_mode,
#endif
};
