...
struct eap_wsc_data {
	enum { EAP_WSC_WAIT_START, EAP_WSC_MESG, EAP_WSC_FRAG_ACK, EAP_WSC_WAIT_FRAG_ACK, EAP_WSC_DONE, EAP_WSC_FAIL } state;
	int registrar;
...
{
	switch (state) {
	case EAP_WSC_WAIT_START:
		return "WAIT_START";
	case EAP_WSC_MESG:
		return "MESG";
	case EAP_WSC_FRAG_ACK:
		return "FRAG_ACK";
	case EAP_WSC_WAIT_FRAG_ACK:
...
static void * eap_wsc_init(struct eap_sm *sm)
{
	...
	if (data == NULL)
		return NULL;
	data->state = registrar ? EAP_WSC_MESG : EAP_WSC_WAIT_START;
					 ...
					 struct eap_method_ret *ret, u8 id)
{
	...
	if (data->out_used == wpabuf_len(data->out_buf)) {
		...
		if ((data->state == FAIL && data->out_op_code == WSC_ACK) ||
			...
			eap_wsc_state(data, EAP_WSC_MESG);
	} else {
		wpa_printf(MSG_DEBUG, "EAP-WSC: Sending out %lu bytes "
			   "(%lu more to send)", (unsigned long) send_len,
			   (unsigned long) wpabuf_len(data->out_buf) -
			   data->out_used);
		eap_wsc_state(data, EAP_WSC_WAIT_FRAG_ACK);
	}
				       ...
				       const struct wpabuf *reqData)
{
	...
	if (data->state == EAP_WSC_WAIT_FRAG_ACK) {
	...
	if (data->state == EAP_WSC_WAIT_START) {
