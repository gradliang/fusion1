...
{
	const int frameratecode2framerate[16] =
	{
		0,
	...
	////MP_DEBUG1("mp_header_process_sequence_header: height = ",height,8);
	////MP_DEBUG1("mp_header_process_sequence_header: width = ",width,8);

	if ((width > 768) || (height > 576))
	{
		//MP_DPF("size restrictions for MP@ML or MPEG1 exceeded! (%dx%d)\n",width,height);
//  return 1;   /* size restrictions for MP@ML or MPEG1 */
    ...
    picture->frame_rate_extension_d = 10000;
	return 0;
}

static int header_process_sequence_extension(mp_mpeg_header_t *const picture, const unsigned char * const buffer)
{
	/* check chroma format, size extensions, marker bit */
	if (((buffer[1] & 0x07) != 0x02) || (buffer[2] & 0xe0) || ((buffer[3] & 0x01) != 0x01))
		return 1;
...
static int header_process_picture_coding_extension(mp_mpeg_header_t * const picture,
...
static int mp_unescape03(unsigned char *buf, int len)
{
	...
	dest = (unsigned char *)mem_malloc(len);
	if (! dest)
		return 0;
