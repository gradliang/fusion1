...
#define XVID_CSP_YYUV     (1<< 16) /* 4:2:2 packed */
...
typedef struct {
	...
	int stride[5];			/* [in] image stride; "bytes per row"*/
} xvid_image_t;
...
typedef struct {
	...
	void * handle; /* [out]	   decode context handle, struct DECODER*/
} xvid_dec_create_t;
...
//image to decode strcture 
typedef struct {
	int version;
	int general;         	/* [in:opt] general flags */
	void *bitstream;     	/* [in]     bitstream (read from) receive the data store in packet buffer from demuxer*/ 
	int length;          	/* [in]     bitstream length  pac packet buffer length*/
