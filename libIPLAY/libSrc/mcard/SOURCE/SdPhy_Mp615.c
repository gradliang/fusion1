...
#define IM_DATA					(IM_TREND|IC_CRC16F|IC_PGMF|IC_WXF|IC_SDTO)
#else
#define IM_CMD					0
#define IM_DATA					0
#endif
...
#define WATCHDOG_TIMER			0x001fffff
...
SWORD SdHost_WaitCardResponse(DWORD *Resp)
{
	...
	if (EventWaitWithTO(MCARD_EVENT_ID, 0xffffffff, OS_EVENT_OR, &dwCause, 5000) != PASS)
		MP_DEBUG("\tSdHost_WaitCardResponse timeout %x", dwCause);
#else
	DWORD tmr = GetSysTime();
	do
	{
	...
	{
		MP_DEBUG("\t-E- SdHost_WaitCardResponse timeout : %x", dwCause);
		ret = RES_TIMEOUT;
	}
	if (ret != SD_PASS)
...
SWORD SdHost_WaitTransfer()
{
	...
	if (EventWaitWithTO(MCARD_EVENT_ID, 0xffffffff, OS_EVENT_OR, &dwCause, 5000) != PASS)
	{
		mpDebugPrint("\tSdHost_WaitTransfer timeout %x", sMcard->McSdIc);
		dwCause = 0;
	}
#else
	DWORD tmr = GetSysTime();
