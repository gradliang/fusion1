...
//    DBG("off=%u, whence=%d", (unsigned int)offset, whence);
    if(!s)
    ...
    while (size > 0) {
        ...
        if (len == 0) {
            if(size > s->buffer_size && !s->update_checksum){
                if(s->read_packet)
                {
                    DBG("calls read_packet sz=%d", size);
                    len = s->read_packet(s->opaque, buf, size);
                    DBG("read_packet returns %d", len);
                }
                if (len <= 0) {
                    /* do not modify buffer if EOF reached so that a seek back can
                    be done without rereading data */
...
//            DBG("s->buffer_size=%d", s->buffer_size);
...
#else
    MP_ASSERT(flags == AVIO_FLAG_WRITE || flags == AVIO_FLAG_READ);
#endif
