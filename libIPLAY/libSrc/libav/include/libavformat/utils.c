...
#define     av_rescale(offset, t1, t2) av_rescale2(offset, t1, t2)
#endif
/**
 * @file
 * various utility functions for use within Libav
...
AVInputFormat *av_probe_input_format2(AVProbeData *pd, int is_opened, int *score_max)
{
    ...
    DBG("count=%d, buf_size=%d", dod216++, lpd.buf_size);
...
//        MP_ASSERT(0);
        if (fmt1->read_probe) {
            ...
            DBG("read_probe=%p returns %d", fmt1->read_probe, score);
...
static int init_input(AVFormatContext *s, const char *filename, AVDictionary **options)
{
    ...
    DBG("filename=%s, s=%p, s->pb=%p",filename, s, s->pb);
//    MP_ASSERT(0);
    if (s->pb) {
...
int avformat_open_input(AVFormatContext **ps, const char *filename, AVInputFormat *fmt, AVDictionary **options)
{
    ...
    DBG("calls init_input");
    if ((ret = init_input(s, filename, &tmp)) < 0)
        goto fail;

    DBG("init_input returns %d", ret);
    ...
    DBG("s->iformat->read_header=%p", s->iformat->read_header);
    if (s->iformat->read_header)
        if ((ret = s->iformat->read_header(s)) < 0)
            goto fail;

    DBG("id3v2_extra_meta=%p", id3v2_extra_meta);
    if (id3v2_extra_meta &&
        (ret = ff_id3v2_parse_apic(s, &id3v2_extra_meta)) < 0)
        goto fail;
    ff_id3v2_free_extra_meta(&id3v2_extra_meta);

    DBG("calls queue_attached_pictures");
    queue_attached_pictures(s);

    DBG("s->data_offset=%d", s->data_offset);
...
//    DBG("");
    for(;;){
        ...
        pkt->size = 0;
        av_init_packet(pkt);
//        DBG("calls s->iformat->read_packet");
...
//        DBG("codec_type=%d", st->codec->codec_type);
...
//    DBG("ctx=%p", s);
    ...
    while (!got_packet && !s->parse_queue) {
        ...
        /* read next packet */
//        DBG("calls ff_read_packet");
        ret = ff_read_packet(s, &cur_pkt);
//        DBG("ff_read_packet returns %d", ret);
        if (ret < 0) {
...
//    DBG("genpts=%d, buf=%p", genpts, s->packet_buffer);
    if (!genpts)
                       ...
                       int64_t pos, int64_t timestamp, int size, int distance, int flags)
{
    ...
    entries = av_fast_realloc(*index_entries,
                              ...
                              (*nb_index_entries + 1) *
                              sizeof(AVIndexEntry));
//    DBG("av_fast_realloc returns %p", entries);
    if(!entries)
        return -1;
...
//        DBG("read_frame_internal returns %d", ret);
        if (ret < 0) {
            /* EOF or error*/
...
//    DBG("ic->nb_streams=%d", ic->nb_streams);
    for(i=0;i<ic->nb_streams;i++) {
        st = ic->streams[i];
        DBG("calls avcodec_close");
#ifdef LINUX
        avcodec_close(st->codec);
#else
        avcodec_close2(st->codec);
#endif
    }
    for(i=0;i<ic->nb_streams;i++) {
        st = ic->streams[i];
        if (st->codec->codec_type == AVMEDIA_TYPE_VIDEO) {
            DBG("codec_type==AVMEDIA_TYPE_VIDEO");
            ...
            DBG("codec_type==AVMEDIA_TYPE_AUDIO");
                        ...
                        int flags)
{
    ...
    for (i = 0; i < nb_streams; i++) {
        ...
        if (st->disposition & (AV_DISPOSITION_HEARING_IMPAIRED|AV_DISPOSITION_VISUAL_IMPAIRED))
            continue;
        if (decoder_ret) {
            DBG("calls avcodec_find_decoder");
            decoder = avcodec_find_decoder(st->codec->codec_id);
            DBG("avcodec_find_decoder returns %p", decoder);
            if (!decoder) {
                if (ret < 0)
