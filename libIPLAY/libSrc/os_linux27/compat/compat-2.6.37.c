...
#else
    if (info->nlhdr->nlmsg_type == NETLINK_NL80211_PID)
        family = m_nl80211_family();
    else
        family = NULL;
    if (family)
        goto found;
...
#undef __copy
	if (family->pre_doit) {
		...
		if (err)
        {
            DBG("pre_doit returns error=%d", err);
			return err;
        }
	}
...
#else
	ret = genl_register_family_kernel(&family->family);
#endif
	if (ret < 0)
		return ret;
