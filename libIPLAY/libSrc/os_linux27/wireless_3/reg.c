...
static bool is_an_alpha2(const char *alpha2)
{
    ...
    {
        DBG("alpha2=%c%c - returns true", alpha2[0], alpha2[1]);
		return true;
    }
    DBG("alpha2=%c%c - returns false", alpha2[0], alpha2[1]);
	return false;
}
...
#else
    reg_requests_lock = (spinlock_t)mpx_SemaphoreCreate(OS_ATTR_PRIORITY, 1);
    reg_pending_beacons_lock = (spinlock_t)mpx_SemaphoreCreate(OS_ATTR_PRIORITY, 1);
#endif
