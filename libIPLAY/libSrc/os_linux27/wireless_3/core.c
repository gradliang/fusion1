...
struct cfg80211_registered_device *g_rdev;
					 ...
					 void *ndev)
{
	...
	mpDebugPrint("%s: state=%u", __func__, state);
...
#ifdef PLATFORM_MPIXEL
extern struct mutex reg_mutex;
int mpx_cfg80211_init(void)
{
    struct mutex *mutex;

//    mutex_init(&cfg80211_mutex);
    mutex = &cfg80211_mutex;
