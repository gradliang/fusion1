			...
			struct sockaddr *ap_addr, char *extra)
{
	...
	if (ap_addr)
		mpDebugPrint("%s:%d ap_addr=" MACSTR, __func__,__LINE__, MAC2STR((u8 *)ap_addr->sa_data));
	else
		mpDebugPrint("%s:%d ap_addr=NULL", __func__,__LINE__);
#ifdef LINUX
	switch (wdev->iftype) {
	case NL80211_IFTYPE_ADHOC:
			   ...
			   struct iw_point *data, char *ssid)
{
	...
	if (ssid)
		mpDebugPrint("%s:%d ssid=%s", __func__,__LINE__, ssid);
	else
		mpDebugPrint("%s:%d ssid=NULL", __func__,__LINE__);
	switch (wdev->iftype) {
	case NL80211_IFTYPE_ADHOC:
