...
extern int		nla_parse(struct nlattr *tb[], int maxtype,
				  ...
				  const struct nla_policy *policy);
#else
extern int		nla_validate_kernel(struct nlattr *head, int len, int maxtype,
				     const struct nla_policy *policy);
extern int		nla_parse_kernel(struct nlattr *tb[], int maxtype,
				  struct nlattr *head, int len,
				  const struct nla_policy *policy);
#endif
#ifdef LINUX
...
#else
extern struct nlattr *	nla_find_kernel(struct nlattr *head, int len, int attrtype);
extern size_t		nla_strlcpy_kernel(char *dst, const struct nlattr *nla,
				    size_t dstsize);
extern int		nla_memcpy_kernel(void *dest, struct nlattr *src, int count);
extern int		nla_memcmp_kernel(const struct nlattr *nla, const void *data,
				   size_t size);
extern int		nla_strcmp_kernel(const struct nlattr *nla, const char *str);
#endif
...
extern struct nlattr *	nla_reserve(struct sk_buff *skb, int attrtype,
				    int attrlen);
#else
extern struct nlattr *	nla_reserve_kernel(struct sk_buff *skb, int attrtype,
				    int attrlen);
#endif
...
extern int		nla_put(struct sk_buff *skb, int attrtype,
				int attrlen, const void *data);
#else
extern int		nla_put_kernel(struct sk_buff *skb, int attrtype,
				int attrlen, const void *data);
#endif
...
#else
	return nla_parse_kernel(tb, maxtype, nlmsg_attrdata(nlh, hdrlen),
			 nlmsg_attrlen(nlh, hdrlen), policy);
#endif
}
...
#else
	return nla_find_kernel(nlmsg_attrdata(nlh, hdrlen),
			nlmsg_attrlen(nlh, hdrlen), attrtype);
#endif
}
...
#else
	return nla_validate_kernel(nlmsg_attrdata(nlh, hdrlen),
			    nlmsg_attrlen(nlh, hdrlen), maxtype, policy);
#endif
}
...
#else
	return nla_find_kernel(nla_data(nla), nla_len(nla), attrtype);
#endif
}
...
#else
	return nla_parse_kernel(tb, maxtype, nla_data(nla), nla_len(nla), policy);
#endif
}
					    ...
					    int len)
{
	...
	if (nested_len >= nla_attr_size(0))
		...
		return nla_parse(tb, maxtype, nla_data(nla) + NLA_ALIGN(len),
				 nested_len, policy);
#else
		return nla_parse_kernel(tb, maxtype, nla_data(nla) + NLA_ALIGN(len),
				 nested_len, policy);
#endif
...
#else
	return nla_put_kernel(skb, attrtype, sizeof(u8), &value);
#endif
}
...
#else
	return nla_put_kernel(skb, attrtype, sizeof(u16), &value);
#endif
}
...
#else
	return nla_put_kernel(skb, attrtype, sizeof(u32), &value);
#endif
}
...
#else
	return nla_put_kernel(skb, attrtype, sizeof(u64), &value);
#endif
}
...
#else
	return nla_put_kernel(skb, attrtype, strlen(str) + 1, str);
#endif
}
...
#else
	return nla_put_kernel(skb, attrtype, 0, NULL);
#endif
}
...
#else
	return nla_put_kernel(skb, attrtype, sizeof(u64), &tmp);
#endif
}
...
#define NLA_PUT(skb, attrtype, attrlen, data) \
	do { \
		if (unlikely(nla_put_kernel(skb, attrtype, attrlen, data) < 0)) \
			goto nla_put_failure; \
	} while(0)
#endif
...
#else
	nla_memcpy_kernel(&tmp, nla, sizeof(tmp));
#endif
...
#else
	if (nla_put_kernel(skb, attrtype, 0, NULL) < 0)
#endif
		return NULL;
...
#else
	if (nla_put_kernel(skb, attrtype, attrlen, data) < 0)
#endif
		return NULL;
...
#else
	return nla_validate_kernel(nla_data(start), nla_len(start), maxtype, policy);
#endif
}
