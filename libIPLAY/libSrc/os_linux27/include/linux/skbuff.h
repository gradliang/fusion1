...
#define 	alloc_skb(s,p)	_alloc_skb(s,p,__location__)
static inline struct sk_buff *_alloc_skb(unsigned int size,
					gfp_t priority, char *loc)
#else
...
{
#ifdef HAVE_HOSTAPD
    extern long mpx_SemaphoreCreate(unsigned char attr, unsigned char count);
//    void *SemaphoreGetPoint(unsigned char id);
//    if (TaskGetId() == 21)
    {
        if (list->lock == 0)
        {
            int _id = mpx_SemaphoreCreate(OS_ATTR_PRIORITY, 1);
            list->lock = (spinlock_t)_id;
        }
        else
        {
            NET_ASSERT(SemaphoreGetPoint(list->lock));
...
static inline unsigned char *skb_put(struct sk_buff *skb, unsigned int len)
{
    ...
    {
        mpDebugPrint("skb_put: skb->tail=%p,skb->end=%p", skb->tail,skb->end); 
        NET_ASSERT(0);
    }
	return tmp;
}
...
#else
#ifdef DEBUG_SKBUFF_LEAKAGE
#define dev_alloc_skb(sz) _dev_alloc_skb(sz, __location__)
static inline struct sk_buff *_dev_alloc_skb(unsigned int length, char *location)
#else
static inline struct sk_buff *dev_alloc_skb(unsigned int length)
#endif
{
#ifdef DEBUG_SKBUFF_LEAKAGE
	return _netBufAlloc(length, location);
#else
