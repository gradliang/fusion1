...
#ifdef PLATFORM_MPIXEL
#define nla_put(skb, attrtype, attrlen, data) nla_put_kernel(skb, attrtype, attrlen, data)
#endif
...
#else
    int r;
    DBG("calls genlmsg_reply(%p)", msg);
	r = genlmsg_reply(msg, info);
    DBG("returns %d", r);
	return r;
#endif
}
	...
	if (info->attrs[NL80211_ATTR_WIPHY_TXQ_PARAMS]) {
		...
		nla_for_each_nested(nl_txq_params,
			...
			nla_parse(tb, NL80211_TXQ_ATTR_MAX,
				  ...
				  txq_params_policy);
#else
			nla_parse_kernel(tb, NL80211_TXQ_ATTR_MAX,
				  nla_data(nl_txq_params),
				  nla_len(nl_txq_params),
				  txq_params_policy);
#endif
...
static int nl80211_set_reg(struct sk_buff *skb, struct genl_info *info)
{
	...
	nla_for_each_nested(nl_reg_rule, info->attrs[NL80211_ATTR_REG_RULES],
		...
		nla_parse(tb, NL80211_REG_RULE_ATTR_MAX,
			...
			reg_rule_policy);
#else
		nla_parse_kernel(tb, NL80211_REG_RULE_ATTR_MAX,
			nla_data(nl_reg_rule), nla_len(nl_reg_rule),
			reg_rule_policy);
#endif
...
#else
			nla_parse_kernel(tb, NL80211_SCHED_SCAN_MATCH_ATTR_MAX,
				  nla_data(attr), nla_len(attr),
				  nl80211_match_policy);
#endif
...
#else
		nla_parse_kernel(tb, NL80211_TXRATE_MAX, nla_data(tx_rates),
			  nla_len(tx_rates), nl80211_txattr_policy);
#endif
...
#else
	err = nla_parse_kernel(tb, MAX_NL80211_REKEY_DATA,
			nla_data(info->attrs[NL80211_ATTR_REKEY_DATA]),
			nla_len(info->attrs[NL80211_ATTR_REKEY_DATA]),
			nl80211_rekey_policy);
#endif
...
int nl80211_init(void)
{
 ...
 err_out:
    ...
    short i;
    DBG("");
    INIT_LIST_HEAD(&nl80211_fam.list);

    nl80211_fam.id = 111;  // TODO

#ifdef LINUX
    for (i=0; i < ARRAY_SIZE(nl80211_ops); i++)
        list_add_tail(&nl80211_ops[i].ops_list, &nl80211_fam.ops_list);
#else
//    MP_ASSERT(0);
#endif
...
#ifdef PLATFORM_MPIXEL
struct genl_family *m_nl80211_family()
{
    return &nl80211_fam;
}
#endif
