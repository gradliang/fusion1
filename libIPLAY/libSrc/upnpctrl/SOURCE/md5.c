...
static void MD5Transformi PROTO_LIST( ( UINT4[4],
                                       unsigned char[64] ) );
    ...
    if( inputLen >= partLen ) {
        ...
        MD5_memcpy
            ( ( POINTER ) & context->buffer[index], ( POINTER ) input,
              partLen );
        MD5Transformi( context->state, context->buffer );

        for( i = partLen; i + 63 < inputLen; i += 64 )
            MD5Transformi( context->state, &input[i] );
     ...
     MD5_CTX *context;          /* context */
{
    ...
    MD5Updatei( context, PADDING, padLen );

    /*
       Append length (before padding) 
     */
    MD5Updatei( context, bits, 8 );
