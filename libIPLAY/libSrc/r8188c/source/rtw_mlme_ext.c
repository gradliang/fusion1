...
#ifdef PLATFORM_MPIXEL
    int is_same_ssid = _FALSE;
	u8	*p;
	sint	len2;
    if ( (pmlmeinfo->state == WIFI_FW_ADHOC_STATE) && ((check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE)) == _FALSE) 
            )
    {
        if ((p = rtw_get_ie((pframe + sizeof(struct ieee80211_hdr_3addr)) + _FIXED_IE_LENGTH_, _SSID_IE_, &len2, 
                        len - sizeof(struct ieee80211_hdr_3addr) - _FIXED_IE_LENGTH_)))
        {
            if (len2 > 0)
            {
                if ((len2 == pmlmeinfo->network.Ssid.SsidLength) &&
                    _rtw_memcmp(pmlmeinfo->network.Ssid.Ssid, (p + 2), len2))
                    is_same_ssid = _TRUE;
            }
        }
    }
	if (is_same_ssid || _rtw_memcmp(GetAddr3Ptr(pframe), get_my_bssid(&pmlmeinfo->network), ETH_ALEN))
#else
	if (_rtw_memcmp(GetAddr3Ptr(pframe), get_my_bssid(&pmlmeinfo->network), ETH_ALEN))
#endif
	{
		...
		else if((pmlmeinfo->state&0x03) == WIFI_FW_ADHOC_STATE)
		{
			...
			if (is_same_ssid)
			{
				if (_rtw_memcmp(GetAddr3Ptr(pframe), get_my_bssid(&pmlmeinfo->network), ETH_ALEN) == _FALSE)
				{
					_rtw_memcpy(pmlmeinfo->network.MacAddress, GetAddr3Ptr(pframe), ETH_ALEN);
					_rtw_memcpy(pmlmepriv->cur_network.network.MacAddress, GetAddr3Ptr(pframe), ETH_ALEN);
					padapter->HalFunc.SetHwRegHandler(padapter, HW_VAR_BSSID, GetAddr3Ptr(pframe));
				}
			}
#endif
			if ((psta = rtw_get_stainfo(pstapriv, GetAddr2Ptr(pframe))) != NULL)
...
void issue_beacon(_adapter *padapter)
{
	...
	*(fctrl) = 0;
	...
	// DS parameter set
#ifdef PLATFORM_MPIXEL
	/* handle big endian cpu */
	__le32 val = cpu_to_le32(cur_network->Configuration.DSConfig);
	pframe = rtw_set_ie(pframe, _DSSET_IE_, 1, (unsigned char *)&val, &pattrib->pktlen);
#else
...
void site_survey(_adapter *padapter)
{
	...
	if(survey_channel != 0)
	{
		...
		if((pmlmeext->sitesurvey_res.scan_mode == SCAN_ACTIVE) && (ScanType == SCAN_ACTIVE))
		{
            ...
            mpDebugPrint("%s(): sitesurvey_res.scan_mode= %d", __func__, pmlmeext->sitesurvey_res.scan_mode);
...
u8 collect_bss_info(_adapter *padapter, union recv_frame *precv_frame, WLAN_BSSID_EX *bssid)
{
    ...
    MP_DEBUG("%s:  DSConfig= %d", __func__, bssid->Configuration.DSConfig);
...
void mlmeext_joinbss_event_callback(_adapter *padapter, int join_res)
{
	...
	else if ((pmlmeinfo->state&0x03) == WIFI_FW_ADHOC_STATE)
	{
		if (check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE) == _TRUE)
		{
			...
			memcpy(wrqu.ap_addr.sa_data, cur_network->MacAddress, ETH_ALEN);
...
#ifdef PLATFORM_MPIXEL
	unsigned short	caps;
	caps = rtw_get_capability((WLAN_BSSID_EX *)&pmlmeinfo->network);
	if (caps&cap_IBSS) //adhoc client
	{
