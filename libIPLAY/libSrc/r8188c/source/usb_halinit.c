...
#include <usb_ops.h>
#include <usb_hal.h>
//#include <usb_osintf.h>	//mark for temp
...
_func_enter_;
	...
	urb_disable();	/* DISABLE USB temporary, wait to initiation the receiver buffer completely. */
...
#define EEPROM_NORMAL_CHANNEL_PLAN (0x75)
	...
	MSG_8192C("RegChannelPlan(%d) EEPROMChannelPlan(%ld)", pregistrypriv->channel_plan, Adapter->eeprompriv.channel_plan);
		...
		case HW_VAR_MLME_JOIN:
			{
...
#ifdef PLATFORM_MPIXEL
				else if(type == 3) // prepare to join
				{
					//enable to rx data frame.Accept all data frame
					//rtw_write32(padapter, REG_RCR, rtw_read32(padapter, REG_RCR)|RCR_ADF);
					rtw_write16(Adapter, REG_RXFLTMAP2,0xFFFF);
				...
				else if(type == 4) // joined
				{
					if(IS_NORMAL_CHIP(pHalData->VersionID))
					{
						u32 v = rtw_read32(Adapter, REG_RCR);
						v |= (RCR_CBSSID_BCN|RCR_CBSSID_DATA);
						rtw_write32(Adapter, REG_RCR, v);
					}
					else
					{
