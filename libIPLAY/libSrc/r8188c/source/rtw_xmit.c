...
_lock _mpx_txservq_lock;                        /* just use one lock for all tx_servq */
#endif
...
#ifdef PLATFORM_MPIXEL
	_rtw_init_listhead(&((&ptxservq->sta_pending)->queue));
	if (!_mpx_txservq_lock)
		spin_lock_init(&_mpx_txservq_lock);
	(&ptxservq->sta_pending)->lock = _mpx_txservq_lock;
#else
...
_func_enter_;   	
	...
	_rtw_init_sema(&pxmitpriv->xmit_sema, 0, 0);
	_rtw_init_sema(&pxmitpriv->terminate_xmitthread_sema, 0, 0);
...
#ifdef PLATFORM_MPIXEL
	pxmitpriv->pallocated_xmitbuf = net_buf_mem_zalloc(NR_XMITBUFF * sizeof(struct xmit_buf) + 4);
#else
...
*/		
	...
	_rtw_init_sema(&(pxmitpriv->tx_retevt), 0, 0);
...
#ifdef PLATFORM_MPIXEL
		net_buf_mem_free(pxmitpriv->pallocated_xmitbuf);
#else	
