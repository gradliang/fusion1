...
_func_enter_;
	...
	if (rtw_end_of_queue_search(phead,plist)== _TRUE) {
		...
		if (_rtw_queue_empty(&(pmlmepriv->free_bss_pool)) == _TRUE) {
			...
			//pnetwork->last_scanned = rtw_get_current_time();	//duplicate below
                        //variable initialize
			pnetwork->fixed = _FALSE;			
			pnetwork->last_scanned = rtw_get_current_time();
			...
			//rtw_list_insert_tail(&(pnetwork->list),&(queue->queue)); 
			rtw_list_insert_tail(&(pnetwork->list), get_list_head(queue));
...
_func_enter_;		
	...
	//RT_TRACE(_module_rtl871x_mlme_c_,_drv_info_,("rtw_survey_event_callback, macaddr=" MAC_FMT,  MAC_ARG(pnetwork->MacAddress)));
	...
	if ((check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE)) == _TRUE)
	{
		...
		DBG_8192C("rtw_survey_event_callback : WIFI_ADHOC_MASTER_STATE");
...
_func_enter_;	
	...
	{
#ifdef PLATFORM_MPIXEL
		if(adapter->stapriv.asoc_sta_count== 2) /* TODO */
#else
...
_func_enter_;
	...
	#ifdef CONFIG_ANTENNA_DIVERSITY
	//if(  ((HAL_DATA_TYPE *)(adapter->HalData))->AntDivCfg )
	{
		u8 CurrentAntenna;
