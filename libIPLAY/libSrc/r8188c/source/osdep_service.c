...
*/
/*
*	create: 0 means to use semaphore pool; !=0 means to create new semaphore.
*/
void _rtw_init_sema(_sema	*sema, int init_val, int create)
{
#ifdef PLATFORM_MPIXEL
	int ret;
	if(create != 0)
	{
		...
		{
			case 1:
				ret = SemaphoreCreate(RTL8188_CMD_QUEUE_SEMA_ID, OS_ATTR_PRIORITY, init_val);
				if(ret == OS_STATUS_OK)
				{
					SemaphoreSet(RTL8188_CMD_QUEUE_SEMA_ID, 255);	//to set max value for the specific semaphore
					sema->lock = (spinlock_t)RTL8188_CMD_QUEUE_SEMA_ID;
				}
				else
					__asm("break 100");
				break;
			case 2:
				ret = SemaphoreCreate(RTL8188_TERMINATE_CMDTHREAD_SEMA_ID, OS_ATTR_PRIORITY, init_val);
				if(ret == OS_STATUS_OK)
				{
					SemaphoreSet(RTL8188_TERMINATE_CMDTHREAD_SEMA_ID, 255);	//to set max value for the specific semaphore
					sema->lock = RTL8188_TERMINATE_CMDTHREAD_SEMA_ID;
				}
				else
					__asm("break 100");
				break;
		}
		//mpDebugPrint("%s(): sema id: %x", __func__, sema->lock);
	}
	else
	{
...
{
#ifdef PLATFORM_MPIXEL
	//mpDebugPrint("%s(): sema id = %d\n", __func__, sema->lock);
	SemaphoreRelease(sema->lock);
#endif
...
//__asm("break 100");
	BYTE tmp_plock = 0;
	//mpDebugPrint("%s: id = %d", __func__, *plock);
	if(*plock)
	{
		//__asm("break 100");
		tmp_plock = (BYTE)*plock;
		if(SemaphoreSet(tmp_plock, 1) !=0)
			__asm("break 100");
	}
	else
	{
...
{
#ifdef PLATFORM_MPIXEL
	return systime * SYS_TIMER_TICK_PERIOD;
#endif
#ifdef PLATFORM_LINUX
