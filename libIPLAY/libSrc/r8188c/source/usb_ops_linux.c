...
static int recvbuf2recvframe(_adapter *padapter, _pkt *pskb)
{
	...
	do{		
        ...
        if (!do_early_discard(pbuf + drvinfo_sz + RXDESC_SIZE))
        {
...
#else			
			precvbuf->pskb = netdev_alloc_skb(adapter->pnetdev, MAX_RECVBUF_SZ + RECVBUFF_ALIGN_SZ);//15368 = MAX_RECVBUF_SZ + RECVBUFF_ALIGN_SZ
#endif			
...
#if 0//def PLATFORM_MPIXEL
static void rtl8188cu_usb_write_port_complete(struct urb *purb)
{
	struct xmit_buf *pxmitbuf = (struct xmit_buf *)purb->context;
	struct net_device *dev = pxmitbuf->dev;
	_adapter	*padapter = pxmitbuf->padapter;
	struct xmit_priv	*pxmitpriv = &padapter->xmitpriv;
...
_func_enter_;	
	...
	DBG_8192C("+%s", __func__);

	ret = rtw_free_xmitbuf(pxmitpriv, pxmitbuf);
	if(ret != _SUCCESS)
		__asm("break 100");
...
*/
	...
	usb_fill_bulk_urb(purb, pusbd, pipe,
					pxmitframe->buf_addr,
					cnt,
					rtl8188cu_usb_write_port_complete,
					pxmitbuf);
#else
#if 0//test for 802.11 Prob Request
	u8 *tmp = NULL;
	tmp = net_buf_mem_zalloc(cnt);
	_rtw_memcpy(tmp, pxmitframe->buf_addr, cnt);
	if(tmp != NULL)
		pxmitframe->buf_addr = tmp;
#endif
...
#if 0
#ifdef PLATFORM_MPIXEL
int do_early_discard(u8 *pPkt)
{
	if (GetFrameType(pPkt) == WIFI_MGT_TYPE)
    {
        u8 subtype = GetFrameSubType(pPkt); //bit(7)~bit(2)
