...
//#include <rtw_ioctl_query.h>		//mark for temp
...
{	
	...
	else if(value & AUTH_ALG_OPEN_SYSTEM)
	{
		...
		padapter->securitypriv.ndisencryptstatus = Ndis802_11EncryptionDisabled; /* TODO */
...
#ifdef PLATFORM_MPIXEL	
	channel = rtw_freq2ch(wrqu->freq.m / 100000);

	MP_DEBUG("fw_state = 0x%x", pmlmepriv->fw_state);
	MP_DEBUG("frequency = %d", wrqu->freq.m);
	MP_DEBUG("channel = %d %d", rtw_freq2ch(wrqu->freq.m / 100000), channel);
	MP_DEBUG("IS_92C_SERIAL return %d", IS_92C_SERIAL(pHalData->VersionID));
	MP_DEBUG("interface type return %d", pHalData->BoardType);
	
	/* set RF channel register */
	registry_par->channel = channel;
#endif
	_func_exit_;
