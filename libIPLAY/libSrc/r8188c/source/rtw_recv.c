...
#ifdef PLATFORM_MPIXEL
_lock mpx_rx_lock, mpx_rx_defrag_q_lock;        /* we don't have enough semaphores, so just use two for all */
#endif
...
#ifdef PLATFORM_MPIXEL
	if(!mpx_rx_lock)
		_rtw_spinlock_init(&(mpx_rx_lock));
	if(!mpx_rx_defrag_q_lock)
		_rtw_spinlock_init(&(mpx_rx_defrag_q_lock));

	psta_recvpriv->lock = mpx_rx_lock;

	_rtw_init_listhead(&((&psta_recvpriv->defrag_q)->queue));
	(&psta_recvpriv->defrag_q)->lock = mpx_rx_defrag_q_lock;
#else
...
_func_enter_;
	...
	_rtw_init_sema(&precvpriv->allrxreturnevt, 0, 0);
...
{
#if defined(PLATFORM_LINUX) || defined(PLATFORM_MPIXEL)	//for amsdu TP improvement,Creator: Thomas 
