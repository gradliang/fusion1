...
_func_enter_;
	...
	DBG_8192C("idVendor=%x\n", le16_to_cpu(pdev_desc->idVendor));
...
#endif
	//.3 misc
	_rtw_init_sema(&(padapter->dvobjpriv.usb_suspend_sema), 0, 0);	
...
static void usb_intf_start(_adapter *padapter)
{
	...
	tasklet_enable(&padapter->recvpriv.recv_tasklet);
	tasklet_enable(&padapter->xmitpriv.xmit_tasklet);
#endif
...
static int rtw_drv_init(struct usb_interface *pusb_intf, const struct usb_device_id *pdid)
{
    ...
    GlobalDebugComponents &= ~(_module_hci_ops_os_c_ | _module_rtl871x_recv_c_ | _module_rtl8192c_xmit_c_);
    GlobalDebugComponents &= ~(_module_recv_osdep_c_);
    GlobalDebugComponents &= ~(_module_rtl871x_xmit_c_);
    GlobalDebugComponents &= ~(_module_rtl871x_mlme_c_);

//	MP_ASSERT(pusb_intf->altsetting);
#endif
	...
	pUsbIFDesc = GetInterfaceStruct(pUsbhDevDes,0,0,0);
#if 1
	static struct usb_host_interface _altsetting;
	pusb_intf->altsetting = &_altsetting;
	/* fill parameter value of Interface Configure to pusb_intf->altsetting->desc */
	pusb_intf->altsetting->desc.bLength = pUsbIFDesc->bLength;
	pusb_intf->altsetting->desc.bDescriptorType = pUsbIFDesc->bDescriptorType;
	pusb_intf->altsetting->desc.bInterfaceNumber = pUsbIFDesc->bInterfaceNumber;
	pusb_intf->altsetting->desc.bAlternateSetting = pUsbIFDesc->bAlternateSetting;
	pusb_intf->altsetting->desc.bNumEndpoints = pUsbIFDesc->bNumEndpoints;
	pusb_intf->altsetting->desc.bInterfaceClass = pUsbIFDesc->bInterfaceClass;
	pusb_intf->altsetting->desc.bInterfaceSubClass = pUsbIFDesc->bInterfaceSubClass;
	pusb_intf->altsetting->desc.bInterfaceProtocol = pUsbIFDesc->bInterfaceProtocol;
	pusb_intf->altsetting->desc.iInterface = pUsbIFDesc->iInterface;
#endif
	...
	for (bep_count = pUsbIFDesc->bNumEndpoints ; bep_count > 0; bep_count--)
	{
    ...
    pusb_intf->altsetting->endpoint= &_pEP[0];
#endif
...
#ifdef PLATFORM_MPIXEL
	//mpDebugPrint("bNumConfigurations = %d", pUsbhDevDes->sDeviceDescriptor.bNumConfigurations);
	static struct usb_host_config _pcf;
	struct usb_host_config *pcf = &_pcf;
	USB_CONFIG_DESCRIPTOR    *pUsbCf;
	pUsbCf = pUsbhDevDes->pConfigDescriptor;
	pcf->desc.bLength = pUsbCf->bLength;
	pcf->desc.bDescriptorType = pUsbCf->bDescriptorType;
	pcf->desc.wTotalLength = 0xFFFF & byte_swap_of_word(pUsbCf->wTotalLength);
	pcf->desc.bNumInterfaces = pUsbCf->bNumInterfaces;
	pcf->desc.bConfigurationValue = pUsbCf->bConfigurationValue;
	pcf->desc.iConfiguration = pUsbCf->iConfiguration;
	pcf->desc.bmAttributes = pUsbCf->bmAttributes;
	pcf->desc.bMaxPower = pUsbCf->bMaxPower;
	pdvobjpriv->pusbdev->actconfig = pcf;
#endif
	// set data
...
#ifdef PLATFORM_MPIXEL
	memcpy(&myethaddr, pnetdev->dev_addr, ETH_ALEN);
	/*mpDebugPrint("%s(): myethaddr = %02x-%02x-%02x-%02x-%02x-%02x\n", __func__, 
	myethaddr[0],
	myethaddr[1],
	myethaddr[2],
	myethaddr[3],
...
#ifdef PLATFORM_MPIXEL
	r = usb_register(&drvpriv.rtw_usb_drv);
	if(r)
	{
		printk(KERN_ERR "%s usb_register() failed. Error number %d\n",
		       drvpriv.rtw_usb_drv.name, r);
		return r;
	}
#else	
...
#ifdef PLATFORM_MPIXEL
int mpx_rtl8188cu_usb_module_init()
{
	return rtw_drv_entry();
}
void mpx_rtl8188cu_usb_module_exit()
{
	rtw_drv_halt();
}
