...
#ifdef PLATFORM_MPIXEL
static _adapter rtl8188_padaptg;
_adapter *rtl8188_padapt = &rtl8188_padaptg;
#endif
...
u32 rtw_start_drv_threads(_adapter *padapter)
{
	...
	ret = TaskCreate(WIFI_USB_CMD, rtw_cmd_thread,  WIFI_PRIORITY, 0x2000);
	if(ret != OS_STATUS_OK)
	{
		_status = _FAIL;
		__asm("break 100");
		...
		padapter->cmdThread = WIFI_USB_CMD;
	rtl8188_padapt = (_adapter *)padapter;
	//DBG_8192C("%s(): padapter->cmdThread = %d", __func__, padapter->cmdThread);
	if(padapter->cmdThread != -1)
	{
		TaskStartup(padapter->cmdThread);
	}
#endif			
