...
#include <asm/byteorder.h>		//change to use Linux function
#else
...
_func_enter_;	
	...
	_rtw_init_sema(&(pcmdpriv->cmd_queue_sema), 0, 1);
	//_rtw_init_sema(&(pcmdpriv->cmd_done_sema), 0);
	_rtw_init_sema(&(pcmdpriv->terminate_cmdthread_sema), 0, 2);
...
#ifdef PLATFORM_MPIXEL
	pcmdpriv->enqueue_cnt = pcmdpriv->dequeue_cnt = 0;
#endif
...
_func_enter_;	
...
#ifdef CONFIG_H2CLBK
	_rtw_init_sema(&(pevtpriv->lbkevt_done), 0, 0);
	...
	_rtw_init_sema(&(pevtpriv->evt_notify), 0, 0);
	_rtw_init_sema(&(pevtpriv->terminate_evtthread_sema), 0, 0);
...
_func_enter_;	
	...
	pcmdpriv->enqueue_cnt++;
	//DBG_8192C("%s(): cmd_code = %d; cnt = %d", __func__, cmd_obj->cmdcode, pcmdpriv->enqueue_cnt);
...
_func_enter_;		
	...
	pcmdpriv->dequeue_cnt++;
	cmd_obj = _rtw_dequeue_cmd(&pcmdpriv->cmd_queue);
	//DBG_8192C("%s(): cmd_code = %d; cnt = %d", __func__, cmd_obj->cmdcode, pcmdpriv->dequeue_cnt);
_func_exit_;			
	return cmd_obj;
}
...
#ifdef PLATFORM_MPIXEL
extern _adapter *rtl8188_padapt;
#endif

#ifdef LINUX
thread_return rtw_cmd_thread(thread_context context)
#elif defined(PLATFORM_MPIXEL)
int rtw_cmd_thread(void)
#endif
{
	u8 ret;
	struct cmd_obj *pcmd;
...
#elif defined(PLATFORM_MPIXEL)
       _adapter *padapter = (_adapter *)rtl8188_padapt;
#endif
...
_next:
		...
		//mpDebugPrint("%s(): cmdcode = %d; sizeof = %d\n", __func__, pcmd->cmdcode, (sizeof(wlancmds) /sizeof(struct cmd_hdl)));
		if(pcmd->cmdcode <= (sizeof(wlancmds) /sizeof(struct cmd_hdl)))
		{
			cmd_hdl = wlancmds[pcmd->cmdcode].h2cfuns;
            MP_DEBUG("%s: cmdcode = %d; cmd_hdl = %p", __func__, pcmd->cmdcode, cmd_hdl);
			if (cmd_hdl)
			{
            ...
            MP_DEBUG("%s: callback=%p", __func__, pcmd_callback);
...
_func_enter_;	
	...
	DBG_8192C("%s(): res = %d", __func__, res);
