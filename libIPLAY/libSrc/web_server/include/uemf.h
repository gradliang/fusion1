...
#ifdef UITRON
	#define		O_BINARY		0x10000         /* from sys/fcntl.h */
...
#define E_ARGS				file, line
	...
	#define a_assert(C)		if (C) ; else webs_error(E_L, E_ASSERT, T("%s"), T(#C))
#else
...
/*
 * Picc's note for B_MAX_CLASS: this value is calculated from B_DEFAULT_MEM and
 * B_SHIFT. For the default values, B_DEFAULT_MEM = 64K bytes and B_SHIFT is 4,
 * so B_MAX_CLASS = log2(B_DEFAULT_MEM) - B_SHIFT + 1 = 16 - 4 + 1 = 13.
 */
...
extern void		webs_error(E_ARGS_DEC, int flags, char_t *fmt, ...);
