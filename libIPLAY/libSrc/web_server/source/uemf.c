...
#endif
static void (*errorHandler)(int etype, char_t *msg) = NULL;
static void (*traceHandler)(int level, char_t *buf) = NULL;
...
#ifdef UITRON
/* error() conflicts with a namesake function in another module
 */
void webs_error(E_ARGS_DEC, int etype, char_t *fmt, ...)
#else
