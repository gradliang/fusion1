...
{
	sched_t	*s;
	//mpDebugPrint("TimerProc 0x%x",schedid);
...
int emfSchedCallback(int delay, emfSchedProc *proc, void *arg)
{
	...
	//mpDebugPrint("emfSchedCallback %d %x %x",delay,proc,arg);
 ...
 */
	...
	//mpDebugPrint("schedid %d",schedid);
...
{
	sched_t	*s;
	//mpDebugPrint("emfReschedCallback 0x%x delay %d",schedid,delay);
	if (sched == NULL || schedid == -1 || schedid >= schedMax || 
		(s = sched[schedid]) == NULL) {
		return;
	}
	s->at = ((delay + 500) / 1000) + time(0);
	//mpDebugPrint("s->at %d",s->at);
}
...
{
	sched_t	*s;
	//mpDebugPrint("emfUnschedCallback 0x%x",schedid);
	if (sched == NULL || schedid == -1 || schedid >= schedMax || 
		(s = sched[schedid]) == NULL) {
		return;
	}
	...
	//mpDebugPrint("schedMax 0x%x",schedMax);
}
