 ...
 */
	if ((date = websGetDateString(NULL)) != NULL) {
		...
		/*
		 * (Picc Sep-09-2009)
		 * For IE, the setting of 'Temporary Internet Files' will affect
		 * the behavior of access a proxy but it doesn't affected by HTTP
		 * response header 'Cache-Control: no-cache'. IE will compare the
		 * expiration date against the date of local copy in 'Temporary
		 * Internet Files' folder. Therefore, forcing the date expires
		 * immediately and hence cause IE to do proxy accesses.
		 */
		if ((strstr(websGetRequestUserAgent(wp), "MSIE") != 0) &&
		    (strcmp(lpath, "/nowplaying.jpg")            == 0)) {
...
int websValidateUrl(webs_t wp, char_t *path)
{
    ...
    */
	//mpDebugPrint("websValidateUrl %s",path);
...
static void websDefaultWriteEvent(webs_t wp)
{
	...
	//mpDebugPrint("websDefaultWriteEvent %s",wp->lpath);
 ...
 */
		if ((buf = balloc(B_L, PAGE_READ_BUFSIZE)) == NULL) {
			...
			//mpDebugPrint("written %d",written);
/*
