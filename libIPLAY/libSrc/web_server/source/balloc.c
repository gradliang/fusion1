...
/* NOTE: DO NOT use extended heap to avoid data corruption due to re-init. */
#define malloc(sz)  mpx_Malloc(sz) 
#define free(mem)   mpx_Free(mem)
...
#endif
			/* NOTE: if there is no enough space, we alloc memory from ext. heap */
			bp = (bType*) ext_mem_malloc(memSize);
			if (bp == NULL) {
...
#endif
			/* NOTE: if there is no enough space, we alloc memory from ext. heap */
			if ((bp = (bType*) ext_mem_malloc(memSize)) == NULL) {
				traceRaw(T("B: malloc failed\n"));
				return NULL;
			}
#ifdef B_STATS
...
void bfree(B_ARGS_DEC, void *mp)
{
	...
	if (bp->flags & B_MALLOCED) {
		/* NOTE: free the 'extra' memory previous-allocated from ext. heap */
		ext_mem_free(bp);
		return;
	}
