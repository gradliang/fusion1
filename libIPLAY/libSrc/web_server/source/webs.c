 ...
 */
	...
	if (i > retries) {
		webs_error(E_L, E_USER, T("Couldn't open a socket on ports %d - %d"),
			orig, port - 1);
		return -1;
	} 
...
void websTimeout(void *arg, int id)
{
	...
	tm = websGetTimeSinceMark(wp)*50;
    //mpDebugPrint("tm %d",tm);
	if (tm >= WEBS_TIMEOUT) {
		//mpDebugPrint("\ntm >= WEBS_TIMEOUT\n");
		websStats.timeouts++;
 ...
 */
	if (wp->flags & WEBS_KEEP_ALIVE) {
        ...
        {
            socket_t		*sp;
            if ((sp = socketPtr(wp->sid))) {
                WEBS_DEBUGF(0, ("websDone: s=%d", sp->sock));
            }
        }
		socketCloseConnection(wp->sid);
	}
...
char_t *websGetRequestUserAgent(webs_t wp)
{
	a_assert(websValid(wp));
...
{
	wp->timestamp = time(0);
    //mpDebugPrint("websSetTimeMark wp->timestamp %d",wp->timestamp);
}
...
static int websGetTimeSinceMark(webs_t wp)
{
    //mpDebugPrint("websGetTimeSinceMark wp->timestamp %d",wp->timestamp);
