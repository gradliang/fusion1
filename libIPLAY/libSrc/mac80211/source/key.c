...
void mp_key_todo(struct ieee80211_key *key);
#endif
...
{
#ifdef PLATFORM_MPIXEL
	extern int key_mutex_global;
	if (key_mutex.semaphore == 0)
	{
		key_mutex.semaphore = key_mutex_global;
		mpDebugPrint("%s: id=%d", __func__, key_mutex.semaphore);
	}
#endif
...
#ifdef PLATFORM_MPIXEL
/* 
 * To speed it up, we need to avoid use of todo_list.  So we call these key 
 * functions directly.
 */
void mp_key_todo(struct ieee80211_key *key)
{
