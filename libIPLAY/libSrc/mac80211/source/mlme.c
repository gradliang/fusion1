...
//    mpDebugPrint("%s: mdev=%p,dev=%p", __func__, sdata->local->mdev, skb->dev);
...
#if Make_WPS	
    if(WpsRun)
		mod_timer(&ifsta->timer, (jiffies*1000) +
				     ...
				     u8 *ssid, size_t ssid_len)
{
	...
	skb_reserve(skb, local->hw.extra_tx_headroom);
//    mpDebugPrint("%s: extra_tx_headroom=%d", __func__, local->hw.extra_tx_headroom);
...
//    mpDebugPrint("%s: skb:", __func__);
//    NetPacketDump(skb->data, 64);
	ieee80211_sta_tx(dev, skb, 0);
}
				   ...
				   size_t len)
{
	...
	if (local->ops->sta_notify) {
		local->ops->sta_notify(local_to_hw(local), &sdata->vif,
				       STA_NOTIFY_AUTH, ifsta->bssid);
	}
#endif
    MP_DEBUG("%s: ifsta->auth_alg=%d", __func__, ifsta->auth_alg);
				       ...
				       size_t len)
{
	...
	if (ifsta->flags & IEEE80211_STA_ASSOCIATED)
	{
		...
		//mpDebugPrint("%s: reason_code=%d", __func__, reason_code);
	}
...
//    mpDebugPrint("%s: hash_idx=%d", __func__, hash_idx);
	bss->hnext = local->sta_bss_hash[hash_idx];
	local->sta_bss_hash[hash_idx] = bss;
//    mpDebugPrint("%s: local->sta_bss_hash[hash_idx]=%p", __func__, local->sta_bss_hash[hash_idx]);
}
					...
					struct ieee80211_sta_bss *bss)
{
    ...
    char *buf[32];
    //mpDebugPrint("%s: bssid=%s", __func__, print_mac(buf, bss->bssid));
	while (b) {
		if (b == bss) {
			if (!prev)
		     ...
		     u8 *ssid, u8 ssid_len)
{
	...
	if (ssid && ssid_len <= IEEE80211_MAX_SSID_LEN) {
        ...
        //mpDebugPrint("%s: ssid=%s\n", __func__, wpa_ssid_string(NULL, ssid, ssid_len));
	}
...
//    if (bss==NULL)
//        mpDebugPrint("%s: hash_idx=%d", __func__, STA_HASH(bssid));
	while (bss) {
//        mpDebugPrint("%s: bss->freq=%d,f=%d,bl=%d,l=%d", __func__, 
//                bss->freq, freq, bss->ssid_len, ssid_len);
		if (!bss_mesh_cfg(bss) &&
				 ...
				 struct ieee80211_sta_bss *bss)
{
	...
	if (!atomic_dec_and_lock(&bss->users, &local->sta_bss_lock)) {
		local_bh_enable();
        MP_DEBUG("%s: ssid=%s", __func__,wpa_ssid_string(NULL,bss->ssid, bss->ssid_len));
		return;
	}
...
#else
    extern int local_sta_bss_lock_global;
	local->sta_bss_lock = local_sta_bss_lock_global;
#endif
    //mpDebugPrint("%s: id=%d", __func__,local->sta_bss_lock);
...
//    MP_FUNCTION_ENTER();
    ...
    {
        char buf[64];
        //mpDebugPrint("%s: bss=%s", __func__, print_mac(buf, mgmt->bssid));
    }
	if (!bss) {
#ifdef CONFIG_MAC80211_MESH
		if (elems->mesh_config)
	...
	if (elems->wpa &&
		...
		{
			kfree(bss->wpa_ie);
			bss->wpa_ie = kmalloc((elems->wpa_len + 2)+(elems->wps_len + 2), GFP_ATOMIC);
		}
#endif		
		if (bss->wpa_ie) {
			...
			if(elems->wps)
			{
				memcpy(bss->wpa_ie+bss->wpa_ie_len, elems->wps - 2, elems->wps_len + 2);
				bss->wpa_ie_len += elems->wps_len + 2;
			}
#endif			
    ...
    MP_DEBUG("%s(%d): ieee80211_rx_bss_put", __func__, __LINE__);
	ieee80211_rx_bss_put(local, bss);
//    MP_FUNCTION_EXIT();
}
...
#if 0
    //mpDebugPrint("BEACON SA = %s BSSID= %s\n",print_mac(mac2, mgmt->sa),print_mac(mac3, mgmt->bssid));
    //mpDebugPrint("DEV SA = %x %x %x %x %x %x\n",dev->dev_addr[0],dev->dev_addr[1],dev->dev_addr[2],dev->dev_addr[3],dev->dev_addr[4],dev->dev_addr[5]);
	BSSID = Adhoc_Get_Create_BSSID();
    //mpDebugPrint("DEV BSSID = %x %x %x %x %x %x\n",BSSID[0],BSSID[1],BSSID[2],BSSID[3],BSSID[4],BSSID[5]);
    if((BSSID[0]!=0)||(BSSID[1]!=0)||(BSSID[2]!=0)||(BSSID[3]!=0)||(BSSID[4]!=0)||(BSSID[5]!=0))
    {
       if((!memcmp(BSSID,mgmt->bssid,6))&&!Adhoc_Get_Connect_Status())
       {
          mpDebugPrint("ADHOC CONNECT!!!");
          Adhoc_Set_SA(mgmt->sa);
		  EventSet(WPA_EVENT, 0x2);
...
{
#ifdef PLATFORM_MPIXEL
	static int ibss_link_down_interval;
#endif
	mod_timer(&ifsta->timer, jiffies + IEEE80211_IBSS_MERGE_INTERVAL);
	...
	if (ieee80211_sta_active_ibss(dev))
    {
		...
		if (netif_dormant(dev))
		{
			...
			union iwreq_data wrqu;
			memset(&wrqu, 0, sizeof(wrqu));
			memcpy(wrqu.ap_addr.sa_data, ifsta->bssid, ETH_ALEN);
			mpDebugPrint("%s: ", __func__);
			wireless_send_event(dev, SIOCGIWAP, &wrqu, NULL);
		}
		ibss_link_down_interval = 0;
#endif
		return;
    }

#ifdef PLATFORM_MPIXEL
	if (ibss_link_down_interval++ > 1)
		if (!netif_dormant(dev))
		{
			MP_DEBUG("%s: IBSS link down", __func__);
...
void ieee80211_sta_timer(unsigned long data)
{
    ...
    MP_DEBUG("%s: timer fires", __func__);
...
//    MP_FUNCTION_ENTER();
    ...
    MP_DEBUG("%s: ifsta->request=%x", __func__, ifsta->request);
	if (ifsta->state != IEEE80211_AUTHENTICATE &&
	    ifsta->state != IEEE80211_ASSOCIATE &&
...
//        MP_FUNCTION_EXIT();
	...
	MP_TRACE_LINE();
    MP_DEBUG("%s: ifsta->state=%d", __func__, ifsta->state);
	switch (ifsta->state) {
				     ...
				     struct ieee80211_if_sta *ifsta)
{
	...
	list_for_each_entry(bss, &local->sta_bss_list, list) {
		...
		if ((ifsta->flags & (IEEE80211_STA_AUTO_SSID_SEL |
			IEEE80211_STA_AUTO_BSSID_SEL |
			IEEE80211_STA_AUTO_CHANNEL_SEL)) == (IEEE80211_STA_AUTO_SSID_SEL |
			IEEE80211_STA_AUTO_BSSID_SEL |
			IEEE80211_STA_AUTO_CHANNEL_SEL))
			continue;
#endif
...
#ifdef PLATFORM_MPIXEL
extern void (*NetScanDoneEventSet_Fn)(void);
#endif
...
//    mpDebugPrint("%s: local->sta_sw_scanning set to 0", __func__);
	if (ieee80211_hw_config(local))
		printk(KERN_DEBUG "%s: failed to restore operational "
		       "channel after scan\n", dev->name);
...
#ifdef PLATFORM_MPIXEL
    MP_ALERT("SCAN done");
    if (NetScanDoneEventSet_Fn)
        NetScanDoneEventSet_Fn();
#endif
...
void ieee80211_sta_scan_work(struct work_struct *work)
{
    ...
    //mpDebugPrint("%s: local->scan_state=%d",__func__, local->scan_state);
...
//        mpDebugPrint("%s: nchan=%d",__func__, sband->n_channels);
		/* if no more bands/channels left, complete scan */
		if (!sband || local->scan_channel_idx >= sband->n_channels) {
            //mpDebugPrint("%s: complete scan",__func__);
        ...
        MP_DEBUG("%s: scan_channel_idx=%d",__func__, local->scan_channel_idx);
				    ...
				    u8 *ssid, size_t ssid_len)
{
    ...
    //mpDebugPrint("%s: w=%p", __func__, &local->scan_work);
	queue_delayed_work(local->hw.workqueue, &local->scan_work,
...
int ieee80211_sta_req_scan(struct net_device *dev, u8 *ssid, size_t ssid_len)
{
	...
	if (local->sta_sw_scanning || local->sta_hw_scanning) {
        ...
        {
//            dod1 = 1;
            //mpDebugPrint("%s: SCAN already running",__func__);
			return 0;
        }
        //mpDebugPrint("%s: EBUSY",__func__);
		return -EBUSY;
	}
	...
	if (ssid_len)
    {
        ...
        //mpDebugPrint("%s: ssid=%s",__func__, wpa_ssid_string(NULL, ssid, ssid_len));
    }
    else
    {
        /* ----------  use broadcast  ---------- */
			       ...
			       char *buf, size_t len)
{
	...
	list_for_each_entry(bss, &local->sta_bss_list, list) {
		...
		current_ev = ieee80211_sta_scan_result(dev, info, bss,
						       current_ev, end_buf);
//        mpDebugPrint("%s: current_ev=%p", __func__, current_ev);
	}
	spin_unlock_bh(&local->sta_bss_lock);
...
#ifdef PLATFORM_MPIXEL
	supp_rates = 2048;//54m(bit11);48m(bit10)
#endif
