...
extern int sta_lock_global, sta_flaglock_global;
extern int local_sta_lock_global;
					...
					u8 *addr)
{
	...
	if (local->sta_hash[STA_HASH(addr)] == NULL)
	{
//		mpDebugPrint("%s: local->sta_hash[STA_HASH(addr)] null", __func__);
//		MP_ASSERT(0);
	}
	sta = rcu_dereference(local->sta_hash[STA_HASH(addr)]);
	while (sta) {
//		mpDebugPrint("%s: sta->addr=%p", __func__, sta->addr);
#ifdef LINUX
...
#else
	sta->lock = (spinlock_t) sta_lock_global;
	sta->flaglock = (spinlock_t) sta_flaglock_global;
#endif
...
#else
	local->sta_lock = local_sta_lock_global;
#endif
	mpDebugPrint("%s: sta_lock=%d", __func__, local->sta_lock);
