			       ...
			       size_t klen, u8 *data, size_t data_len)
{
	...
	if (memcmp(&crc, data + data_len, WEP_ICV_LEN) != 0)
		...
		return -1;
#else
    MP_DEBUG("zd_DecryptData");
	if (zd_DecryptData(klen,rc4key, NULL, data_len+WEP_ICV_LEN,data,data, data + data_len) == 0)
	{
		return -1;
	}
//	NetPacketDump(data, 64);
#endif
...
ieee80211_crypto_wep_encrypt(struct ieee80211_tx_data *tx)
{
	...
	if (tx->extra_frag) {
        mpDebugPrint("%s: tx->extra_frag %d", __func__,tx->num_extra_frag);
		int i;
