				    ...
				    struct iw_point *data, char *extra)
{
	...
	if (sdata->vif.type == IEEE80211_IF_TYPE_STA ||
		...
		struct iwreq *iwr = container_of(data, struct iwreq, u);
		mp_zd1205_ioctl(iwr, SIOCSIWGENIE);
#endif
		return 0;
	}
				   ...
				   __u32 *mode, char *extra)
{
	...
	mpDebugPrint("ieee80211_ioctl_siwmode: mode=%u", *mode);
				   ...
				   struct iw_point *data, char *extra)
{
	...
	if (local->sta_sw_scanning || local->sta_hw_scanning)
    {
        mpDebugPrint("%s: EAGAIN (s=%d,h=%d)", __func__, local->sta_sw_scanning, local->sta_hw_scanning);
		return -EAGAIN;
    }
...
#ifdef PLATFORM_MPIXEL
	struct iwreq *iwr = container_of(data, struct iwreq, u);
	mp_zd1205_ioctl(iwr, SIOCSIWAUTH);
#endif
	return ret;
}
...
//#ifdef PLATFORM_MPIXEL
	...
	if (sdata->vif.type == IEEE80211_IF_TYPE_IBSS)
	{
		struct iwreq *iwr = container_of(erq, struct iwreq, u);
		memset(&iwr, 0, sizeof(iwr));
#if Make_WPA
		os_strlcpy(iwr->ifr_name, dev->name, IFNAMSIZ);
#endif
		iwr->u.encoding.flags = idx + 1;
		iwr->u.encoding.pointer = (caddr_t) ext->key;
		iwr->u.encoding.length = ext->key_len;
		mp_zd1205_ioctl(iwr, SIOCSIWENCODE);
	}
#endif
