...
ieee80211_rx_h_defragment(struct ieee80211_rx_data *rx)
{
	...
	while ((skb = __skb_dequeue(&entry->skb_list))) {
		...
		mmcp_memcpy(skb_put(rx->skb, skb->len), skb->data, skb->len);
		dev_kfree_skb(skb);
	}
...
//        mpDebugPrint("%s: skb->protocol=%x", __func__, skb->protocol);
		memset(skb->cb, 0, sizeof(skb->cb));
//		MP_ASSERT(0);
//        mpDebugPrint("%s: skb->data=%p", __func__, skb->data);
		netif_rx(skb);
	}
...
ieee80211_rx_h_amsdu(struct ieee80211_rx_data *rx)
{
	...
	while (skb != frame) {
		...
		if (remaining <= subframe_len + padding)
			...
			mmcp_memcpy(skb_put(frame, ntohs(len)), skb->data,
				ntohs(len));
...
#ifdef LINUX
	CALL_RXH(ieee80211_rx_h_defragment)         /* TODO */
#endif
