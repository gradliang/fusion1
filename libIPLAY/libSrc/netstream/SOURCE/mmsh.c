...
#define lprintf     MP_DEBUG
#define xprintf     lprintf
#define xine_fast_memcpy(d,s,n)     memcpy(d,s,n)
...
int mpx_UrlParse(char *url, char **proto, char** host, int *port, char **uri);
...
static int send_command (mmsh_t *this, char *cmd)
{
  ...
  if (send(this->s, cmd, length, 0) != length) {
    lprintf ("libmmsh: send error\n");
    return 0;
  }
  return 1;
}
...
static int get_answer (mmsh_t *this)
{
  ...
  while (!done) {
    ...
    if (_x_io_tcp_read(this->stream,this->s, (char*)&(this->buf[len]), 1) != 1) {
      lprintf ("libmmsh: alert: end of stream\n");
      return 0;
    }

    if (this->buf[len] == '\012') {             /* LF */
      ...
      if ((len >= 0) && (this->buf[len] == '\015')) {
        ...
        if (linenum == 1) {
			...
			if (sscanf((char*)this->buf, "HTTP/%d.%d %d %50[^\015\012]", &httpver, &httpsub,
                ...
                lprintf ( _("libmmsh: bad response format\n"));
                return 0;
            }

            if (httpcode >= 300 && httpcode < 400) {
                lprintf ( _("libmmsh: 3xx redirection not implemented: >%d %s<\n"),
                        httpcode, httpstatus);
                return 0;
            }

            if (httpcode < 200 || httpcode >= 300) {
                lprintf ( _("libmmsh: http status not 2xx: >%d %s<\n"),
                        httpcode, httpstatus);
                return 0;
            }
        } else {
            ...
            if (!strncasecmp((char*)this->buf, "Location: ", 10)) {
                lprintf ( _("libmmsh: Location redirection not implemented\n"));
                return 0;
            }
  ...
  if (this->stream_type == MMSH_UNKNOWN) {
    lprintf ( "libmmsh: unknown stream type\n");
...
void NetPacketDump(u32_t address, u32_t size);
static int get_chunk_header (mmsh_t *this)
{
  ...
  if (read_len != CHUNK_HEADER_LENGTH) {
    xprintf ( "libmmsh: chunk header read failed, %d != %d\n", read_len, CHUNK_HEADER_LENGTH);
  ...
  //MP_DEBUG ("get_chunk_header: %x %x\n", this->chunk_type, this->chunk_length);
  ...
  if (ext_header_len > 0) {
    ...
    if (read_len != ext_header_len) {
      xprintf ("extended header read failed, %d != %d\n", read_len, ext_header_len);
 ...
 int get_header (mmsh_t *this)
{
  ...
  while (1) {
    ...
    if (get_chunk_header(this))
	{
      if (this->chunk_type == CHUNK_TYPE_ASF_HEADER) 
	  {
		...
		{
          xprintf ( "libmmsh: the asf header exceed %d bytes\n", ASF_HEADER_SIZE);
		...
		else
        {
		  ...
		  {
			mpDebugPrint("get_header: error len=%d", len);
			TaskYield();
            return 0;
          }
		  else
		  {
			  //mpDebugPrint("get_header: len=%d", len);
			  //irready = 1;
		  }
        }
      }
  ...
  if (this->chunk_type == CHUNK_TYPE_DATA) 
  {
    ...
    len = _x_io_tcp_read(this->stream,this->s, (char*)this->buf, this->chunk_length);
  	...
  	//mpDebugPrint("CHUNK_TYPE_DATA len %d",len);
	//mpDebugPrint("this->chunk_length %d",this->chunk_length);
    ...
    {
		//for(i= 0;i<this->chunk_length;i+=16)
		//{
		//  mpDebugPrint(" %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x",
		//	this->buf[i],this->buf[i+1],this->buf[i+2],this->buf[i+3],this->buf[i+4],
		//	this->buf[i+5],this->buf[i+6],this->buf[i+7],this->buf[i+8],this->buf[i+9],
		//	this->buf[i+10],this->buf[i+11],this->buf[i+12],this->buf[i+13],this->buf[i+14],
		//	this->buf[i+15]);
		//  }
...
static int mmsh_tcp_connect(mmsh_t *this)
{
  ...
  this->s = mpx_TcpConnect(this->host, this->port);

  if (this->s < 0) {
    lprintf ("libmmsh: failed to connect '%s'\n", this->host);
    return 1;
  }
  ...
  do {
    ...
    res = mpx_SocketSelect (this->s, SOCKET_WRITE_READY, 500);
    progress += 1;
  } while ((res == -ETIMEDOUT) && (progress < 20));
  if (res != 0) {
    return 1;
  }
  lprintf ("connected\n");
 ...
 * second http request
 */
 int mmsh_connect_int2(mmsh_t *this, int bandwidth) {
  int    i;
  ...
  //mpDebugPrint("this->asf_header->stream_count %x",this->asf_header->stream_count);
  //mpDebugPrint("this->audio_stream %x",this->audio_stream);
  //mpDebugPrint("this->video_stream %x",this->video_stream);
  for (i = 0; i < this->asf_header->stream_count; i++)
  {
    if ((i == this->audio_stream) ||(i == this->video_stream))
	{
	  //mpDebugPrint("stream %d\n", this->asf_header->streams[i]->stream_number);
      size = snprintf(stream_selection + offset, sizeof(stream_selection) - offset,
                      "ffff:%d:0 ", this->asf_header->streams[i]->stream_number);
    }
	else
	{
      //mpDebugPrint("disabling stream %d\n", this->asf_header->streams[i]->stream_number);
      size = snprintf(stream_selection + offset, sizeof(stream_selection) - offset,
                      "ffff:%d:2 ", this->asf_header->streams[i]->stream_number);
    }
	
	//mpDebugPrint("size %x",size);
    if (size < 0)
      return 0;
    offset += size;
  }
  //mpDebugPrint("this->stream_type %x",this->stream_type);
...
mmsh_t *mmsh_connect (xine_stream_t *stream,const char *url, int bandwidth)
{
  ...
  if (mpx_UrlParse (this->url, &this->proto, &this->host, &this->port,
                     &this->uri)) {
    goto fail;
  }
  
  if (!mmsh_valid_proto(this->proto)) {
    lprintf (_("unsupported protocol\n"));
    goto fail;
  }
...
static int get_media_packet (mmsh_t *this) {
  ...
  if (get_chunk_header(this))
  {
    switch (this->chunk_type) {
      ...
      default:
        xprintf ("libmmsh: unexpected chunk type\n");
        return 0;
    }
    ...
    if (len == this->chunk_length) {
      ...
      if (this->chunk_length > this->asf_header->file->packet_size) {
        xprintf ("libmmsh: chunk_length(%d) > packet_length(%d)\n",
      ...
      xprintf ("libmmsh: read error, %d != %d\n", len, this->chunk_length);
      return 0;
    }
  } else {
...
int mmsh_read (mmsh_t *this, char *data, int len)
{
  ...
  while (total < len)
  {
    ...
    if (this->asf_header_read < this->asf_header_len) 
	{
	  ...
	  //lprintf ("n: %d\n",n);
	  //lprintf ("copy %d from this->asf_header_read %d to data %d\n",n,this->asf_header_read,total);
      xine_fast_memcpy (&data[total], &this->asf_header_buffer[this->asf_header_read], n);
	  //NetPacketDump(data,n);
	...
	else 
    {
      ...
      if (bytes_left == 0)
	  {
        ...
        if (packet_type == 0)
		{
          xprintf ("libmmsh: get_media_packet failed\n");
	  ...
	  //lprintf ("from this buf %d copy len  %d to buf %d\n",this->buf_read,n,total);
