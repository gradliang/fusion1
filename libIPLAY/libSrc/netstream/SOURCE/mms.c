...
static void print_command (char *data, int len) {
  ...
  mpDebugPrint("----------------------------------------------\n");
  if (dir == 3) {
    mpDebugPrint ("send command 0x%02x, %d bytes\n", comm, len);
  } else {
    mpDebugPrint ("receive command 0x%02x, %d bytes\n", comm, len);
  }
  mpDebugPrint ("  start sequence %08x\n", _X_LE_32 (data +  0));
  mpDebugPrint ("  command id     %08x\n", _X_LE_32 (data +  4));
  mpDebugPrint ("  length         %8x \n", _X_LE_32 (data +  8));
  mpDebugPrint ("  protocol       %08x\n", _X_LE_32 (data + 12));
  mpDebugPrint ("  len8           %8x \n", _X_LE_32 (data + 16));
  mpDebugPrint ("  sequence #     %08x\n", _X_LE_32 (data + 20));
  mpDebugPrint ("  len8  (II)     %8x \n", _X_LE_32 (data + 32));
  mpDebugPrint ("  dir | comm     %08x\n", _X_LE_32 (data + 36));
  if (len >= 4)
    mpDebugPrint ("  prefix1        %08x\n", _X_LE_32 (data + 40));
  if (len >= 8)
    mpDebugPrint ("  prefix2        %08x\n", _X_LE_32 (data + 44));
...
static int get_packet_header (mms_t *this, mms_packet_header_t *header) {
  ...
  if (_X_LE_32(this->buf + 4) == 0xb00bface) {
    ...
    if (header->packet_id_type == ASF_HEADER_PACKET_ID_TYPE) {
      mpDebugPrint("asf header\n");
      packet_type = MMS_PACKET_ASF_HEADER;
    } else {
      mpDebugPrint("asf packet\n");
...
error:
  mpDebugPrint("read error, len=%zd\n", len);
...
static int mms_tcp_connect(mms_t *this) {
   ...
   * try to connect 
   */
  mpDebugPrint("try to connect to %s on port %d \n", this->host, this->port);
  this->s = mpx_TcpConnect(this->host, this->port);
  if (this->s < 0) {
    mpDebugPrint ("failed to connect '%s'\n", this->host);
    return 1;
  }
  ...
  do {
    ...
    res = mpx_SocketSelect (this->stream, this->s, XIO_WRITE_READY, 500);
    progress += 1;
  } while ((res == XIO_TIMEOUT) && (progress < 30));
  if (res != XIO_READY) {
    return 1;
  }
  mpDebugPrint("connected\n");
  return 0;
}
...
static void mms_gen_guid(char guid[]) {
  ...
  //srand(time(NULL));
  for (i = 0; i < 36; i++) {
    guid[i] = digit[(int) ((16*rand())/(RAND_MAX+1))];
  }
...
mms_t *mms_connect (xine_stream_t *stream, const char *url, int bandwidth) {
  ...
  if (!mmst_valid_proto(this->proto)) {
    mpDebugPrint("unsupported protocol\n");
    goto fail;
  }
  ...
  /* command 0x1 */
  mpDebugPrint("send command 0x01\n");
  ...
  /* command 0x2 */
  mpDebugPrint("send command 0x02\n");
  ...
  /* command 0x5 */
  {
    ...
    mpDebugPrint("send command 0x05\n");
...
void mms_set_start_time (mms_t *this, int time_offset) {
  if (time_offset >= 0)
    this->start_time = (int) time_offset / 1000;
}
