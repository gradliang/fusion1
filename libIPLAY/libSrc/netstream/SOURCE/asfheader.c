...
static uint8_t *asf_reader_get_bytes(asf_reader_t *reader, size_t size) {
  uint8_t *buffer;
	...
	mpDebugPrint("asf_reader_get_bytes %d",size);
...
int asf_find_object_id (GUID *g) {
  ...
  for (i = 1; i < GUID_END; i++) {
    if (!memcmp(g, &guids[i].guid, sizeof(GUID))) {
      //mpDebugPrint("asf_find_object_id: %s\n", guids[i].name);
      return i;
    }
  }
  mpDebugPrint("asf_find_object_id: unknown GUID: 0x%04X, 0x%02X, 0x%02X, {0x%01X, 0x%01X, 0x%01X, 0x%01X, 0x%01X, 0x%01X, 0x%01X, 0x%01X}\n",
...
static int asf_header_parse_file_properties(asf_header_t *header, uint8_t *buffer, int buffer_len) {
  ...
  if (buffer_len < 80) {
    mpDebugPrint("invalid asf file properties object\n");
    return 0;
  }

  asf_file = malloc(sizeof(asf_file_t));
  if (!asf_file) {
    mpDebugPrint("cannot allocate asf_file_struct\n");
    return 0;
  }
...
#if print_message
  mpDebugPrint("File properties\n");
  mpDebugPrint("  file_id:                           %04X\n", asf_file->file_id.Data1);
  mpDebugPrint("  file_size:                         %x\n", asf_file->file_size);
  mpDebugPrint("  data_packet_count:                 %x\n", asf_file->data_packet_count);
  mpDebugPrint("  play_duration:                     %x\n", asf_file->play_duration);
  mpDebugPrint("  send_duration:                     %x\n", asf_file->send_duration);
  mpDebugPrint("  preroll:                           %x\n", asf_file->preroll);
  mpDebugPrint("  broadcast_flag:                    %d\n", asf_file->broadcast_flag);
  mpDebugPrint("  seekable_flag:                     %d\n", asf_file->seekable_flag);
  mpDebugPrint("  packet_size:                       %x\n", asf_file->packet_size);
  mpDebugPrint("  max_bitrate:                       %x\n", asf_file->max_bitrate);
#endif  
  return 1;
}
...
static int asf_header_parse_stream_properties(asf_header_t *header, uint8_t *buffer, int buffer_len) {
  ...
  if (!asf_stream->error_correction_data)
    goto exit_error;
#if print_message
  mpDebugPrint("Stream_properties\n");
  mpDebugPrint("  stream_number:                     %d\n", asf_stream->stream_number);
  mpDebugPrint("  stream_type:                       %s\n", guids[asf_stream->stream_type].name);
  mpDebugPrint("  error_correction_type:             %s\n", guids[asf_stream->error_correction_type].name);
  mpDebugPrint("  time_offset:                       %x\n", asf_stream->time_offset);
  mpDebugPrint("  private_data_length:               %x\n", asf_stream->private_data_length);
  mpDebugPrint("  error_correction_data_length:      %x\n", asf_stream->error_correction_data_length);
  mpDebugPrint("  encrypted_flag:                    %d\n", asf_stream->encrypted_flag);
#endif  
    ...
    if (asf_reader_get_size(&reader) == (object_length - 24)) {
      ...
      switch (object_id) {
        case GUID_ASF_STREAM_PROPERTIES:
          ...
          break;
        default:
          mpDebugPrint ("unexpected object\n");
          break;
      }
    } else {
      mpDebugPrint ("invalid object length\n");
    }
  }
#if print_message
  mpDebugPrint("Stream extension properties\n");
  mpDebugPrint("  stream_number:                     %x\n", stream_number);
  mpDebugPrint("  start_time:                        %x\n", asf_stream_extension->start_time);
  mpDebugPrint("  end_time:                          %x\n", asf_stream_extension->end_time);
  mpDebugPrint("  data_bitrate:                      %x\n", asf_stream_extension->data_bitrate);
  mpDebugPrint("  buffer_size:                       %x\n", asf_stream_extension->buffer_size);
  mpDebugPrint("  initial_buffer_fullness:           %x\n", asf_stream_extension->initial_buffer_fullness);
  mpDebugPrint("  alternate_data_bitrate:            %x\n", asf_stream_extension->alternate_data_bitrate);
  mpDebugPrint("  alternate_buffer_size:             %x\n", asf_stream_extension->alternate_buffer_size);
  mpDebugPrint("  alternate_initial_buffer_fullness: %x\n", asf_stream_extension->alternate_initial_buffer_fullness);
  mpDebugPrint("  max_object_size:                   %x\n", asf_stream_extension->max_object_size);
  mpDebugPrint("  language_id:                       %x\n", asf_stream_extension->language_id);
  mpDebugPrint("  average_time_per_frame:            %x\n", asf_stream_extension->average_time_per_frame);
  mpDebugPrint("  stream_name_count:                 %x\n", asf_stream_extension->stream_name_count);
  mpDebugPrint("  payload_extension_system_count:    %x\n", asf_stream_extension->payload_extension_system_count);
  mpDebugPrint("  reliable_flag:                     %d\n", asf_stream_extension->reliable_flag);
  mpDebugPrint("  seekable_flag:                     %d\n", asf_stream_extension->seekable_flag);
  mpDebugPrint("  no_cleanpoints_flag:               %d\n", asf_stream_extension->no_cleanpoints_flag);
  mpDebugPrint("  resend_live_cleanpoints_flag:      %d\n", asf_stream_extension->resend_live_cleanpoints_flag);
#endif
  return 1;
}
...
static int asf_header_parse_stream_bitrate_properties(asf_header_t *header_pub, uint8_t *buffer, int buffer_len) {
  ...
  for(i = 0; i < bitrate_count; i++) {
    ...
    mpDebugPrint ("  stream num %d, bitrate %x\n", stream_number, bitrate);
...
static int asf_header_parse_header_extension(asf_header_t *header, uint8_t *buffer, int buffer_len) {
  ...
  mpDebugPrint("parse_asf_header_extension: length: %x\n", data_length);
...
asf_header_t *asf_header_new (uint8_t *buffer, int buffer_len) {
  ...
  //mpDebugPrint("parsing_asf_header\n");
  ...
  if (buffer_len < 6) {
    mpDebugPrint("invalid buffer size\n");
    free(asf_header);
    return NULL;
  }
  ...
  if (!asf_header->pub.file) {
    mpDebugPrint("no file object present\n");
    goto exit_error;
  }
  if (!asf_header->pub.content) {
    mpDebugPrint("no content object present\n");
...
void asf_header_choose_streams (asf_header_t *header_pub, uint32_t bandwidth,
  ...
  *video_id = asf_header_choose_stream (header, GUID_ASF_VIDEO_MEDIA, bandwidth_left);
  if (*video_id != -1) {
    ...
    mpDebugPrint("selected video stream %d, bandwidth left: %x\n",
      header->pub.streams[*video_id]->stream_number, bandwidth_left);
  } else {
    mpDebugPrint("no video stream\n");
  }
  ...
  *audio_id = asf_header_choose_stream (header, GUID_ASF_AUDIO_MEDIA, bandwidth_left);
  if (*audio_id != -1) {
    ...
    mpDebugPrint("selected audio stream %d, bandwidth left: %x\n",
      header->pub.streams[*audio_id]->stream_number, bandwidth_left);
  } else {
    mpDebugPrint("no audio stream\n");
  }
}
