...
const unsigned int scm_auiInitP[18] = {
...
const unsigned int scm_auiInitS[4][256] = {
	//0
...
//Function F
static inline unsigned int F(unsigned int ui)
{
...
void BytesToBlock(unsigned char const* p,struct SBlock *b)
{
	unsigned int y;
	//Left
	b->m_uil = 0;
...
void BlockToBytes(struct SBlock  *b, unsigned char* p)
{
	unsigned int y;
	//Right
	y = b->m_uir;
...
void CBlowFish(unsigned char* ucKey, int keysize, struct SBlock *roChain)
{
	...
	int n;
	int iVal;
	//mpDebugPrint("CBlowFish");
	
	//mpDebugPrint("m_ochain %x %x",m_oChain.m_uil,m_oChain.m_uir);
	//mpDebugPrint("m_ochain0 %x %x",m_oChain0.m_uil,m_oChain0.m_uir);
	//mpDebugPrint("roChain %x %x",roChain->m_uil,roChain->m_uir);

	if(keysize<1)
	{
		mpDebugPrint("Incorrect key length %x",keysize);
	}
	...
	for(i=0; i<18; )
	{
		//mpDebugPrint("i %2x ",i);
		Encrypt_BK(&bf_block);
		m_auiP[i++] = bf_block.m_uil;
		m_auiP[i++] = bf_block.m_uir;
	...
	for(j=0; j<4; j++)
	{
		for(k=0; k<256; )
		{
			//mpDebugPrint("j %2x k %2x",j,k);
			Encrypt_BK(&bf_block);
			m_auiS[j][k++] = bf_block.m_uil;
			m_auiS[j][k++] = bf_block.m_uir;
			//mpDebugPrint("bf_block.m_uil %x",bf_block.m_uil);
			//mpDebugPrint("bf_block.m_uir %x",bf_block.m_uir);
		}
	}
...
void Encrypt_BK(struct SBlock *block)
{
	//mpDebugPrint("Encrypt_BK");

	unsigned int uiLeft = block->m_uil;
	unsigned int uiRight = block->m_uir;
	
	//mpDebugPrint("uiLeft %2x ",uiLeft);

	//mpDebugPrint("m_auiP[0] %2x ",m_auiP[0]);
	uiLeft ^= m_auiP[0];
	
	//mpDebugPrint("uiLeft %2x ",uiLeft);
	uiRight ^= F(uiLeft)^m_auiP[1]; 
	
	//mpDebugPrint("uiRight %2x ",uiRight);
...
void Decrypt_BK(struct SBlock *block)
{
	unsigned int uiLeft = block->m_uil;
	unsigned int uiRight = block->m_uir;
	uiLeft ^= m_auiP[17];
...
void Encrypt(unsigned char* buf, int n, int iMode)
{
	struct SBlock work;
	//struct SBlock chain(m_oChain);

	//Check the buffer's length - should be > 0 and multiple of 8
	if((n==0)||(n%8!=0))
		mpDebugPrint("Incorrect buffer length");
	if(iMode == CBC) //CBC mode, using the Chain
	{
		for(; n >= 8; n -= 8)
		{
			...
			//work ^= chain;
			work.m_uil ^= m_oChain.m_uil;
			work.m_uir ^= m_oChain.m_uir;
			Encrypt_BK(&work);
			m_oChain = work;
...
//Returns false if n is multiple of 8
void Decrypt(unsigned char* buf, int n, int iMode)
{
	struct SBlock work;
	struct SBlock crypt;
	//struct SBlock chain(m_oChain);
...
//Returns false if n is multiple of 8
void Encrypt_IO(const unsigned char* in, unsigned char* out, int n, int iMode)
{
	struct SBlock work;
	work.m_uil = 0;
	work.m_uir = 0;
	...
	if(iMode == CBC) //CBC mode, using the Chain
	{
		for(; n >= 8; n -= 8, in += 8)
		{
			...
			//mpDebugPrint("m_oChain.m_uil %x",m_oChain.m_uil);
			work.m_uir ^= m_oChain.m_uir;
			
			//mpDebugPrint("m_oChain.m_uir %x",m_oChain.m_uir);
...
//Returns false if n is multiple of 8
void Decrypt_IO(const unsigned char* in, unsigned char* out, int n, int iMode)
{
	struct SBlock work;
	struct SBlock crypt;//,chain(m_oChain);
